...
class WPFN_Contacts_Table extends WP_List_Table {
    ...
    protected function column_default( $item, $column_name ) {
        switch ( $column_name ) {
            case 'email':
                ...
                $html .= '</div>';
                $html .= "<a href='$editUrl'>{$item[ $column_name ]}</a>";
                return $html;
                break;
            case 'user_id':
    ...
    protected function get_views() {
        ...
        return apply_filters( 'contact_views', array(
            ...
            'opted_out' => "<a class='" . ($view === 'opted_out' ? 'current' : '') . "' href='" . $base_url . "opted_out" . "'>" . __( 'Opted Out <span class="count">('.$count['opted_out'].')</span>' ) . "</a>"
        ) );
    }
    ...
    function prepare_items() {
            ...
            case 'optin_status':
                ...
                if ( isset( $_REQUEST['optin_status'] ) ){
                    switch ( $_REQUEST['optin_status'] ){
                        case 'unconfirmed':
                            $view = ' AND optin_status = 0';
                            break;
                        case 'confirmed':
                            $view = ' AND optin_status = 1';
                            break;
                        case 'opted_out':
                            $view = ' AND optin_status = 2';
                            break;
                        default:
                            break;
                    }
        ...
        if ( isset( $_REQUEST['s'] ) ){
            $pattern = "%" . $wpdb->esc_like( $_REQUEST['s'] ) . "%" ;
            $data =  $wpdb->get_results(
                "
	SELECT c.*, u.`user_login` AS `owner`
	FROM $table_name c
	LEFT JOIN `" . $wpdb->prefix . "users` u ON u.`ID` = c.`owner_id`
	WHERE (email LIKE '$pattern' OR first_name LIKE '$pattern' OR last_name LIKE '$pattern') $view
