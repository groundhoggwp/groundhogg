...
class Create_Users_Rest extends Bulk_Job {
	...
	public function query( $items ) {
		...
		$args  = [
			'tags_include' => wp_parse_id_list( $this->get_context( 'tags_include'  , []) ),
			'tags_exclude' => wp_parse_id_list( $this->get_context( 'tags_exclude'  , []) )
		];
	...
	public function max_items( $max, $items ) {
		...
		return min( 20, intval( $this->items_per_request) );
	}
...
//    	$config = get_transient( 'gh_create_user_job_config' );
		...
		$this->send_notification = $this->get_context( 'send_email' );
		$this->role              = $this->get_context( 'role', 'subscriber' );
	}
