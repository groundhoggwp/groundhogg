...
class Export_Contacts_Rest extends Bulk_Job {
	 ...
	 */
	function get_action() {
		return 'gh_export_contacts_rest';
	}
	...
	protected function pre_loop() {
		...
		$headers = $this->get_context( 'headers' );

//		$file_name = Plugin::$instance->settings->get_transient( 'gh_export_file' );
//		$file  = $this->get_context( 'file' );
//		$file_name = $file ["file_name"];

		$file_name = $this->get_context('file_name');
...
//		}
		...
		if (count( file( $file_path, FILE_SKIP_EMPTY_LINES ) ) - 1  <=0) {
			$fp = fopen( $file_path, "w" );
			fputcsv( $fp, $headers );
		}
	...
	protected function get_response_data() {
		$filepath = files()->get_csv_exports_dir( $this->get_context( 'file_name') );
		return [
			"file" => get_csv_file_info( $filepath )
		];
	}
