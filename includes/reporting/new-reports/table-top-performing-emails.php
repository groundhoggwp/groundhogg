...
class Table_Top_Performing_Emails extends Base_Table_Report {
	...
	protected function get_top_emails() {
		...
		foreach ( $emails as $email ) {
			...
			$report = $this->get_email_stats( $email->ID );

			$title = $email->title ? $email->title : $email->subject;
...
//			'opened' => $item_data [ 'opened' ]
	...
	public function get_email_stats( $email_id ) {
		...
		if ( ! empty( $steps_ids ) ) {
			...
			$where_opened = [
				'relationship' => "AND",
				[ 'col' => 'email_id', 'val' => $email_id, 'compare' => '=' ],
