...
//            var_dump( date( 'Y-m-d H:i:s', $this->start_range ) );
    ...
    protected function extra_widget_info()
    {
        ...
        /* ALL SENDS */
        $num_emails_sent = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(e.ID) FROM $events AS e LEFT JOIN $steps AS s ON e.step_id = s.ID WHERE $this->start_time < e.time AND e.time <= $this->end_time AND ( s.step_type = 'send_email' OR e.funnel_id = %d OR e.event_type = %d ) ", WPGH_BROADCAST, GROUNDHOGG_EMAIL_NOTIFICATION_EVENT ) );
        /* ALL OPENS */
