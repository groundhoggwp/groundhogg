...
import { useEffect, useRef, RawHTML, Fragment } from "@wordpress/element";
...
const ConvertToBlocksButton = ({ clientId }) => {
  ...
  return (
    <ToolbarButton
      onClick={() =>
        replaceBlocks(block.clientId, rawHandler({ HTML: serialize(block) }))
      }
    >
...
registerBlockType("groundhogg/text-block", {
  ...
  keywords: [__("Groundhogg- TEXT block"), __("text")],
  attributes: {
    content: {
      type: "string",
      source: "html",
