...
import { Component, RawHTML, cloneElement, renderToString } from '@wordpress/element';
...
export function serializeBlock(block) {
  var _ref = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {},
      ...
      renderCallback = _ref.__experimentalRenderCallback;

  var blockName = block.name;
  var saveContent = getBlockContent(block); // Serialized block content before wrapping it with an InnerBlocks item
  // wrapper.
  ...
  if (blockName === getUnregisteredTypeHandlerName() || !isInnerBlocks && blockName === getFreeformContentHandlerName()) {
    ...
    unwrappedContent = getCommentDelimitedContent(blockName, saveAttributes, saveContent);
  }

  if (renderCallback) {
    return renderToString(renderCallback(createElement(RawHTML, null, unwrappedContent)));
  }
