...
export default () => {
  ...
  const { tempState, setTempState, resetTempState } = useTempState(
    step.settings
  );
    ...
    <>
      ...
      <div className={"edit-panel"}>
        <StepType.edit
          data={step.data}
          settings={tempState}
          updateSettings={setTempState}
        />
        {!objEquals(tempState, step.settings) && (
          <SettingsRow>
            <div className={classes.saveButtons}>
              ...
              <Button onClick={resetTempState} variant={"contained"}>
                {"Cancel"}
              </Button>
            </div>
          </SettingsRow>
