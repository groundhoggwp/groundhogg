...
class Tag_Relationships extends DB {
	 ...
	 * Only update tag values that do not already exist for the primary contact record
	...
	public function contacts_merged( $primary, $other ) {
		...
		WHERE `contact_id` = {$other->get_id()} AND `tag_id` NOT IN (
			SELECT `tag_id` FROM {$this->table_name} WHERE `contact_id` = {$primary->get_id()}
		); " );
