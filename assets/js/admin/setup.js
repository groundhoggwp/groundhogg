(($) => {
  ...
  let optedIntoTelementry, acceptedMarketing, installedMailhawk, isLicensed = false
    ...
    return `
		<div class="gh-panel outlined download">
			...
			<div class="plugin-actions space-between">
				<a href="#" data-id="${download.info.id}" class="more-details" target="_blank">${__('More details')}</a>
				<button data-link="${download.info.link}&utm_source=plugin&utm_medium=extension&utm_campaign=guided_setup&utm_content=${currentStep.id}" data-id="${download.info.id}" class="link gh-button buy-download secondary">
    ...
    modal({
      ...
      content: `
		  ...
		  <div id="more-details-content">
			  ...
			  <p><a href="${download.info.link}">${__('View on Groundhogg.io')}</a></p>
  ...
  const steps = [
        ...
        return stepTemplate({
          ...
          inside: () => {
            ...
            return `
				...
				<p>
					${__('In just a few minutes you\'ll have Groundhogg configured for your site! Click the button below to start the guided setup.', 'groundhogg')}</p>
				<div class="space-between align-center" style="margin-top: 40px">
					<button id="start" class="gh-button primary big">${__('Let\'s get started!', 'groundhogg')}
					</button>
				</div>`
          }
        })
        ...
        return stepTemplate({
          ...
          inside: () => {
            ...
            return `
				...
				<p>
					${__('Your business information is needed so your list knows who is sending them emails. All this information will appear in your email footer.', 'groundhogg')}</p>
				<div class="gh-rows-and-columns">
					<div class="gh-row">
						<div class="gh-col">
							<label for="business-name">${__('Business Name', 'groundhogg')}</label>
							${input({
								name: 'gh_business_name',
								id: 'business-name',
								placeholder: __('My Business', 'groundhogg'),
								value: gh_business_name ? gh_business_name : blogname
							})}
						</div>
						<div class="gh-col">
							<label for="business-name">${__('Business Phone', 'groundhogg')}</label>
							${input({
								type: 'tel',
								name: 'gh_phone',
								id: 'business-phone',
				...
				<div class="gh-rows-and-columns">
					...
					<div class="gh-row">
						...
						<div class="gh-col">
							<label for="state">${__('State', 'groundhogg')}</label>
							${input({
								name: 'gh_region',
								id: 'state',
								value: gh_region
				...
				<div class="space-between align-right gap-10" style="margin-top: 40px">
					<button id="skip" class="gh-button secondary text">${__('Skip', 'groundhogg')}</button>
					<button id="next" class="gh-button primary">${__('Next 👉', 'groundhogg')}</button>
				</div>`
          }
        })
      },
        ...
        return stepTemplate({
          inside: () => {
            ...
            return `
				...
				<p>
					${__('Answer the questions below and we\'ll configure Groundhogg to be in compliance with the relevant privacy and marketing regulations.', 'groundhogg')}</p>
				<div class="gh-rows-and-columns">
					<div class="gh-row">
						<div class="gh-col">
							<label for="privacy-policy">${__('Link to Privacy Policy', 'groundhogg')}</label>
							${input({
								type: 'url',
						...
						<div class="gh-col">
							<label for="terms">${__('Link to Terms & Conditions', 'groundhogg')}</label>
							${input({
								type: 'url',
								className: 'link-picker input',
								name: 'gh_terms',
        ...
        return stepTemplate({
          inside: () => {
            ...
            return `
				...
				<p>
					${__('Who do you want your emails to send from? <i>This can always be changed later.</i>', 'groundhogg')}</p>
				<div class="gh-rows-and-columns">
					<div class="gh-row">
						<div class="gh-col">
							<label for="from-name">${__('From Name', 'groundhogg')}</label>
							${input({
						...
						<div class="gh-col">
							<label for="from-email">${__('From Address', 'groundhogg')}</label>
							${input({
								type: 'email',
								name: 'gh_override_from_email',
								id: 'from-email',
								value: gh_override_from_email,
								placeholder: __('john@example.com', 'groundhogg')
							})}
						</div>
					</div>
				</div>
        ...
        return stepTemplate({
          inside: () => {
            ...
            return `
				...
				<p>
					${__('If you have previously purchased a license for Groundhogg you can enter it now! <i><a href="https://www.groundhogg.io/account/licenses/" target="_blank">Where do I find my license?</a></i>', 'groundhogg')}</p>
    ...
    },
    {
          ...
          return stepTemplate({
            inside: () => {
              ...
              return `
				  ...
				  <p><i>${__('You can opt out at any time.', 'groundhogg')}</i></p>
				  <div class="space-between align-center" style="margin-top: 40px">
					  <button id="skip" class="gh-button secondary text">
						  ${__('No thanks, I don\'t want to help improve Groundhogg.', 'groundhogg')}
					  </button>
				  </div>`
            }
          })
        ...
        return stepTemplate({
          inside: () => {
            ...
            return `
				...
				<p>
					${__('You can help improve Groundhogg and get 15% OFF the first year of any paid plan by enabling anonymous telemetry.', 'groundhogg')}</p>
				<div class="inside display-flex column align-center gap-20">
				...
				<div class="space-between align-center" style="margin-top: 40px">
					<button id="skip" class="gh-button secondary text">
						${__('No thanks, I don\'t want to save 15% OFF.', 'groundhogg')}
					</button>
				</div>`
          }
        })
      ...
      next: () => isLicensed ? (!GroundhoggGuidedSetup.mailhawkInstalled ? 'mailhawk' : 'community') : 'integrations'
    },
    {
      id: 'integrations',
      render: () => {
        ...
        return stepTemplate({
          inside: () => {
            ...
            return `
				...
				<p>
					${__('We noticed you\'re using some plugins which we integrate with! Integrations can unlock powerful marketing and segmentation features you can use to increase leads & sales.', 'groundhogg')}</p>
				<div id="services">
					${GroundhoggGuidedSetup.integrations.map(d => downloadTemplate(d)).join('')}
				</div>
				<p>
					${__('You can unlock all these integrations by signing up for our <b>Pro Plan</b>.', 'groundhogg')}</p>
				<div class="display-flex column gap-20 inside align-center">
					...
					<button data-link="https://groundhogg.io/pricing/?utm_source=plugin&utm_medium=button&utm_campaign=guided_setup&utm_content=integrations" class="link gh-button primary medium"><b>${__('Get PRO Now!')}</b></button>
				</div>
      ...
      onMount: ({ next, prev }) => {
        ...
        $('.more-details').on('click', e => {
          ...
          let download = GroundhoggGuidedSetup.integrations.find(d => d.info.id == downloadId)
      ...
      next: () => !GroundhoggGuidedSetup.mailhawkInstalled ? 'mailhawk' : 'subscribe'
    },
    {
      id: 'mailhawk',
      render: () => {
        ...
        return stepTemplate({
          ...
          inside: () => {
            ...
            return `
				...
				<h2>${__('A better WordPress SMTP plugin.', 'groundhogg')}</h2>
				<p>
					${__('Use MailHawk to deliver your email! We get that sending email can be a pain. MailHawk makes it painless to send email with excellent deliverability and no-nonsense pricing starting from just <b>$10/year</b>.', 'groundhogg')}</p>
				<div class="display-flex column gap-20 inside align-center">
					<button id="install-mailhawk" class="gh-button primary medium"><b>${__('Install MailHawk!')}</b>
					</button>
				</div>
      ...
      onMount: ({ next, prev }) => {
        ...
        $('#install-mailhawk').on('click', (e) => {
          ...
          ajax(data).then(r => {
            ...
            dialog({
              message: __('MailHawk was installed. You will be able to complete the setup at the end.', 'groundhogg')
            })
      ...
      next: () => installedMailhawk ? 'subscribe' : (isLicensed ? 'community' : 'smtp')
    },
    {
      id: 'smtp',
      render: () => {
        ...
        return stepTemplate({
          inside: () => {
            ...
            return `
				...
				<p>
					${__('We\'ve detected you\'re not using an official Groundhogg SMTP service. For best results we recommend using one of the options listed below.', 'groundhogg')}</p>
				<div id="services">
					${GroundhoggGuidedSetup.smtpProducts.filter(d => d.info.id !== 90048).map(d => downloadTemplate(d)).join('')}
				</div>
				<p>
					${__('You can unlock all these SMTP services by signing up for our <b>Pro Plan</b>.', 'groundhogg')}</p>
				<div class="display-flex column gap-20 inside align-center">
					${optedIntoTelementry ? `<p class="pill green">${__('Use code <b>IFOUND15OFF</b> to save 15% off your first year!', 'groundhogg')}</p>` : ''}
					<button data-link="https://groundhogg.io/pricing/?utm_source=plugin&utm_medium=button&utm_campaign=guided_setup&utm_content=smtp" class="link gh-button primary medium"><b>${__('Get PRO Now!')}</b></button>
				</div>
      ...
      onMount: ({ next, prev }) => {
        ...
        $('.more-details').on('click', e => {
          ...
          let downloadId = $link.data('id')
          let download = GroundhoggGuidedSetup.smtpProducts.find(d => d.info.id == downloadId)
        ...
        return stepTemplate({
          inside: () => {
            ...
            return `
				...
				<div class="space-between align-center" style=" margin-top: 40px">
					<button id="skip" class="gh-button secondary text">
						${__('I don\'t want to stay informed...', 'groundhogg')}
					</button>
				</div>`
          }
        })
        ...
        return stepTemplate({
          inside: () => {
            ...
            return `
				...
				<div class="space-between align-center" style=" margin-top: 40px">
					<button id="next" class="gh-button medium primary">${__('Next 👉', 'groundhogg')}</button>
				</div>`
          }
        })
      },
    ...
    },
    {
          ...
          inside: () => {
            ...
            return `
				...
				${installedMailhawk ? `<div class="display-flex gap-20 space-between">
					...
					<button id="mailhawk" class="gh-button space-between gap-10 primary  medium">${__('Go to MailHawk')}
					</button>
				</div>` : ''}
				<div class="display-flex gap-20 space-between">
					<div>
					...
					<button data-target="_blank" data-link="https://academy.groundhogg.io/course/groundhogg-quickstart/?utm_source=plugin&utm_medium=button&utm_campaign=guided_setup&utm_content=next-steps"
				...
				<div class="display-flex gap-20 space-between">
					...
					<button data-target="_blank" data-link="https://groundhogg.io/pricing/?utm_source=plugin&utm_medium=button&utm_campaign=guided_setup&utm_content=next-steps" class="link gh-button space-between gap-10 secondary medium">${__('Sign up!')}</button>
				</div>
				...
				`}
				<div class="space-between align-center" style=" margin-top: 40px">
					<button data-target="_self" data-link="${adminPageURL('groundhogg')}"
					        class="link gh-button medium primary">${__('Finish! 🎉', 'groundhogg')}
					</button>
				</div>`
          }
        })
      ...
      onMount: ({ next, prev }) => {
        ...
        $('#mailhawk').on('click', () => {
          ...
          let { register_url = '', client_state = '', redirect_uri = '', partner_id = '' } = installedMailhawk
