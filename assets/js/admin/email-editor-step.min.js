(function($){const{slot:slot,fill:fill,slotsDemounted:slotsDemounted,slotsMounted:slotsMounted,registerStepType:registerStepType,updateCurrentStepMeta:updateCurrentStepMeta,stepTitle:stepTitle,renderStepEdit:renderStepEdit,getCurrentStep:getCurrentStep,getCurrentStepMeta:getCurrentStepMeta,getSteps:getSteps}=Groundhogg.funnelEditor.functions;const EmailsStore=Groundhogg.stores.emails;const{select:select,input:input,tinymceElement:tinymceElement,specialChars:specialChars,inputWithReplacementsAndEmojis:inputWithReplacementsAndEmojis}=Groundhogg.element;const CreateEmail=(selector,onSelect)=>({view:"choices",renderTemplates:function(){},renderTemplates:function(){},renderChoices:function(){return`
		  ...
		  </div>`},mount:function(){const reMount=()=>{this.mount()};const setView=view=>{this.view=view;reMount()};$(selector).html(this.render());switch(this.view){case"choices":$(`${selector} .choice`).on("click",function(e){const choice=$(this).data("choice");switch(choice){case"template":setView("template");break;case"existing":setView("existing");break;case"scratch":break}});break}}});fill("beforeStepNotes.send_email",{render:function({data:data,meta:meta}){const{email_id:email_id}=meta;const email=EmailsStore.get(email_id);if(!email_id||!email){return""}const fromOptions={};Groundhogg.filters.owners.forEach(owner=>{fromOptions[owner.ID]=`${owner.data.display_name} (${owner.data.user_email})`});return`
		  <div id="email-options" class="panel">
			  ...
			  <div class="row">
				  ...
				  ${select({id:"from-user",name:"form_user"},fromOptions,email.data.from_user)}
			  </div>
			  <div class="row">
				  <label class="row-label">Replies are sent to...</label>
				  ${input({id:"reply-to",name:"reply_to",value:email.meta.reply_to_override})}
			  </div>
			  <div id="alignment-and-message-type" class="row">
				  ...
				  <div class="row">
					  ...
					  ${select({id:"message-type",name:"message_type"},{marketing:"Marketing",transactional:"Transactional"},email.meta.message_type)}
				  </div>
			  </div>
		  </div>`},onMount:function({meta:meta},updateStepMeta){const{email_id:email_id}=meta;if(!email_id){return}}});fill("beforeStepSettings.send_email",{render:function({data:data,meta:meta}){const{email_id:email_id}=meta;if(!email_id){return""}const headers={"x-header":"x-value"};const keyValueRow=(key,value,index)=>{return`
			...
			</div>`};const headerRows=[];for(const header in headers){if(headers.hasOwnProperty(header)){headerRows.push({key:header,value:headers[header]})}}return`
		  <div id="email-editor" class="panel">
			  <div class="row">
				  ...
				  <div class="inline-label subject-wrap">
					  <label>Preview:</label>
					  ${inputWithReplacementsAndEmojis({name:"preview",placeholder:"Preview text..."})}
				  </div>
			  </div>
		  ...
		  </div>`},onMount:function({meta:meta},updateStepMeta,updateStep){const{email_id:email_id}=meta;if(!email_id){return}tinymceElement("email-content",{},content=>{console.log(content)})},onDemount:function(){wp.editor.remove("email-content")}})})(jQuery);