(function($){const{slot:slot,fill:fill,slotsDemounted:slotsDemounted,slotsMounted:slotsMounted,registerStepType:registerStepType,updateCurrentStepMeta:updateCurrentStepMeta,stepTitle:stepTitle,renderStepEdit:renderStepEdit,getCurrentStep:getCurrentStep,getCurrentStepMeta:getCurrentStepMeta,getSteps:getSteps}=Groundhogg.funnelEditor.functions;const{select:select,input:input,tinymceElement:tinymceElement,specialChars:specialChars}=Groundhogg.element;fill("beforeStepNotes.send_email",{render:function({data:data,meta:meta}){const{email_id:email_id}=meta;if(!email_id){return""}const ownerOptions={};Groundhogg.filters.owners.forEach(owner=>{ownerOptions[owner.ID]=`${owner.data.display_name} (${owner.data.user_email})`});return`
		  <div id="email-options" class="panel">
			  ...
			  <div class="row">
				  ...
				  ${select({id:"from-user",name:"form_user"},ownerOptions)}
			  </div>
			  <div class="row">
				  <label class="row-label">Replies are sent to...</label>
				  ${input({id:"reply-to",name:"reply_to"})}
			  </div>
			  <div id="alignment-and-message-type" class="row">
				  ...
				  <div class="row">
					  ...
					  ${select({id:"message-type",name:"message_type"},{marketing:"Marketing",transactional:"Transactional"})}
				  </div>
			  </div>
		  </div>`}});fill("afterStepSettings.send_email",{render:function({data:data,meta:meta}){const{email_id:email_id}=meta;if(!email_id){return""}const headers={"x-header":"x-value"};const keyValueRow=(key,value,index)=>{return`
		  ...
		  <div id="email-editor" class="panel">
			  <div class="row">
				  <div class="inline-label subject-wrap">
					  <label>Subject:</label>
					  ${input({name:"subject",placeholder:"Subject line..."})}
				  </div>
				  <div class="inline-label subject-wrap">
					  <label>Preview:</label>
					  ${input({name:"preview",placeholder:"Preview text..."})}
				  </div>
			  </div>
		  ...
		  </div>`},onMount:function(step,updateStepMeta,updateStep){tinymceElement("email-content",{},content=>{console.log(content)})},onDemount:function(){wp.editor.remove("email-content")}})})(jQuery);