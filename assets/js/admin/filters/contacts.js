( function ($) {
  ...
  const createFilters = (el = '', filters = [], onChange = (f) => {
    ...
    el, onChange, filters: Array.isArray(filters) ? filters : [], id: uid(),
  ...
  const registerFilter = (type, group = 'general', name = '', opts = {}) => {
    ...
    ContactFilterRegistry.registerFilter(createFilter(type, name, group, {
      display: view,
      preload,
      edit: ({ updateFilter, ...filter }) => Fragment([edit(filter)], {
        onCreate: el => {
          // Do the onmount
          setTimeout(() => {
  ...
  const filterCountDefaults = {
    count: 1, count_compare: 'greater_than_or_equal_to',
  }
    ...
    return `
        <div class="space-between" style="gap: 10px">
            <div class="gh-input-group">
                ${ select({
                    id: 'filter-count-compare', name: 'count_compare',
...
//  REGISTER ALL FILTERS HERE
  const BasicTextFilter = (name) => ( {
    name, view ({ compare, value }) {
      return ComparisonsTitleGenerators[compare](`<b>${ name }</b>`,
        `<b>"${ value }"</b>`)
    }, edit ({ compare, value }) {
      // language=html
      return `${ select({
          id: 'filter-compare', name: 'compare',
      }, StringComparisons, compare) } ${ input({
          id: 'filter-value', name: 'value', value,
      }) }`
    }, onMount (filter, updateFilter) {
      // console.log(filter)
  ...
  registerFilter('phone', 'contact', {
    ...
    edit ({ phone_type, compare, value }) {
      // language=html
      return `${ select({
          id: 'filter-phone-type', name: 'phone_type',
    ...
    defaults: {
      phone_type: 'primary', compare: 'equals', value: '',
    },
  })
  ...
  registerFilter('optin_status', 'contact', __('Opt-in Status', 'groundhogg'), {
    view ({ compare, value }) {
      ...
      return `
          ${ select({
              id: 'filter-compare', name: 'compare', class: '',
          }, {
          ...
          ${ select({
              ...
              multiple: true,
                  ...
                  map(k => ( { value: k, text: optin_status[k] } )), value) } `
    }, onMount (filter, updateFilter) {
  ...
  registerFilter('is_marketable', 'contact', __('Marketable', 'groundhogg'), {
    view ({ marketable }) {
      ...
      return `
          ${ select({
              id: 'filter-marketable', name: 'marketable',
          }, {
  ...
  registerFilter('owner', 'contact', __('Owner', 'groundhogg'), {
    view ({ compare, value = [] }) {
      ...
      return `
          ...
          ${ select({
                      id: 'filter-value', name: 'value', multiple: true,
  ...
  registerFilter('tags', 'contact',
    _x('Tags', 'noun referring to contact segments', 'groundhogg'), {
      view ({ tags = [], compare, compare2 }) {
        ...
        ${ select({
            id: 'filter-compare2', name: 'compare2',
        }, {
            any: __('Any', 'groundhogg'), all: __('All', 'groundhogg'),
        }, compare2) }
        ...
        ${ select({
            ...
            value: t.ID, text: t.data.tag_name,
        } )), tags.map(t => t.ID)) }`
      }, onMount (filter, updateFilter) {
        ...
        compare: 'includes', compare2: 'any', tags: [],
      }, preload: ({ tags }) => {
  ...
  registerFilter('country', 'location', __('Country', 'groundhogg'), {
    view ({ country }) {
      ...
      return `
          ${ select({
              id: 'filter-country', name: 'country',
          }, countries, country) }`
    }, onMount (filter, updateFilter) {
  ...
  registerFilter('email_received', 'activity',
    __('Email Received', 'groundhogg'), {
      view ({ email_id, ...rest }) {
        ...
        ...standardActivityDateDefaults, ...filterCountDefaults, email_id: 0,
      }, preload: ({ email_id }) => {
        if (email_id) {
  ...
  registerFilter('page_visited', 'activity', __('Page Visited', 'groundhogg'), {
    view ({ link, ...rest }) {
      ...
      ...standardActivityDateDefaults, ...filterCountDefaults, link: '',
    },
  })
  ...
  registerFilter('funnel_history', 'funnels',
    __('Funnel History', 'groundhogg'), {
      view ({
        ...
        return `
      ${ select({
          id: 'filter-funnel', name: 'funnel_id',
        }, FunnelsStore.getItems().
          map(f => ( { value: f.ID, text: f.data.title } )), funnel_id) }
      ${ select({
          id: 'filter-step', name: 'step_id',
        }, funnel_id ? FunnelsStore.get(funnel_id).steps.map(s => ( {
          value: s.ID, text: s.data.step_title,
        } )) : [], step_id) }
      ${ standardActivityDateOptions({ date_range, before, after }) }`
        ...
        funnelPicker('#filter-funnel', false, (items) => {
          ...
          updateFilter({
            funnel_id: parseInt($(target).val()), step_id: 0,
          }, true)
        })
  ...
  registerFilter('broadcast_received', 'broadcast',
    __('Received Broadcast', 'groundhogg'), {
      view ({ broadcast_id, status = 'complete' }) {
        ...
        return select({
          id: 'filter-broadcast', name: 'broadcast_id',
        ...
        broadcast_id: 0, status: 'complete',
