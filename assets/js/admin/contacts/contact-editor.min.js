				...
				<button id="contact-more" class="gh-button secondary text icon">${icons.verticalDots}</button>`;$("#contact-more-actions").html(actions);ContactActions.forEach(({id,tooltip:__tp,onClick})=>{$(`#action-${id}`).on("click",e=>onClick(e,getContact()));tooltip(`#action-${id}`,{content:__tp})});$("#contact-more").on("click",e=>moreMenu(e.target,{items:[{key:"merge",cap:"delete_contacts",text:__("Merge")},{key:"delete",cap:"delete_contacts",text:`<span class="gh-text danger">${__("Delete")}</span>`}].filter(i=>userHasCap(i.cap)),onSelect:k=>{switch(k){case"merge":selectContactModal({exclude:[contact.ID],onSelect:_contact=>{confirmationModal({confirmText:__("Merge"),width:500,alert:`<p>
                  ...
                  <p>
                      <a href="https://help.groundhogg.io/article/540-merging-contacts"
                         target="_blank">${__("What happens when contacts are merged?","groundhogg")}</a></p>`,onConfirm:()=>{loadingModal();post(`${ContactsStore.route}/${contact.ID}/merge`,[_contact.ID]).then(()=>{location.reload()})}})}});break;case"delete":dangerConfirmationModal({confirmText:__("Delete"),alert:`<p>${sprintf(__("Are you sure you want to delete %s?","groundhogg"),bold(getContact().data.full_name))}</p>`,onConfirm:()=>{ContactsStore.delete(contact.ID).then(()=>{dialog({message:sprintf(__("%s was deleted!","groundhogg"),contact.data.full_name)});window.open(adminPageURL("gh_contacts"),"_self")})}})}}}))};const ActivityTimeline={hiddenActivity:["thread_reply"],addType(type,opts){this.types[type]={icon:"",render:()=>"",preload:()=>{},...opts}},types:{wp_fusion:{icon:icons.wp_fusion,iconFramed:false,render:({meta})=>{const{event_name,event_value}=meta;return`${event_name}: <code>${event_value}</code>`},preload:()=>{}},wp_login:{icon:icons.login,render:()=>{return __("Logged in","groundhogg")},preload:()=>{}},wp_logout:{icon:icons.logout,render:()=>{return __("Logged out","groundhogg")},preload:()=>{}},composed_email_sent:{icon:icons.open_email,render:({meta,i18n,ID})=>{return sprintf(__("%s sent an email with subject %s","groundhogg"),bold(i18n.sent_by),`<a href="#" class="view-composed-email-log-item" data-activity-id="${ID}">${bold(meta.subject)}</a>`)},preload:()=>{}},email_opened:{icon:icons.open_email,render:({data})=>{return sprintf(__("Opened %s","groundhogg"),bold(EmailsStore.get(data.email_id).data.title))}},email_link_click:{icon:icons.link_click,render:({data})=>{const maybeTruncateLink=link=>{return link.length>50?`${link.substring(0,47)}...`:link};return sprintf(__("Clicked %s in %s","groundhogg"),el("a",{target:"_blank",href:data.referer},bold(maybeTruncateLink(data.referer))),bold(EmailsStore.get(data.email_id).data.title))}},imported:{icon:'<span class="dashicons dashicons-upload"></span>',render:({i18n})=>{return sprintf(__("Imported by %s from %s","groundhogg"),bold(i18n.by),bold(i18n.file))}},funnel_conversion:{icon:'<span class="dashicons dashicons-flag"></span>',render:({data})=>{let funnelTitle=bold(FunnelsStore.get(data.funnel_id).data.title);let link=el("a",{href:adminPageURL("gh_funnels",{action:"edit",funnel:data.funnel_id},data.step_id)},funnelTitle);return sprintf(__("Converted in %s","groundhogg"),link)}},fallback:{icon:icons.heartbeat,render:({data,meta})=>{let html=[data.activity_type];if(data.value){html.push(`: <code>${data.value}</code>`)}if(Object.keys(meta).length){html.push(textarea({className:"full-width code custom-activity-meta space-above-20",value:JSON.stringify(meta,null,2),readonly:true}))}return html.join("")}}},renderActivity(activity){if(activity.type==="submission"){let funnel=FunnelsStore.get(activity.form.data.funnel_id);return`
