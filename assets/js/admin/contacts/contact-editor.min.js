(($,editor)=>{const{contact,meta_exclusions}=editor;const{gh_contact_custom_properties}=Groundhogg.filters;const{tooltip,regexp,inputRepeaterWidget,inputRepeater,el,searchOptionsWidget,input,select,isNumeric,textarea,icons,bold,loadingModal,modal,uuid,dangerConfirmationModal,confirmationModal,adminPageURL,moreMenu,setFrameContent,loadingDots,spinner,dialog}=Groundhogg.element;const{currentUser,filters,propertiesEditor,isWPFusionActive}=Groundhogg;const{userHasCap}=Groundhogg.user;const{events:EventsStore,event_queue:EventQueue,tags:TagsStore,contacts:ContactsStore,emails:EmailsStore,activity:ActivityStore,funnels:FunnelsStore,broadcasts:BroadcastsStore,page_visits:PageVisitsStore,submissions:SubmissionsStore}=Groundhogg.stores;const{emailPicker}=Groundhogg.pickers;const{post,delete:_delete,get,patch,routes,ajax}=Groundhogg.api;const{selectContactModal,betterTagPicker,internalForm}=Groundhogg.components;const{sprintf,__,_x,_n}=wp.i18n;ContactsStore.itemsFetched([contact]);let files=[];const maybeCall=(maybeFunc,...args)=>{if(typeof maybeFunc==="string"){return maybeFunc}if(typeof maybeFunc==="function"){return maybeFunc(...args)}return maybeFunc};const activityUpdated=()=>{window.dispatchEvent(new Event("activityupdated"))};const getContact=()=>{return ContactsStore.get(contact.ID)};const sanitizeKey=label=>{return label.toLowerCase().replace(/[^a-z0-9]/g,"_")};const sendEmail=()=>{let contact=getContact();let email={to:[contact.data.email],from_email:currentUser.data.user_email,from_name:currentUser.data.display_name};if(contact.data.owner_id&&currentUser.ID!=contact.data.owner_id){email.cc=[filters.owners.find(u=>u.ID==contact.data.owner_id).data.user_email]}Groundhogg.components.emailModal(email)};const ContactActions=[{id:"send-email",icon:icons.email,tooltip:__("Send Email","groundhogg"),show:contact=>true,onClick:e=>{moreMenu(e.currentTarget,{items:[{key:"compose",text:__("Compose","groundhogg")},{key:"template",text:__("Use template","groundhogg")}],onSelect:k=>{switch(k){case"compose":sendEmail();break;case"template":let emailId;const preview=()=>{return`
                      <div class="gh-row">
                        ...
                        <div class="gh-rows-and-columns">
                            <div class="gh-row">
                                <div class="gh-col">
                                    ${select({name:"email",id:`select-email`,options:[{text:"",value:""},...EmailsStore.getItems().map(e=>({text:e.data.title,value:e.ID}))],selected:emailId})}
                                </div>
                            </div>
                            ${preview()}
                            <div class="gh-row">
                    ...
                    `,onOpen:({close})=>{let $btn=$("#send-email-template");emailPicker(`#select-email`,false,items=>{EmailsStore.itemsFetched(items)},{status:"ready"},{placeholder:__("Select an email to send...","groundhogg")}).on("change",({target})=>{emailId=parseInt(target.value);showFrame();$btn.prop("disabled",false)});$btn.on("click",e=>{$btn.prop("disabled",true);let{stop}=loadingDots(e.currentTarget);post(`${EmailsStore.route}/${emailId}/send`,{to:getContact().ID}).then(r=>{dialog({message:__("Email sent!")});stop();close()}).catch(e=>{stop();$btn.prop("disabled",false);dialog({type:"error",message:e.message})})})}});break}}})}},{id:"call-primary",icon:icons.phone,tooltip:__("Call primary phone","groundhogg"),show:contact=>contact.meta.primary_phone,onClick:e=>{window.open(`tel:${contact.meta.primary_phone}`)}},{id:"call-mobile",icon:icons.smartphone,tooltip:__("Call mobile","groundhogg"),show:contact=>contact.meta.mobile_phone,onClick:e=>{window.open(`tel:${contact.meta.mobile_phone}`)}},{id:"add-to-funnel",icon:icons.funnel,tooltip:__("Add to a funnel","groundhogg"),show:contact=>true,onClick:e=>{modal({content:``,onOpen:({close,setContent})=>{let funnel=FunnelsStore.hasItems()?FunnelsStore.getItems()[0]:null;let step=funnel?funnel.steps[0]:null;const addToFunnelUi=()=>{return`
                  <div>
                  ...
                  </div>`};setContent(addToFunnelUi());const onMount=()=>{$("#select-funnel").ghPicker({endpoint:FunnelsStore.route,getParams:q=>{return{...q,status:"active"}},data:FunnelsStore.getItems().map(f=>({id:f.ID,text:f.data.title,selected:funnel&&f.ID===funnel.ID})),getResults:({items})=>{FunnelsStore.itemsFetched(items);return items.map(f=>({id:f.ID,text:f.data.title}))},placeholder:__("Select a funnel...","groundhogg")}).on("change",({target})=>{funnel=FunnelsStore.get(parseInt($(target).val()));step=funnel.steps.find(s=>s.data.step_order==1);setContent(addToFunnelUi());onMount()});if(funnel){$("#select-step").select2({placeholder:__("Select a step...","groundhogg"),data:funnel.steps.sort((a,b)=>a.data.step_order-b.data.step_order).map(s=>({id:s.ID,text:`${s.data.step_title} (${Groundhogg.rawStepTypes[s.data.step_type].name})`,selected:s.ID==step.ID}))}).on("change",({target})=>{step=funnel.steps.find(s=>s.ID===parseInt($(target).val()));setContent(addToFunnelUi());onMount()})}if(funnel&&step){$("#confirm-add-to-funnel").on("click",e=>{FunnelsStore.addContacts({funnel_id:funnel.ID,step_id:step.ID,contact_id:getContact().ID}).then(()=>{dialog({message:sprintf(__("%s added to funnel!","groundhogg"),getContact().data.full_name)});close()})})}};onMount()}})}},{id:"internal-form",icon:icons.form,tooltip:__("Submit an internal form","groundhogg"),show:contact=>true,onClick:e=>{internalForm({contact:getContact(),onSubmit:()=>{activityUpdated()}})}}];const contactMoreActions=()=>{let actions=`
        ${ContactActions.filter(action=>action.show(getContact())).map(({icon,id})=>`<button id="action-${id}" class="gh-button secondary text icon">${icon}</button>`).join("")}
				<button id="contact-more" class="gh-button secondary text icon">${icons.verticalDots}</button>`;$("#contact-more-actions").html(actions);ContactActions.forEach(({id,tooltip:__tp,onClick})=>{$(`#action-${id}`).on("click",e=>onClick(e,getContact()));tooltip(`#action-${id}`,{content:__tp})});$("#contact-more").on("click",e=>moreMenu(e.target,{items:[{key:"merge",cap:"delete_contacts",text:__("Merge")},{key:"delete",cap:"delete_contacts",text:`<span class="gh-text danger">${__("Delete")}</span>`}].filter(i=>userHasCap(i.cap)),onSelect:k=>{switch(k){case"merge":selectContactModal({exclude:[contact.ID],onSelect:_contact=>{confirmationModal({confirmText:__("Merge"),width:500,alert:`<p>
                  ...
                  <p>
                      <a href="https://help.groundhogg.io/article/540-merging-contacts"
                         target="_blank">${__("What happens when contacts are merged?","groundhogg")}</a></p>`,onConfirm:()=>{loadingModal();post(`${ContactsStore.route}/${contact.ID}/merge`,[_contact.ID]).then(()=>{location.reload()})}})}});break;case"delete":dangerConfirmationModal({confirmText:__("Delete"),alert:`<p>${sprintf(__("Are you sure you want to delete %s?","groundhogg"),bold(getContact().data.full_name))}</p>`,onConfirm:()=>{ContactsStore.delete(contact.ID).then(()=>{dialog({message:sprintf(__("%s was deleted!","groundhogg"),contact.data.full_name)});window.open(adminPageURL("gh_contacts"),"_self")})}})}}}))};const ActivityTimeline={hiddenActivity:["thread_reply"],addType(type,opts){this.types[type]={icon:"",render:()=>"",preload:()=>{},...opts}},types:{wp_fusion:{icon:icons.wp_fusion,iconFramed:false,render:({data,meta})=>{const{event_name,event_value}=meta;return`${event_name}: <code>${event_value}</code>`},preload:()=>{}},wp_login:{icon:icons.login,render:({email})=>{return __("Logged in","groundhogg")},preload:()=>{}},wp_logout:{icon:icons.logout,render:({email})=>{return __("Logged out","groundhogg")},preload:()=>{}},email_opened:{icon:icons.open_email,render:({email})=>{return sprintf(__("Opened %s","groundhogg"),bold(email.data.title))},preload:({email})=>{EmailsStore.itemsFetched([email])}},composed_email_sent:{icon:icons.open_email,render:({meta,sent_by,ID})=>{return sprintf(__("%s sent an email with subject %s","groundhogg"),bold(sent_by),`<a href="#" class="view-composed-email-log-item" data-activity-id="${ID}">${bold(meta.subject)}</a>`)},preload:()=>{}},email_link_click:{icon:icons.link_click,render:({email,data})=>{const maybeTruncateLink=link=>{return link.length>50?`${link.substring(0,47)}...`:link};return sprintf(__("Clicked %s in %s","groundhogg"),el("a",{target:"_blank",href:data.referer},bold(maybeTruncateLink(data.referer))),bold(email.data.title))},preload:({email})=>{EmailsStore.itemsFetched([email])}},fallback:{icon:icons.heartbeat,render:({data,meta})=>{let html=[data.activity_type];if(data.value){html.push(`: <code>${data.value}</code>`)}if(Object.keys(meta).length){html.push(textarea({className:"full-width code custom-activity-meta space-above-20",value:JSON.stringify(meta,null,2),readonly:true}))}return html.join("")}}},renderActivity(activity){if(activity.type==="submission"){let funnel=FunnelsStore.get(activity.form.data.funnel_id);return`
          ...
          <ul id="activity-timeline">
              ...
              </div>`},onMount:()=>{Groundhogg.taskEditor("#tasks-here",{object_id:contact.ID,object_type:"contact",title:""})}},{id:"files",name:__("Files"),render:()=>{return`
              <div class="gh-panel top-left-square">
          ...
          </div>`};const maybeShowTagChangeActions=()=>{if(removeTags.length||addTags.length){$(".tag-change-actions").addClass("align-right-space-between")}else{$(".tag-change-actions").removeClass("align-right-space-between")}};const mount=()=>{$("#tags-here").html(template());onMount()};const onMount=()=>{betterTagPicker("#gh-better-tag-picker",{selected:getContact().tags,onChange:({addTags:_addTags,removeTags:_removeTags})=>{removeTags=_removeTags;addTags=_addTags;maybeShowTagChangeActions()}});$("#save-tag-changes").on("click",()=>{ContactsStore.patch(getContact().ID,{remove_tags:removeTags,add_tags:addTags}).then(()=>{dialog({message:__("Changes saved!")});addTags=[];removeTags=[];mount()})});$("#cancel-tag-changes").on("click",()=>{addTags=[];removeTags=[];mount()})};TagsStore.itemsFetched(getContact().tags);mount()};$.extend(editor,{init(){handleFormSubmit();contactMoreActions();manageTags();managePrimaryTabs();otherContactStuff();$("#send-email").on("click",e=>{e.preventDefault();sendEmail()});$("#primary-contact-stuff .toggle-indicator").on("click",e=>{$(e.target).closest(".gh-panel").toggleClass("closed")});if(window.location.href.match(/send_email=true/)){sendEmail()}}});const{email_log:LogsStore}=Groundhogg.stores;const{EmailLogModal,EmailPreviewModal}=Groundhogg.components;$(document).on("click","a.view-event-email-log-item",async e=>{e.preventDefault();let eventId=parseInt($(e.currentTarget).data("event-id"));let event=EventsStore.get(eventId);let{close}=loadingModal();try{let logItems=await LogsStore.fetchItems({queued_event_id:event.data.queued_id,limit:1});EmailLogModal(logItems[0]);close()}catch(err){close();if(event.data.email_id){try{await EmailPreviewModal(event.data.email_id,{});return}catch(err2){}}dialog({message:err.message,type:"error",ttl:5e3})}});$(document).on("click","a.view-composed-email-log-item",async e=>{e.preventDefault();let activityId=parseInt($(e.currentTarget).data("activity-id"));let activity=ActivityStore.get(activityId);let{close}=loadingModal();try{let logItems=await LogsStore.fetchItems({subject:activity.meta.subject,recipients:["RLIKE",getContact().data.email],date_sent:moment.unix(activity.data.timestamp).utc().format("YYYY-MM-DD hh:mm:ss"),limit:1});EmailLogModal(logItems[0])}catch(err){dialog({message:err.message,type:"error",ttl:5e3})}close()});$(function(){editor.init()});Groundhogg.ActivityTimeline=ActivityTimeline;Groundhogg.ContactActions=ContactActions})(jQuery,ContactEditor);