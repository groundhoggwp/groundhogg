        ...
        ${ContactActions.filter(action=>action.show(getContact())).map(({icon,id})=>`<button id="action-${id}" class="gh-button secondary text icon">${icon}</button>`).join("")}
              ...
              <div class="inside" style="padding: 0">
                  <table class="wp-list-table widefat striped" style="border: none">
                      <tbody>
                      ${Object.keys(meta).map(key=>{return`<tr><td><code>${key}</code></td><td>${meta[key]}</td></tr>`}).join("")}
                      </tbody>
                  </table>
              </div>
          </div>`)}return html.join("")}}},renderActivity(activity){if(activity.type==="submission"&&activity.data.type==="form"){let funnel=FunnelsStore.get(activity.form.data.funnel_id);return`
