(function($){const{urlEncodeFilters}=Groundhogg.filters;const{createFilters}=Groundhogg.filters.functions;const{searchOptionsWidget,regexp,specialChars,modal,input,loadingDots,copyObject,objectEquals,toggle,moreMenu,dangerConfirmationModal,confirmationModal,dialog,bold,tooltip,adminPageURL}=Groundhogg.element;const{quickEditContactModal,addContactModal}=Groundhogg.components;const{ajax}=Groundhogg.api;const{searches:SearchesStore,contacts:ContactsStore}=Groundhogg.stores;const{userHasCap}=Groundhogg.user;const{formatNumber}=Groundhogg.formatting;const{sprintf,__,_x,_n}=wp.i18n;SearchesStore.itemsFetched(ContactSearch.searches);const loadFilters=query=>{window.location.href=ContactSearch.url+"&"+query};const loadSearch=search=>{window.location.href=ContactSearch.url+"&saved_search="+search};const{Div,Button}=MakeEl;let abortHandler;const SearchApp={filtersEnabled:false,excludeEnabled:false,savedSearchEnabled:false,query:{filters:[],exclude_filters:[]},filtersApp:null,excludeFiltersApp:null,searchesApp:null,currentSearch:null,getContacts(){if(abortHandler){abortHandler.abort()}abortHandler=new AbortController;const{signal}=abortHandler;ContactsStore.count({filters:urlEncodeFilters(this.query.filters),exclude_filters:urlEncodeFilters(this.excludeEnabled?this.query.exclude_filters:[])},{signal:signal}).then(total=>{$("#search-contacts").html(sprintf(_n("Show %s contact","Show %s contacts",total,"groundhogg"),formatNumber(total)))})},init(){const onUpdate=()=>{this.getContacts();if(this.currentSearch){if(objectEquals(this.query,this.currentSearch.query)){$("#update-search").prop("disabled",true)}else{$("#update-search").prop("disabled",false)}}};const handleUpdateExcludeFilters=filters=>{this.query.exclude_filters=filters;onUpdate()};const handleUpdateFilters=filters=>{this.query.filters=filters;onUpdate()};this.query=ContactSearch.filter_query||[];if(this.query.filters.length||this.query.exclude_filters.length){this.filtersEnabled=true;if(this.query.exclude_filters.length){this.excludeEnabled=true}$(".contact-quick-search").hide()}if(ContactSearch.currentSearch){this.currentSearch=copyObject(ContactSearch.currentSearch);if(this.currentSearch.query){this.query=copyObject(this.currentSearch.query,{});if("exclude_filters"in this.query&&this.query.exclude_filters.length){this.excludeEnabled=true}}}this.filtersApp=createFilters("#search-filters",this.query.filters,handleUpdateFilters);this.excludeFiltersApp=createFilters("#exclude-filters",this.query.exclude_filters,handleUpdateExcludeFilters);this.mount()},initSavedSearches(){searchOptionsWidget({selector:"#searches-picker",options:SearchesStore.getItems(),filterOption:(option,search)=>{return option.name.match(regexp(search))},renderOption:option=>option.name,onClose:()=>{this.savedSearchEnabled=false;this.mount()},onSelect:option=>{this.loadingSearch=true;loadSearch(option.id)},noOptions:__("No matching searches...","groundhogg")}).mount()},render(){if(this.filtersEnabled){return`
            ...
            <div class="search-filters-wrap">
                ${this.excludeEnabled?`<div class="include-filters-wrap"><div class="include-block">${__("Include","groundhogg")}</div>`:""}
                <div id="search-filters"></div>
                ${this.excludeEnabled?`</div><div class="exclude-filters-wrap"><div class="exclude-block">${__("Exclude","groundhogg")}</div><div id="exclude-filters"></div></div>`:""}
                <div id="below-filters" class="space-between">
                    ...
                    <div class="align-right-space-between">
                        ...
                        ${!this.currentSearch?`<button id="save-search" class="gh-button secondary">${__("Save this search","groundhogg")}</button>`:`<button id="update-search" class="gh-button secondary" ${objectEquals(this.query.filters,this.currentSearch.query.filters)&&(!this.excludeEnabled||objectEquals(this.query.exclude_filters,this.currentSearch.query.exclude_filters))?"disabled":""}>${sprintf(__('Update "%s"',"groundhogg"),this.currentSearch.name)}</button><a id="delete-search" class="gh-text danger delete-search">${__("Delete","groundhogg")}</a>`}
                    </div>
        ...
        `}return`
          ...
          ${this.savedSearchEnabled?`<div id="searches-picker"></div>`:ContactSearch.searches.length?`<button id="load-saved-search" class="gh-button secondary small"><span class="dashicons dashicons-search" style="line-height: 1"></span> <span class="text">${this.loadingSearch?__("Loading search","groundhogg"):__("Load saved search","groundhogg")}</span></button>`:""}`},mount(){$("#search-panel .filters").html(this.render());this.addListeners()},addListeners(){var self=this;const remount=()=>{this.mount()};const enableFilters=()=>{this.filtersEnabled=!this.filtersEnabled;if(this.filtersEnabled){$(".contact-quick-search").hide()}else{$(".contact-quick-search").show()}remount()};const enableSavedSearch=()=>{this.savedSearchEnabled=!this.savedSearchEnabled;remount()};if(this.filtersEnabled){this.filtersApp.init();tooltip(".enable-filters",{content:__("Turn off filters","groundhogg"),position:"top"});if(this.excludeEnabled){this.excludeFiltersApp.init()}}if(this.savedSearchEnabled){this.initSavedSearches()}$(".enable-filters").on("click",function(){enableFilters()});$("#load-saved-search").on("click",function(){enableSavedSearch()});if(this.loadingSearch){$("#load-saved-search").prop("disabled",true);loadingDots("#load-saved-search span.text")}$("#enable-exclude").on("change",e=>{this.excludeEnabled=e.target.checked;remount();this.getContacts()});$("#search-contacts").on("click",e=>{$(e.target).html("Searching").prop("disabled",true);loadingDots("#search-contacts");if(this.currentSearch&&objectEquals(this.query.filters,this.currentSearch.query.filters)&&objectEquals(this.currentSearch.query.exclude_filters,this.excludeEnabled?this.query.exclude_filters:[])){loadSearch(this.currentSearch.id)}else{let query=`filters=${urlEncodeFilters(this.query.filters)}`;if(this.excludeEnabled){query+=`&exclude_filters=${urlEncodeFilters(this.query.exclude_filters)}`}loadFilters(query)}});$("#delete-search").on("click",e=>{e.preventDefault();dangerConfirmationModal({alert:`<p>${__("Are you sure you want to delete this search","groundhogg")}</p>`,onConfirm:()=>{SearchesStore.delete(this.currentSearch.id).then(()=>{this.currentSearch=null;$("#current-search").remove();this.mount()})}})});$("#update-search").on("click",e=>{const $button=$(e.target);$button.prop("disabled",true);$button.html(__("Updating","groundhogg"));$button.addClass("loading-dots");SearchesStore.patch(this.currentSearch.id,{query:{filters:this.query.filters,exclude_filters:this.excludeEnabled?this.query.exclude_filters:[]}}).then(search=>{$button.removeClass("loading-dots");this.currentSearch=search;$button.html(__("Updated!","groundhogg"));dialog({message:__("Search updated!","groundhogg")});setTimeout(()=>{this.mount()},1e3)})});if(!userHasCap("manage_options")){$("#update-search").remove();$("#delete-search").remove()}$("span#search-name").on("click",e=>{if(!userHasCap("manage_options")){return}const $span=$(e.target);$span.html(input({name:"search_name",id:"saved-search-name-edit",value:this.currentSearch.name}));$("#saved-search-name-edit").focus().on("change blur keydown",e=>{if(e.type==="keydown"&&e.key!=="Enter"){return}const newName=e.target.value;$span.html(specialChars(newName));if(newName!==this.currentSearch.name){SearchesStore.patch(this.currentSearch.id,{name:newName}).then(s=>this.currentSearch=s).then(()=>$span.html(specialChars(this.currentSearch.name)))}})});$("#save-search").on("click",()=>{const{$modal,close}=modal({content:`
