(function($){const{input:input,select:select,regexp:regexp,specialChars:specialChars,clickInsideElement:clickInsideElement,orList:orList,andList:andList,searchOptionsWidget:searchOptionsWidget}=Groundhogg.element;const{tagPicker:tagPicker,emailPicker:emailPicker,linkPicker:linkPicker}=Groundhogg.pickers;const{emails:emails}=Groundhogg.stores;const Filters={view:function(){},edit:function(){},types:{},groups:{}};const renderFilterView=(filter,filterGroupIndex,filterIndex)=>{return`
		...
		<div class="filter filter-edit-wrap" data-key="${filterIndex}" data-group="${filterGroupIndex}">
		  ...
		  ${select({id:"filter-value",name:"value",multiple:true},values,value)} `},onMount:function(filter,updateFilter){$("#filter-value").select2();$("#filter-value, #filter-compare").on("change",function(e){const $el=$(this);updateFilter({[$el.prop("name")]:$el.val()})})},defaults:{compare:"equals",value:[]}},"Owner");registerFilter("tags","contact",{view:function({tags:tags,compare:compare,compare2:compare2}){const tagNames=tags.map(id=>`<b>${id}</b>`);switch(compare2){case"any":return`<b>Tags</b> ${compare} ${orList(tagNames)}`;default:case"all":return`<b>Tags</b> ${compare} ${andList(tagNames)}`}},edit:function({tags:tags,compare:compare,compare2:compare2},filterGroupIndex,filterIndex){return`
