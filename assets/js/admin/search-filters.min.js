(function($){const{input:input,select:select,regexp:regexp,specialChars:specialChars,clickInsideElement:clickInsideElement}=Groundhogg.element;const Filters={view:function(){},edit:function(){},types:{},groups:{}};const renderFilterView=(filter,filterGroupIndex,filterIndex)=>{return`
		...
		<div class="filter filter-edit-wrap" data-key="${filterIndex}" data-group="${filterGroupIndex}">
		  ...
		  </div>`},mount:function(){$(el).html(this.render());this.eventHandlers()},eventHandlers:function(){var self=this;const reMount=()=>{self.mount()};const getFilterSettings=(group,key)=>{return self.filters[group][key]};const setActiveFilter=(group,filter)=>{self.currentFilter=filter;self.currentGroup=group;reMount();if(self.currentGroup!==false&&self.currentFilter!==false){const $filterEdit=$(".filter-edit");$filterEdit.parent().width($filterEdit.width());const filterSettings=getFilterSettings(self.currentGroup,self.currentFilter);self.tempFilterSettings=filterSettings;const{type:type}=filterSettings;Filters.types[type].onMount(filterSettings,updateFilter)}};const addFilter=(opts,group)=>{group=group>=0?group:0;self.filters[group].push({...opts});onChange(self.filters);self.isAddingFilter=false;self.currentGroup=group;self.currentFilter=self.filters[group].length-1;reMount()};const updateFilter=opts=>{console.log(opts);self.tempFilterSettings={...self.tempFilterSettings,...opts}};const commitFilter=(group,key)=>{group=group>=0?group:self.currentGroup;key=key>=0?key:self.currentFilter;self.filters[group][key]={...self.filters[group][key],...self.tempFilterSettings};this.tempFilterSettings={};onChange(self.filters);setActiveFilter(false,false)};const deleteFilter=(group,key)=>{group=group>=0?group:self.currentGroup;key=key>=0?key:self.currentFilter;console.log({group:group,key:key});self.filters[group].splice(key,1);if(self.filters[group].length===0){self.filters.splice(group,1)}onChange(self.filters);reMount()};if(this.isAddingFilter){const adding=FilterSearch(addFilter);adding.mount()}$(`${el} #search-filters-editor`).on("click",function(e){const clickedOnAddFilter=clickInsideElement(e,"button.add-filter");const clickedOnAddFilterSearch=clickInsideElement(e,"div.add-filter");const clickedOnFilterView=clickInsideElement(e,".filter.filter-view");const clickedOnFilterEdit=clickInsideElement(e,".filter-edit");if(clickedOnAddFilter){self.isAddingFilter=true;reMount()}else if(clickedOnFilterView){const clickedOnFilterDelete=clickInsideElement(e,".delete-filter");const filter=parseInt(clickedOnFilterView.dataset.key);const group=parseInt(clickedOnFilterView.dataset.group);if(clickedOnFilterDelete){deleteFilter(group,filter)}else{setActiveFilter(group,filter)}}else if(clickedOnFilterEdit){const clickedOnEditClose=clickInsideElement(e,".close-edit");const clickedOnDeleteFilter=clickInsideElement(e,".delete");const clickedOnCommitChanges=clickInsideElement(e,".commit");if(clickedOnEditClose){setActiveFilter(false,false)}else if(clickedOnCommitChanges){commitFilter()}else if(clickedOnDeleteFilter){deleteFilter()}else{}}else if(clickedOnAddFilterSearch){const clickedOnClose=clickInsideElement(e,".close");self.isAddingFilter=false;reMount()}else{self.currentFilter=false;self.currentGroup=false;reMount()}})}});Groundhogg.filters.functions={createFilters:createFilters};$(function(){const defaultFilters=[[{type:"first_name",compare:"equals",value:"adrian"}]];const app=createFilters("#search-filters",defaultFilters,function(filters){console.log(filters)});app.mount();console.log("mounted")});const BasicTextFilter=name=>({name:name,view:function({compare:compare,value:value},filterGroupIndex,filterIndex){switch(compare){case"_":case"!_":return`<b>${name}</b> ${Comparisons[compare].toLowerCase()}`;default:return`<b>${name}</b> ${Comparisons[compare].toLowerCase()} <b>${specialChars(value)}</b>`}},edit:function({compare:compare,value:value},filterGroupIndex,filterIndex){return`${select({id:"filter-compare",name:"compare",dataGroup:filterIndex,dataKey:filterIndex},Comparisons,compare)} ${input({id:"filter-value",name:"value",dataGroup:filterIndex,dataKey:filterIndex,value:value})}`},onMount:function(filter,updateFilter){console.log(filter);$("#filter-compare, #filter-value").on("change",function(e){console.log(e);const $el=$(this);updateFilter({[$el.prop("name")]:$el.val()})})},defaults:{compare:"equals",value:""}});registerFilterGroup("contact","Contact");registerFilter("first_name","contact",{...BasicTextFilter("First Name")});registerFilter("last_name","contact",{...BasicTextFilter("Last Name")});registerFilter("email","contact",{...BasicTextFilter("Email Address")});registerFilter("date_created","contact",{view:function({compare:compare,value:value,value2:value2},filterGroupIndex,filterIndex){switch(compare){case"before":return`is before <b>${value}</b>`;case"after":return`is after <b>${value}</b>`;case"between":return`is between <b>${value}</b> and <b>${value2}</b>`}},edit:function({compare:compare,value:value,value2:value2},filterGroupIndex,filterIndex){return`${select({id:"filter-compare",name:"compare"},{before:"Before",after:"After",between:"Between"},compare)} ${input({type:"date",value:value,id:"filter-value",name:"value"})} ${input({type:"date",value:value2,id:"filter-value2",name:"value2"})}`},onMount:function(filter,updateFilter){$("#filter-compare, #filter-value, #filter-value2").on("change",function(e){const $el=$(this);updateFilter({[$el.prop("name")]:$el.val()})})},defaults:{compare:"before",value:"",value2:""}},"Date Created");const{optin_status:optin_status,owners:owners}=Groundhogg.filters;console.log({optin_status:optin_status,owners:owners})})(jQuery);