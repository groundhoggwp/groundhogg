(function($){const{input,select,regexp,specialChars,clickInsideElement,orList,andList,searchOptionsWidget,loadingDots,tooltip,bold}=Groundhogg.element;const{broadcastPicker,funnelPicker,tagPicker,emailPicker,linkPicker,metaValuePicker,metaPicker,userMetaPicker}=Groundhogg.pickers;const{broadcasts:BroadcastsStore,emails:EmailsStore,tags:TagsStore,funnels:FunnelsStore,searches:SearchesStore}=Groundhogg.stores;const{sprintf,__,_x,_n}=wp.i18n;const Filters={has(type){return typeof this.types[type]!=="undefined"},view(){},edit(){},types:{},groups:{}};const{formatNumber,formatTime,formatDate,formatDateTime}=Groundhogg.formatting;const renderFilterBroken=(filter,filterGroupIndex,filterIndex)=>{if(!Filters.has(filter.type)){return""}return`
      ...
      ${standardActivityDateOptions({date_range:date_range,before:before,after:after})}`},onMount(filter,updateFilter){funnelPicker("#filter-funnel",false,items=>{FunnelsStore.itemsFetched(items)},{},{placeholder:__("Select a funnel","groundhogg")}).on("select2:select",({target})=>{updateFilter({funnel_id:parseInt($(target).val()),step_id:0},true)});$("#filter-step").select2({placeholder:__("Select a step or leave empty for any step","groundhogg")}).on("select2:select",({target})=>{updateFilter({step_id:parseInt($(target).val())})});standardActivityDateFilterOnMount(filter,updateFilter)},defaults:{funnel_id:0,step_id:0,status:"complete",...standardActivityDateDefaults},preload:({funnel_id})=>{if(funnel_id){return FunnelsStore.fetchItem(funnel_id)}}});registerFilterGroup("broadcast",_x("Broadcast","noun meaning email blast","groundhogg"));registerFilter("broadcast_received","broadcast",__("Received Broadcast","groundhogg"),{view({broadcast_id,status:status="complete"}){if(!broadcast_id){return __("Received any broadcast","groundhogg")}const broadcast=BroadcastsStore.get(broadcast_id);return status==="complete"?sprintf(broadcast?__("Received %1$s on %2$s","groundhogg"):__("Will receive a broadcast","groundhogg"),`<b>${broadcast.object.data.title}</b>`,`<b>${formatDateTime(broadcast.data.send_time*1e3)}</b>`):sprintf(broadcast?__("Will receive %1$s on %2$s","groundhogg"):__("Received a broadcast","groundhogg"),`<b>${broadcast.object.data.title}</b>`,`<b>${formatDateTime(broadcast.data.send_time*1e3)}</b>`)},edit({broadcast_id}){return select({id:"filter-broadcast",name:"broadcast_id"},BroadcastsStore.getItems().map(b=>({value:b.ID,text:`${b.object.data.title} (${b.date_sent_pretty})`})),broadcast_id)},onMount(filter,updateFilter){broadcastPicker("#filter-broadcast",false,items=>{BroadcastsStore.itemsFetched(items)},{},{placeholder:__("Select a broadcast","groundhogg")}).on("select2:select",({target})=>{updateFilter({broadcast_id:parseInt($(target).val())})})},defaults:{broadcast_id:0,status:"complete"},preload:({broadcast_id})=>{if(broadcast_id){return BroadcastsStore.fetchItem(broadcast_id)}}});registerFilter("broadcast_opened","broadcast",__("Opened Broadcast","groundhogg"),{view({broadcast_id}){if(!broadcast_id){return __("Opened any broadcast","groundhogg")}const broadcast=BroadcastsStore.get(broadcast_id);return sprintf(broadcast?__("Opened %1$s after %2$s","groundhogg"):__("Will receive a broadcast","groundhogg"),`<b>${broadcast.object.data.title}</b>`,`<b>${formatDateTime(broadcast.data.send_time*1e3)}</b>`)},edit({broadcast_id}){return select({id:"filter-broadcast",name:"broadcast_id"},BroadcastsStore.getItems().map(b=>({value:b.ID,text:`${b.object.data.title} (${b.date_sent_pretty})`})),broadcast_id)},onMount(filter,updateFilter){broadcastPicker("#filter-broadcast",false,items=>{BroadcastsStore.itemsFetched(items)},{},{placeholder:__("Select a broadcast","groundhogg")}).on("select2:select",({target})=>{updateFilter({broadcast_id:parseInt($(target).val())})})},defaults:{broadcast_id:0},preload:({broadcast_id})=>{if(broadcast_id){return BroadcastsStore.fetchItem(broadcast_id)}}});registerFilter("broadcast_link_clicked","broadcast",__("Broadcast Link Clicked","groundhogg"),{view({broadcast_id,link}){if(!broadcast_id&&!link){return __("Clicked any link in any broadcast","groundhogg")}if(!broadcast_id&&link){return sprintf(__("Clicked %s in any broadcast","groundhogg"),bold(link))}const broadcast=BroadcastsStore.get(broadcast_id);if(broadcast_id&&!link){return sprintf(__("Clicked any link in %1$s after %2$s","groundhogg"),bold(broadcast.object.data.title),bold(formatDateTime(broadcast.data.send_time*1e3)))}return sprintf(__("Clicked %1$s in %2$s after %3$s","groundhogg"),bold(link),bold(broadcast.object.data.title),bold(formatDateTime(broadcast.data.send_time*1e3)))},edit({broadcast_id,link}){return`
            ...
            ${input({id:"filter-link",name:"link",value:link,autocomplete:"off",placeholder:__("Start typing to select a link or leave blank for any link","groundhogg")})}`},onMount(filter,updateFilter){broadcastPicker("#filter-broadcast",false,items=>{BroadcastsStore.itemsFetched(items)},{},{placeholder:__("Please select a broadcast or leave blank for any broadcast","groundhogg")}).on("change",e=>{updateFilter({broadcast_id:parseInt(e.target.value)})});linkPicker("#filter-link").on("change input blur",({target})=>{updateFilter({link:target.value})})},defaults:{link:"",broadcast_id:0},preload:({broadcast_id})=>{if(broadcast_id){return BroadcastsStore.fetchItem(broadcast_id)}}});registerFilterGroup("site","Site");registerFilter("saved_search","site",__("Saved Search"),{view:({compare:compare="in",search})=>{return sprintf(__("Is %s search %s"),compare==="in"?"in":"not in",bold(SearchesStore.get(search)?.name))},edit:({compare})=>{return[select({name:"filter_compare",id:"filter-compare",options:{in:__("In"),not_in:__("Not in")},selected:compare}),select({name:"filter_search",id:"filter-search"})].join("")},onMount:({search},updateFilter)=>{SearchesStore.maybeFetchItems().then(items=>{$("#filter-search").select2({data:[{id:"",text:""},...items.map(({id,name})=>({id:id,text:name,selected:id===search}))],placeholder:__("Type to search...")}).on("change",e=>{updateFilter({search:e.target.value})})});$("#filter-compare").on("change",e=>{updateFilter({compare:e.target.value})})},defaults:{compare:"in",search:null},preload:({search})=>{if(!SearchesStore.hasItems()){return SearchesStore.fetchItems()}}});const{tabs,fields,groups}=Groundhogg.filters.gh_contact_custom_properties;const getField=id=>{return fields.find(f=>f.id==id)};const filterFactory={text:f=>({view({field,compare,value}){return ComparisonsTitleGenerators[compare](`<b>${f.label}</b>`,`<b>"${value}"</b>`)},edit({compare,value}){return`
            ${select({id:"filter-compare",name:"compare"},StringComparisons,compare)} ${input({id:"filter-value",name:"value",value:value})}`},onMount(filter,updateFilter){$("#filter-compare, #filter-value").on("change",function(e){const $el=$(this);updateFilter({[$el.prop("name")]:$el.val()})})},defaults:{field:f.id,meta:f.name,compare:"equals",value:""}}),url:f=>({...filterFactory.text(f)}),custom_email:f=>({...filterFactory.text(f)}),tel:f=>({...filterFactory.text(f)}),textarea:f=>({...filterFactory.text(f)}),number:f=>({view({field,compare,value}){return ComparisonsTitleGenerators[compare](`<b>${f.label}</b>`,`<b>${value}</b>`)},edit({compare,value}){return`
            ${select({id:"filter-compare",name:"compare"},NumericComparisons,compare)} ${input({id:"filter-value",name:"value",type:"number",step:"0.01",value:value})}`},onMount(filter,updateFilter){$("#filter-compare, #filter-value").on("change",function(e){const $el=$(this);updateFilter({[$el.prop("name")]:$el.val()})})},defaults:{field:f.id,meta:f.name,compare:"equals",value:""}}),date:f=>({view({field,...rest}){return standardActivityDateTitle(`<b>${f.label}</b>`,rest)},edit(filter){return standardActivityDateOptions(filter)},onMount(filter,updateFilter){standardActivityDateFilterOnMount(filter,updateFilter)},defaults:{...standardActivityDateDefaults,field:f.id,meta:f.name}}),datetime:f=>({view({field,...rest}){return standardActivityDateTitle(`<b>${f.label}</b>`,rest)},edit(filter){return standardActivityDateOptions(filter)},onMount(filter,updateFilter){standardActivityDateFilterOnMount(filter,updateFilter)},defaults:{...standardActivityDateDefaults,field:f.id,meta:f.name}}),time:f=>({view({field,compare,value}){return ComparisonsTitleGenerators[compare](`<b>${f.label}</b>`,`<b>${value}</b>`)},edit({compare,value}){return`
            ${select({id:"filter-compare",name:"compare"},NumericComparisons,compare)} ${input({id:"filter-value",name:"value",type:"time",value:value})}`},onMount(filter,updateFilter){$("#filter-compare, #filter-value").on("change",function(e){const $el=$(this);updateFilter({[$el.prop("name")]:$el.val()})})},defaults:{field:f.id,meta:f.name,compare:"equals",value:""}}),radio:f=>({view:({options,compare})=>{return ComparisonsTitleGenerators[compare](`<b>${f.label}</b>`,orList(options.map(o=>bold(o))))},edit:({field,options,compare})=>{return`${select({id:"filter-compare",name:"compare"},{in:_x("Is one of","comparison, groundhogg"),not_in:_x("Is not one of","comparison","groundhogg")},compare)} ${select({id:"filter-options",name:"options",multiple:true},f.options.map(o=>({value:o,text:o})),options)}`},onMount:({field,options},updateFilter)=>{$("#filter-compare").on("change",e=>{updateFilter({compare:$(e.target).val()})});$("#filter-options").select2({multiple:true}).on("change",e=>{updateFilter({options:$(e.target).val()})})},defaults:{field:f.id,meta:f.name,compare:"in",options:[]}}),dropdown:f=>({view:({options,compare})=>{if(ComparisonsTitleGenerators[compare]){return ComparisonsTitleGenerators[compare](`<b>${f.label}</b>`,orList(options.map(o=>bold(o))))}return moreComparisonTitles[compare](`<b>${f.label}</b>`,options)},edit:({field,options,compare})=>{return`${select({id:"filter-compare",name:"compare"},!f.multiple?{in:_x("Is one of","comparison, groundhogg"),not_in:_x("Is not one of","comparison","groundhogg")}:{all_in:__("Has all selected"),all_not_in:__("Does not have all selected")},compare)} ${select({id:"filter-options",name:"options",multiple:true},f.options.map(o=>({value:o,text:o})),options)}`},onMount:({field,options},updateFilter)=>{$("#filter-compare").on("change",e=>{updateFilter({compare:$(e.target).val()})});$("#filter-options").select2({multiple:true}).on("change",e=>{updateFilter({options:$(e.target).val()})})},defaults:{field:f.id,meta:f.name,compare:f.multiple?"all_in":"in",options:[]}}),checkboxes:f=>({view:({options,compare})=>{return moreComparisonTitles[compare](bold(f.label),options)},edit:({field,options,compare})=>{return`${select({id:"filter-compare",name:"compare"},{all_checked:__("Is Checked","groundhogg-better-meta"),not_checked:__("Is Not Checked","groundhogg-better-meta")},compare)} ${select({id:"filter-options",name:"options",multiple:true},f.options.map(o=>({value:o,text:o})),options)}`},onMount:({field,options},updateFilter)=>{$("#filter-compare").on("change",e=>{updateFilter({compare:$(e.target).val()})});$("#filter-options").select2({multiple:true}).on("change",e=>{updateFilter({options:$(e.target).val()})})},defaults:{field:f.id,meta:f.name,compare:"all_checked",options:[]}})};const moreComparisonTitles={all_checked:(prefix,options)=>sprintf(__("%2$s is checked for %1$s","groundhogg-better-meta"),prefix,andList(options.map(b=>bold(b)))),not_checked:(prefix,options)=>sprintf(__("%2$s is not checked for %1$s","groundhogg-better-meta"),prefix,andList(options.map(b=>bold(b)))),all_in:(prefix,options)=>sprintf(__("%2$s is selected for %1$s","groundhogg-better-meta"),prefix,andList(options.map(b=>bold(b)))),all_not_in:(prefix,options)=>sprintf(__("%2$s is not selected for %1$s","groundhogg-better-meta"),prefix,andList(options.map(b=>bold(b))))};Object.values(tabs).forEach(t=>{Object.values(groups).filter(f=>f.tab===t.id).forEach(s=>{let groupId=`${t.id}-${s.id}`;registerFilterGroup(groupId,`${t.name}: ${s.name}`);Object.values(fields).filter(f=>f.group===s.id).forEach(f=>{if(f.type in filterFactory){registerFilter(f.id,groupId,f.label,filterFactory[f.type](f))}})})});const registerActivityFilter=(id,group,label,{view:view=()=>{},edit:edit=()=>{},onMount:onMount=()=>{},defaults:defaults={}})=>{registerFilter(id,group,label,{view(filter){return standardActivityDateTitle(filterCountTitle(view(filter),filter),filter)},edit(filter){return[edit(filter),filterCount(filter),standardActivityDateOptions(filter)].join("")},onMount(filter,updateFilter){onMount(filter,updateFilter);filterCountOnMount(updateFilter);standardActivityDateFilterOnMount(filter,updateFilter)},defaults:{...defaults,...standardActivityDateDefaults,...filterCountDefaults}})};const registerActivityFilterWithValue=(id,group,label,{view:view=()=>{},edit:edit=()=>{},onMount:onMount=()=>{},defaults:defaults={}})=>{registerFilter(id,group,label,{view(filter){let{value,value_compare}=filter;let content=view(filter);if(value&&value_compare){content+=` worth ${activityFilterComparisons[value_compare].toLowerCase()} ${value}`}return standardActivityDateTitle(filterCountTitle(content,filter),filter)},edit(filter){let{value,value_compare}=filter;return[edit(filter),`
              <div class="space-between" style="gap: 10px">
