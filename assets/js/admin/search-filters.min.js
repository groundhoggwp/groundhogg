(function($){const{input:input,select:select,regexp:regexp,specialChars:specialChars,clickInsideElement:clickInsideElement,orList:orList,andList:andList,searchOptionsWidget:searchOptionsWidget}=Groundhogg.element;const{tagPicker:tagPicker,linkPicker:linkPicker}=Groundhogg.pickers;const Filters={view:function(){},edit:function(){},types:{},groups:{}};const renderFilterView=(filter,filterGroupIndex,filterIndex)=>{return`
		...
		<div class="filter filter-edit-wrap" data-key="${filterIndex}" data-group="${filterGroupIndex}">
		  ...
		  ${input({type:"date",value:date2,id:"filter-date2",className:`value ${date_range==="custom"?"":"hidden"}`,name:"date2"})}`},onMount:function(filter,updateFilter){$("#filter-value, #meta_key, #filter-value").on("change",function(e){const $el=$(this);updateFilter({[$el.prop("name")]:$el.val()})})},defaults:{date_range:"24_hours",date:"",date2:"",page_url:""}},"Email Opened");registerFilter("page_visited","activity",{view:function({page_url:page_url,date_range:date_range,date:date,date2:date2}){switch(date_range){default:return`Visited <b>${page_url}</b> ${dateRanges[date_range].toLowerCase()}`;case"custom":return`Visited <b>${page_url}</b> between <b>${date}</b> and <b>${date2}</b>`}},edit:function({page_url:page_url,date_range:date_range,date:date,date2:date2}){return`${input({id:"filter-page-url",name:"page_url",value:page_url})} 
