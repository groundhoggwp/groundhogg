(function($){const{input,select,regexp,specialChars,clickInsideElement,orList,andList,searchOptionsWidget,loadingDots,tooltip,bold}=Groundhogg.element;const{broadcastPicker,funnelPicker,tagPicker,emailPicker,linkPicker,metaValuePicker,metaPicker,userMetaPicker}=Groundhogg.pickers;const{broadcasts:BroadcastsStore,emails:EmailsStore,tags:TagsStore,funnels:FunnelsStore}=Groundhogg.stores;const{sprintf,__,_x,_n}=wp.i18n;const Filters={has(type){return typeof this.types[type]!=="undefined"},view(){},edit(){},types:{},groups:{}};const{formatNumber,formatTime,formatDate,formatDateTime}=Groundhogg.formatting;const renderFilterBroken=(filter,filterGroupIndex,filterIndex)=>{if(!Filters.has(filter.type)){return""}return`
      ...
      ${select({id:"filter-tags",name:"tags",className:"tag-picker",multiple:true},tags.map(t=>({value:t.ID,text:t.data.tag_name})),tags.map(t=>t.ID))}`},onMount(filter,updateFilter){tagPicker("#filter-tags",true,items=>{TagsStore.itemsFetched(items)},{tags:false}).on("change",e=>{updateFilter({tags:$(e.target).val()})});$("#filter-compare, #filter-compare2").on("change",function(e){const $el=$(this);updateFilter({[$el.prop("name")]:$el.val()})})},defaults:{compare:"includes",compare2:"any",tags:[]},preload:({tags})=>{if(!TagsStore.hasItems(tags)){return TagsStore.fetchItems({tag_id:tags})}}});registerFilter("meta","contact",__("Custom meta","groundhogg"),{view({meta,compare,value}){return ComparisonsTitleGenerators[compare](`<b>${meta}</b>`,`<b>"${value}"</b>`)},edit({meta,compare,value},filterGroupIndex,filterIndex){return`
          ...
          ${input({id:"filter-region",name:"region",value:region,autocomplete:"off",placeholder:__("Start typing to select a region","groundhogg")})}`},onMount(filter,updateFilter){metaValuePicker("#filter-region","region").on("change",function(e){updateFilter({region:$(e.target).val()})})},defaults:{region:""}});registerFilter("city","location",__("City","groundhogg"),{view({city}){return sprintf(__("City is %s","groundhogg"),bold(city))},edit({city}){return`
