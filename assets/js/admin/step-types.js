( function ($) {
  ...
  const StepTypes = {
    ...
    apply_note: {
      ...
      title ({ meta }) {
        ...
        if (note_text) {
          return sprintf(_x('Add %s', 'add - note text', 'groundhogg'),
            `<i>${ note_text.replace(/(<([^>]+)>)/gi, '').substring(0, 30) }...</i>`)
        }
                ...
                <div class="gh-rows-and-columns">
                    ...
                    <div class="gh-row">
                        <div class="gh-col">
                            ...
                            <p class="description">A single email address which you'd like the notification to come
                                from.</p>
                        </div>
                        <div class="gh-col">
                            ...
                            <p class="description">A single email address which replies to this notification should be
                                sent
                                to.</p>
                        </div>
                    </div>
                    <div class="gh-row">
                        <div class="gh-col">
                            ...
                            ${ inputWithReplacements({
                                ...
                                id: 'subject',
                                name: 'subject',
                                className: 'regular-text full-width',
                                value: subject,
                            }) }
            ...
            <div class="panel">
                <div class="row">
                    ...
                    ${ select(
                            ...
                            Object.keys(roles).map(r => ( { value: r, text: roles[r]?.name } )),
                            meta.role,
                    ) }
                    <p class="description">
                        ${ __('Runs when a new user is created with any of the given roles. Leave empty to run for every new user.',
                                'groundhogg') }</p>
                </div>
            </div>`
      },
    ...
    tag_applied: {
      ...
      title ({ ID, data, meta }) {
        ...
        if (tags.length >= 4) {
          ...
          return sprintf(_x('%s is applied', 'list of tags - is applied', 'groundhogg'),
            ...
            <div class="panel">
                <div class="row">
                    <label class="row-label" for="tags">
                        ${ sprintf(
                                _x('Run when %s of the following tags are applied...', '%s is any|all', 'groundhogg'),
                                select({
                                    id: 'condition',
                                }, {
    ...
    tag_removed: {
      ...
      title ({ ID, data, meta }) {
        ...
        if (tags.length >= 4) {
          ...
          return sprintf(_x('%s is removed', 'list of tags - is removed', 'groundhogg'),
            `<b>${ TagsStore.get(tags[0]).data.tag_name }</b>`)
        }
        else {
        ...
        return `
            <div class="panel">
                <div class="row">
                    <label class="row-label" for="tags">
                        ${ sprintf(
                                _x('Run when %s of the following tags are removed...', '%s is any|all', 'groundhogg'),
                                select({
                                    id: 'condition',
                                }, {
                                    any: __('Any', 'groundhogg'),
      ...
      edit ({ ID, data, meta }) {
        ...
        return `
            <div class="panel">
                ...
                <div class="row">
                    <label
                            class="row-label">${ _x('Then run at...', 'meaning to run a process at a certain time',
                            'groundhogg') }</label>
                    ${ select({
                                className: 'delay-input re-render',
                                name: 'run_when',
    ...
    send_email: {
      ...
      title ({ ID, data, meta }) {
        ...
        return email_id && email ? sprintf(_x('Send %s', 'send - email title', 'groundhogg'),
          `<b>${ email.data.title }</b>`) : __('Send <b>an email</b>', 'groundhogg')
      },
      edit ({ ID, data, meta }) {
        const { email_id } = meta
        const email = EmailsStore.get(parseInt(email_id))
          ...
          return `
              <div class="panel">
                  <div class="row">
                      ...
                      <div id="from-line">
                          <img class="avatar" alt="avatar" width="40" src="${ context.avatar }"/>
                          <div class="from">
        ...
        return `
            <div class="panel">
                ...
                <div class="row">
                    ...
                    <p class="description">
                        ${ __('Upon clicking the tracking link contacts will be redirected to this page.',
                                'groundhogg') }</p>
                </div>
            </div>`
      },
      ...
      title: ({ meta, data, ID }) => {
        ...
        switch (method) {
          ...
          case 'selected':
            ...
            return sprintf(__('Restart from "%s"', 'groundhogg'),
              StepTypes.getTitle(FunnelsStore.getSteps(funnel_id, true).find(s => s.ID == step_id)))
        }
      },
      edit: ({ meta }) => {
  ...
  const stepTypeSelect = (el, {
    ...
    return $(el).select2({
      ...
      data: [
        { id: '', text: '' },
        ...FunnelsStore.getSteps(funnel_id, edited).
          filter(s => group === 'any' || s.data.step_group == group).
          map(s => ( {
            ...s,
