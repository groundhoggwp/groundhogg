(($,editor)=>{const{contact:contact,meta_exclusions:meta_exclusions}=editor;const{gh_contact_custom_properties:gh_contact_custom_properties}=Groundhogg.filters;const{tooltip:tooltip,regexp:regexp,inputRepeaterWidget:inputRepeaterWidget,searchOptionsWidget:searchOptionsWidget,input:input,select:select,textarea:textarea,icons:icons,bold:bold,modal:modal,uuid:uuid,dangerConfirmationModal:dangerConfirmationModal,adminPageUrl:adminPageUrl,moreMenu:moreMenu,loadingDots:loadingDots,spinner:spinner,dialog:dialog}=Groundhogg.element;const{currentUser:currentUser,filters:filters,propertiesEditor:propertiesEditor,isWPFusionActive:isWPFusionActive}=Groundhogg;const{userHasCap:userHasCap}=Groundhogg.user;const{events:EventsStore,tags:TagsStore,contacts:ContactsStore,emails:EmailsStore,activity:ActivityStore,funnels:FunnelsStore,broadcasts:BroadcastsStore,page_visits:PageVisitsStore,submissions:SubmissionsStore}=Groundhogg.stores;const{post:post,get:get,patch:patch,routes:routes,ajax:ajax}=Groundhogg.api;const{sprintf:sprintf,__:__,_x:_x,_n:_n}=wp.i18n;ContactsStore.itemsFetched([contact]);const getContact=()=>{return ContactsStore.get(contact.ID)};const sanitizeKey=label=>{return label.toLowerCase().replace(/[^a-z0-9]/g,"_")};const sendEmail=()=>{let contact=getContact();let email={to:[contact.data.email],from_email:currentUser.data.user_email,from_name:currentUser.data.display_name};if(contact.data.owner_id&&currentUser.ID!=contact.data.owner_id){email.cc=[filters.owners.find(u=>u.ID==contact.data.owner_id).data.user_email]}Groundhogg.components.emailModal(email)};const contactMoreActions=()=>{let actions=`
    <button id="action-send-email" class="gh-button secondary text icon">${icons.email}</button>
			...
			</li>`}if(activity.type==="event"){let{step:step}=activity;switch(parseInt(activity.data.event_type)){case 1:return`
				<li class="activity-item">
		  ...
		  <ul id="activity-timeline">
			  ...
			  <div id="activity-here">
				  ${spinner()}
			  </div>`},onMount:()=>{let order="desc";let filter="all";$("#activity-order").on("change",e=>{order=e.target.value;fetchActivity()});$("#filter-by").on("change",e=>{filter=e.target.value;loadTimeline()});const fetchActivity=()=>{Promise.all([SubmissionsStore.fetchItems({contact_id:contact.ID,limit:50,order:order,orderby:"date_created"}),ActivityStore.fetchItems({contact_id:contact.ID,limit:50,order:order,orderby:"timestamp"}),EventsStore.fetchItems({contact_id:contact.ID,limit:50,orderby:"time",order:order}),PageVisitsStore.fetchItems({contact_id:contact.ID,limit:50,orderby:"timestamp",order:order})]).then(()=>{loadTimeline()})};const loadTimeline=()=>{let allActivities=[...SubmissionsStore.getItems().map(a=>({...a,type:"submission",time:parseInt(a.data.time)})),...ActivityStore.getItems().map(a=>({...a,type:"activity",time:parseInt(a.data.timestamp)})),...EventsStore.getItems().map(e=>({...e,type:"event",time:parseInt(e.data.time)+parseFloat(e.data.micro_time)})),...PageVisitsStore.getItems().map(v=>({...v,type:"page_visit",time:parseInt(v.data.timestamp)}))].sort((a,b)=>order==="desc"?b.time-a.time:a.time-b.time);switch(filter){case"submissions":allActivities=allActivities.filter(a=>a.type==="submission");break;case"funnel":allActivities=allActivities.filter(a=>a.type==="event"&&a.data.event_type==1);break;case"email":allActivities=allActivities.filter(a=>a.data.email_id>0);break;case"web":allActivities=allActivities.filter(a=>a.type==="page_visit");break;case"wp_fusion":allActivities=allActivities.filter(a=>a.type==="activity"&&a.data.activity_type==="wp_fusion");break}ActivityTimeline.mount("#activity-here",allActivities);$("#activity-here").css({maxHeight:$("#primary-contact-stuff").height()})};if(ActivityStore.hasItems()||EventsStore.hasItems()||PageVisitsStore.hasItems()||SubmissionsStore.hasItems()){loadTimeline();return}fetchActivity()}},{id:"notes",name:__("Notes"),render:()=>{return`
			  <div class="gh-panel top-left-square">
              ...
              <button id="bulk-delete-files" class="gh-button danger icon"><span class="dashicons dashicons-trash"></span></button>
			  ...
			  </div>`},onMount:()=>{let files=[];let fileSearch="";$("#search-files").on("input change",e=>{fileSearch=e.target.value;mount()});tooltip("#bulk-delete-files",{content:__("Bulk delete files"),position:"right"});const renderFile=file=>{return`
				<tr class="file">
					<th scope="row" class="check-column">${input({type:"checkbox",name:"select[]",className:"file-toggle",value:file.file_name})}
					</th>
				...
				</tr>`};const mount=()=>{$("#files-here").html(files.filter(f=>!fileSearch||f.file_name.match(regexp(fileSearch))).map(f=>renderFile(f)).join(""));onMount()};const onMount=()=>{let selectedFiles=[];const maybeShowBulkActions=()=>{if(selectedFiles.length){$("#bulk-actions").removeClass("hidden")}else{$("#bulk-actions").addClass("hidden")}};$(".file-toggle").on("change",e=>{if(e.target.checked){selectedFiles.push(e.target.value)}else{selectedFiles.splice(selectedFiles.indexOf(e.target.value),1)}maybeShowBulkActions()})};$("#upload-file").on("click",e=>{e.preventDefault();Groundhogg.components.fileUploader({action:"groundhogg_contact_upload_file",nonce:"",beforeUpload:fd=>fd.append("contact",contact.ID),onUpload:(json,file)=>{files=json.files;mount()}})});ContactsStore.fetchFiles(contact.ID).then(_files=>{files=_files;mount()})}},{id:"inbox",name:__("Inbox"),render:()=>{return`
			  <div class="gh-panel top-left-square">
		  ...
		  <div id="secondary-tabs"><h2 class="nav-tab-wrapper secondary gh">
			...
			</div>`;$(metaUi).insertAfter("#primary-contact-stuff form");$("#cancel-meta-changes").on("click",cancelMetaChanges);let combinedMeta={...getContact().meta,...metaChanges};inputRepeaterWidget({selector:"#contact-phones-here",rows:[],cellProps:[{type:"email",options:{mobile:__("Mobile"),home:__("Home"),business:__("Business")}},{type:"tel",placeholder:__("(123) 456-7890")}],cellCallbacks:[select,input],onMount:()=>{},onChange:rows=>{},onRemove:([key,value])=>{}}).mount();inputRepeaterWidget({selector:"#contact-emails-here",rows:[],cellProps:[{type:"email",className:"alternate-email-address",placeholder:__("john.doe@example.com","groundhogg")}],cellCallbacks:[input],onMount:()=>{$(`<button class="make-primary">${"P"}</button>`).insertAfter(".alternate-email-address")},onChange:rows=>{},onRemove:([key,value])=>{}}).mount();inputRepeaterWidget({selector:"#meta-here",rows:Object.keys(combinedMeta).filter(k=>!meta_exclusions.includes(k)).map(k=>[k,combinedMeta[k]]).map(([k,v])=>[k,["array","object"].includes(typeof v)?"SERIALIZED DATA":v]),cellProps:[{className:"meta-key",readonly:true},{}],cellCallbacks:[input,input],onMount:()=>{$(`[value="SERIALIZED DATA"]`).prop("readonly",true);$(".meta-key").each((i,el)=>{let $el=$(el);if(!$el.val()){$el.prop("readonly",false)}}).on("input",e=>{let key=sanitizeKey(e.target.value);$(e.target).val(key)})},onChange:rows=>{rows.forEach(([key,value])=>{if(!key){return}metaChanges[key]=value})},onRemove:([key,value])=>{if(!key){return}deleteKeys.push(key);delete metaChanges[key]}}).mount();$("#save-meta").on("click",commitMetaChanges)}};$(".nav-tab-wrapper.primary").append(`<div id="tab-actions" class="space-between"><button type="button" id="add-tab"><span class="dashicons dashicons-plus-alt2"></span></button></div>`);$("#add-tab").on("click",e=>{e.preventDefault();modal({content:`
			<div>
