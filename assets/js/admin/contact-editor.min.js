(($,editor)=>{const{contact,meta_exclusions}=editor;const{gh_contact_custom_properties}=Groundhogg.filters;const{tooltip,regexp,inputRepeaterWidget,el,searchOptionsWidget,input,select,isNumeric,textarea,icons,bold,loadingModal,modal,uuid,dangerConfirmationModal,confirmationModal,adminPageURL,moreMenu,setFrameContent,loadingDots,spinner,dialog}=Groundhogg.element;const{currentUser,filters,propertiesEditor,isWPFusionActive,StepTypes}=Groundhogg;const{userHasCap}=Groundhogg.user;const{events:EventsStore,event_queue:EventQueue,tags:TagsStore,contacts:ContactsStore,emails:EmailsStore,activity:ActivityStore,funnels:FunnelsStore,broadcasts:BroadcastsStore,page_visits:PageVisitsStore,submissions:SubmissionsStore}=Groundhogg.stores;const{emailPicker}=Groundhogg.pickers;const{post,delete:_delete,get,patch,routes,ajax}=Groundhogg.api;const{selectContactModal,betterTagPicker,internalForm}=Groundhogg.components;const{sprintf,__,_x,_n}=wp.i18n;ContactsStore.itemsFetched([contact]);let files=[];const activityUpdated=()=>{window.dispatchEvent(new Event("activityupdated"))};const getContact=()=>{return ContactsStore.get(contact.ID)};const sanitizeKey=label=>{return label.toLowerCase().replace(/[^a-z0-9]/g,"_")};const sendEmail=()=>{let contact=getContact();let email={to:[contact.data.email],from_email:currentUser.data.user_email,from_name:currentUser.data.display_name};if(contact.data.owner_id&&currentUser.ID!=contact.data.owner_id){email.cc=[filters.owners.find(u=>u.ID==contact.data.owner_id).data.user_email]}Groundhogg.components.emailModal(email)};const contactMoreActions=()=>{let actions=`
    <button id="action-send-email" class="gh-button secondary text icon">${icons.email}</button>
            ...
            <li class="activity-item">
                ...
                <li class="activity-item">
                    ...
                    <div class="activity-rendered gh-panel space-between">
                        <div>
                            <div class="activity-info">
                                <span>${sprintf(step.data.step_group==="action"?pending?__("Pending action: %s | %s","groundhogg"):__("Completed action: %s | %s","groundhogg"):pending?__("Pending benchmark: %s | %s","groundhogg"):__("Completed benchmark: %s | %s","groundhogg"),StepTypes.getType(step.data.step_type).title(step),Groundhogg.rawStepTypes[step.data.step_type].name)}</span>
                            </div>
          ...
          <ul id="activity-timeline">
              ${activities.map(a=>{try{return this.renderActivity(a)}catch(e){console.log(e);return""}}).join("")}
