(($,editor)=>{const{contact:contact,meta_exclusions:meta_exclusions,gh_contact_custom_properties:gh_contact_custom_properties}=editor;const{tooltip:tooltip,regexp:regexp,inputRepeaterWidget:inputRepeaterWidget,searchOptionsWidget:searchOptionsWidget,input:input,select:select,icons:icons,bold:bold,modal:modal,uuid:uuid,dangerConfirmationModal:dangerConfirmationModal,adminPageUrl:adminPageUrl,moreMenu:moreMenu,loadingDots:loadingDots,spinner:spinner,dialog:dialog}=Groundhogg.element;const{currentUser:currentUser,filters:filters,propertiesEditor:propertiesEditor}=Groundhogg;const{userHasCap:userHasCap}=Groundhogg.user;const{events:EventsStore,tags:TagsStore,contacts:ContactsStore,emails:EmailsStore,activity:ActivityStore,funnels:FunnelsStore,broadcasts:BroadcastsStore,page_visits:PageVisitsStore}=Groundhogg.stores;const{post:post,get:get,patch:patch,routes:routes,ajax:ajax}=Groundhogg.api;const{sprintf:sprintf,__:__,_x:_x,_n:_n}=wp.i18n;ContactsStore.itemsFetched([contact]);const getContact=()=>{return ContactsStore.get(contact.ID)};const sanitizeKey=label=>{return label.toLowerCase().replace(/[^a-z0-9]/g,"_")};const sendEmail=()=>{let contact=getContact();let email={to:[contact.data.email],from_email:currentUser.data.user_email,from_name:currentUser.data.display_name};if(contact.data.owner_id&&currentUser.ID!=contact.data.owner_id){email.cc=[filters.owners.find(u=>u.ID==contact.data.owner_id).data.user_email]}Groundhogg.components.emailModal(email)};const contactMoreActions=()=>{let actions=`
    <button id="action-send-email" class="gh-button secondary text icon">${icons.email}</button>
				...
				<button id="contact-more" class="gh-button secondary text icon">${icons.verticalDots}</button>`;$("#contact-more-actions").html(actions);tooltip("#action-send-email",{content:__("Send email","groundhogg")});tooltip("#call-primary",{content:__("Call","groundhogg")});tooltip("#call-mobile",{content:__("Call mobile","groundhogg")});tooltip("#contact-more",{content:__("More actions","groundhogg")});$("#contact-more").on("click",e=>moreMenu(e.target,{items:[{key:"delete",cap:"delete_contacts",text:`<span class="gh-text danger">${__("Delete")}</span>`}],onSelect:k=>{switch(k){case"delete":dangerConfirmationModal({confirmText:__("Delete"),alert:`<p>${sprintf(__("Are you sure you want to delete %s?","groundhogg"),bold(getContact().data.full_name))}</p>`,onConfirm:()=>{ContactsStore.delete(contact.ID).then(()=>{dialog({message:sprintf(__("%s was deleted!","groundhogg"),getContact().data.full_name)});window.location.href=adminPageUrl("gh_contacts")})}})}}}));$("#action-send-email").on("click",sendEmail)};const ActivityTimeline={types:{wp_fusion:{icon:icons.wp_fusion,render:({data:data,meta:meta})=>{const{event_name:event_name,event_value:event_value}=meta;return`${event_name}: <code>${event_value}</code>`},preload:()=>{}},wp_login:{icon:icons.login,render:({email:email})=>{return __("Logged in","groundhogg")},preload:()=>{}},wp_logout:{icon:icons.logout,render:({email:email})=>{return __("Logged out","groundhogg")},preload:()=>{}},email_opened:{icon:icons.open_email,render:({email:email})=>{return sprintf(__("Opened %s","groundhogg"),bold(email.data.title))},preload:({email:email})=>{EmailsStore.itemsFetched([email])}},email_link_click:{icon:icons.link_click,render:({email:email,data:data})=>{return sprintf(__("Clicked %s in %s","groundhogg"),el("a",{target:"_blank",href:data.referer},bold(data.referer)),bold(email.data.title))},preload:({email:email})=>{EmailsStore.itemsFetched([email])}}},renderActivity:function(activity){if(activity.type==="page_visit"){return`
			...
			</li>`}if(activity.type==="event"&&activity.data.event_type==1){let{step:step}=activity;return`
			<li class="activity-item">
				  ...
				  <div class="inside">
					  <div class="align-left-space-between">
						  ...
						  <div class="filter-by">
							  ...
							  ${select({id:"filter-by",name:"filter"},{all:__("All Activity","groundhogg"),funnel:__("Funnel Activity","groundhogg"),email:__("Email Activity","groundhogg"),web:__("Web Activity","groundhogg"),wp_fusion:__("WPFusion Activity","groundhogg")},"")}
						  </div>
		  ...
		  <div id="secondary-tabs"><h2 class="nav-tab-wrapper secondary gh">
			...
			</div>`;$(metaUi).insertAfter("#primary-contact-stuff form");$("#cancel-meta-changes").on("click",cancelMetaChanges);let combinedMeta={...getContact().meta,...metaChanges};inputRepeaterWidget({selector:"#meta-here",rows:Object.keys(combinedMeta).filter(k=>!meta_exclusions.includes(k)).map(k=>[k,combinedMeta[k]]).map(([k,v])=>[k,["array","object"].includes(typeof v)?"SERIALIZED DATA":v]),cellProps:[{className:"meta-key",readonly:true},{}],cellCallbacks:[input,input],onMount:()=>{$(`[value="SERIALIZED DATA"]`).prop("readonly",true);$(".meta-key").each((i,el)=>{let $el=$(el);if(!$el.val()){$el.prop("readonly",false)}}).on("input",e=>{let key=sanitizeKey(e.target.value);$(e.target).val(key)})},onChange:rows=>{rows.forEach(([key,value])=>{if(!key){return}metaChanges[key]=value})},onRemove:([key,value])=>{if(!key){return}deleteKeys.push(key);delete metaChanges[key]}}).mount();$("#save-meta").on("click",commitMetaChanges)}};$(".nav-tab-wrapper.primary").append(`<div id="tab-actions" class="space-between"><button type="button" id="add-tab"><span class="dashicons dashicons-plus-alt2"></span></button></div>`);$("#add-tab").on("click",e=>{e.preventDefault();modal({content:`
			<div>
