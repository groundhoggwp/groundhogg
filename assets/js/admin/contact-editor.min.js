                ...
                <div>
                  ...
                  <p>
                      <a href="https://help.groundhogg.io/article/540-merging-contacts"
                         target="_blank">${__("What happens when contacts are merged?","groundhogg")}</a></p>`,onConfirm:()=>{loadingModal();post(`${ContactsStore.route}/${contact.ID}/merge`,[_contact.ID]).then(()=>{location.reload()})}})}});break;case"delete":dangerConfirmationModal({confirmText:__("Delete"),alert:`<p>${sprintf(__("Are you sure you want to delete %s?","groundhogg"),bold(getContact().data.full_name))}</p>`,onConfirm:()=>{ContactsStore.delete(contact.ID).then(()=>{dialog({message:sprintf(__("%s was deleted!","groundhogg"),contact.data.full_name)});window.open(adminPageURL("gh_contacts"),"_self")})}})}}}));$("#action-send-email").on("click",sendEmail)};const ActivityTimeline={addType:(type,opts)=>{this.types[type]={icon:"",render:()=>"",preload:()=>{},...opts}},types:{wp_fusion:{icon:icons.wp_fusion,render:({data,meta})=>{const{event_name,event_value}=meta;return`${event_name}: <code>${event_value}</code>`},preload:()=>{}},wp_login:{icon:icons.login,render:({email})=>{return __("Logged in","groundhogg")},preload:()=>{}},wp_logout:{icon:icons.logout,render:({email})=>{return __("Logged out","groundhogg")},preload:()=>{}},email_opened:{icon:icons.open_email,render:({email})=>{return sprintf(__("Opened %s","groundhogg"),bold(email.data.title))},preload:({email})=>{EmailsStore.itemsFetched([email])}},email_link_click:{icon:icons.link_click,render:({email,data})=>{const maybeTruncateLink=link=>{return link.length>50?`${link.substring(0,47)}...`:link};return sprintf(__("Clicked %s in %s","groundhogg"),el("a",{target:"_blank",href:data.referer},bold(maybeTruncateLink(data.referer))),bold(email.data.title))},preload:({email})=>{EmailsStore.itemsFetched([email])}}},renderActivity(activity){if(activity.type==="submission"){let funnel=FunnelsStore.get(activity.form.data.funnel_id);return`
