($=>{const{moreMenu,loadingModal,icons,select,modal,input,uuid,dialog,dangerConfirmationModal,adminPageURL}=Groundhogg.element;const{routes,post,get}=Groundhogg.api;const{options:OptionsStore}=Groundhogg.stores;const{createFilters}=Groundhogg.filters.functions;const{metaPicker}=Groundhogg.pickers;const{__}=wp.i18n;function adjust(color,amount){return"#"+color.replace(/^#/,"").replace(/../g,color=>("0"+Math.min(255,Math.max(0,parseInt(color,16)+amount)).toString(16)).substr(-2))}function utf8_to_b64(str){return window.btoa(unescape(encodeURIComponent(str)))}function b64_to_utf8(str){return decodeURIComponent(escape(window.atob(str)))}const base64_json_encode=stuff=>{return utf8_to_b64(JSON.stringify(stuff))};const ReportTypes={pie_chart:{name:__("Pie Chart","groundhogg"),settings:({field=""})=>{return`
        ...
        `},onMount:({id,field,data})=>{let cuttoff=11;let ctx=$(`.pie-chart[data-id=${id}]`)[0].getContext("2d");let _data=data;if(_data.length>cuttoff){_data=data.slice(0,cuttoff);let _rest=data.slice(cuttoff).reduce((carr,i)=>{return carr+parseInt(i.count)},0);_data.push({count:_rest,value:__("Other")})}let chart=new Chart(ctx,{type:"doughnut",data:{datasets:[{data:_data.map(({count})=>count),backgroundColor:_data.map((d,i)=>adjust("#4fa4ff",-(i*30)))}],labels:_data.map(({value})=>value)},options:{onClick:(e,arr)=>{if(arr.length&&arr[0]._index===cuttoff&&arr[0]._view.label===__("Other")){let _data=data.slice(cuttoff);modal({content:`<canvas class="pie-chart-large" style="height: 600px" data-id="${id}"></canvas>`,onOpen:()=>{let ctx=$(`.pie-chart-large[data-id=${id}]`)[0].getContext("2d");let chart=new Chart(ctx,{type:"doughnut",data:{datasets:[{data:_data.map(({count})=>count),backgroundColor:_data.map((d,i)=>adjust("#4fa4ff",-(i*30)))}],labels:_data.map(({value})=>value)},options:{maintainAspectRatio:false,aspectRatio:1,onClick:(e,arr)=>{if(arr.length&&arr[0]._view){window.open(adminPageURL("gh_contacts",{meta_key:field,meta_value:arr[0]._view.label}),"_blank")}},legend:{position:"bottom"}}})}});return}if(arr.length&&arr[0]._view){window.open(adminPageURL("gh_contacts",{meta_key:field,meta_value:arr[0]._view.label}),"_blank")}},legend:{position:"right"}}})}},table:{name:__("Table","groundhogg"),settings:({field=""})=>{return`
            ...
            </div>`:""}`},onMount:({id,data,field})=>{let num=10;const setData=()=>{$(`#${id} tbody`).html(data.slice(0,num).map(row=>dataRow(row)).join(""));$(`#${id} .gh-input-group`).html([10,25,50].filter(i=>i<data.length).map(_num=>`<button class="gh-button ${num===_num?"primary":"secondary"} num-records" data-num="${_num}">${_num}</button>`));$(`#${id} .num-records`).on("click",e=>{num=parseInt(e.target.dataset.num);setData()});$(`.number-total[data-id=${id}]`).on("click",e=>{window.open(adminPageURL("gh_contacts",{meta_key:field,meta_value:e.target.dataset.value}),"_blank")})};const dataRow=({value,count})=>{return`
			  <tr>
				  <td>${value}</td>
				  <td class="number-total" data-id="${id}" data-value="${value}">${count}</td>
			  </tr>`};setData()}},number:{name:__("Number","groundhogg"),settings:({value="contacts",field="",activity=""})=>{const maybeExtra=()=>{switch(value){case"contacts":return"";case"sum":case"average":case"distinct":return`
				  <div class="row">
					  <div class="col">
						  <label for="value">${__("Custom Field")}</label>
						  ${input({id:"field",value:field})}
			...
			</div>`},onMount:({id,filters=[],exclude_filters=[]})=>{$(`.big-number[data-id=${id}]`).on("click",e=>{window.open(adminPageURL("gh_contacts",{filters:base64_json_encode(filters),exclude_filters:base64_json_encode(exclude_filters)}),"_blank")})}}};const renderReport=report=>{if(report.type==="header"){return`
