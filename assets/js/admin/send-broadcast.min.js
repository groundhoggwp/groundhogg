(function($){const{modal:modal,select:select,stepNav:stepNav,stepNavHandler:stepNavHandler,input:input,errorDialog:errorDialog,setFrameContent:setFrameContent,progressBar:progressBar,tooltip:tooltip}=Groundhogg.element;const{emailPicker:emailPicker,searchesPicker:searchesPicker}=Groundhogg.pickers;const{emails:EmailsStore,searches:SearchesStore,contacts:ContactsStore}=Groundhogg.stores;const{routes:routes,post:post}=Groundhogg.api;const{createFilters:createFilters}=Groundhogg.filters.functions;const SendBroadcast=(selector,{email:email=false,query:query={}}={})=>{let state={email_id:email?email.ID:false,when:"now",which:"filters",query:query};const setState=newState=>{state={...state,...newState};console.log(state)};const elPrefix="gh-broadcast";if(email){EmailsStore.itemsFetched([email])}const step1=()=>{const preview=()=>{return`
			<div class="gh-row">
				<div class="gh-col">
      ...
      `};const step2=()=>{const laterSettings=()=>{return`
		  ...
		  </div>`};$(selector).html(`<div id="gh-send-broadcast-form"></div>`);const{$el:$el,nextStep:nextStep,lastStep:lastStep,setStep:setStep}=stepNavHandler("#gh-send-broadcast-form",{currentStep:0,steps:[step1,step2,step3,step4],showNav:true,labels:["Email","Schedule","Contacts","Review"],onStepChange:(step,{nextStep:nextStep,lastStep:lastStep,setStep:setStep})=>{switch(step){case 0:const showFrame=()=>{if(state.email_id){setFrameContent($(`#${elPrefix}-email-preview`)[0],EmailsStore.get(state.email_id).context.built)}};emailPicker(`#${elPrefix}-email`,false,items=>{EmailsStore.itemsFetched(items)},{status:"ready"}).on("change",({target:target})=>{setState({email_id:parseInt(target.value)});setStep(0);showFrame()});showFrame();break;case 1:$(".change-when").on("change",({target:target})=>{setState({when:$(target).val()});setStep(1)});const updateButton=()=>{const isValid=state.when==="now"||moment().isBefore(`${state.date} ${state.time}`);$(".gh-next-step").prop("disabled",!isValid)};$(`#${elPrefix}-date`).on("change",({target:target})=>{setState({date:target.value});updateButton()});$(`#${elPrefix}-time`).on("change",({target:target})=>{setState({time:target.value});updateButton()});break;case 2:const updateTotal=()=>{const query={...state.query,optin_status:[1,2,4,6]};ContactsStore.count(query).then(total=>{$(`#${elPrefix}-total-contacts`).html(`${total} contacts found`);$(".gh-next-step").prop("disabled",total===0);setState({total_contacts:total})})};$(".change-search-which").on("change",({target:target})=>{setState({which:$(target).val()});setStep(2)});if(state.which==="filters"){createFilters(`#${elPrefix}-search-method`,state.query.filters,filters=>{setState({query:{filters:filters}});updateTotal()}).mount()}else{searchesPicker(`#${elPrefix}-search-method-searches`,items=>{SearchesStore.itemsFetched(items)}).on("change",({target:target})=>{setState({query:{saved_search:$(target).val()}});updateTotal()})}break;case 3:$(`#${elPrefix}-confirm`).on("click",()=>{const{query:query={},total_contacts:total_contacts=0,when:when="now",date:date="",time:time="",send_in_local_time:send_in_local_time=false}=state;post(routes.v4.broadcasts,{object_id:state.email_id,object_type:"email",query:query,date:date,time:time,send_now:when==="now",send_in_local_time:send_in_local_time}).then(r=>r.item).then(b=>{const{setProgress:setProgress}=progressBar("#gh-send-broadcast-form");const schedule=()=>{post(`${routes.v4.broadcasts}/${b.ID}/schedule`).then(({finished:finished,scheduled:scheduled})=>{setProgress(scheduled/total_contacts);if(!finished){schedule()}else{setTimeout(()=>{$(`#gh-send-broadcast-form`).html(`
								<div><p>Broadcast scheduled!</p>
									<p><a href="#" id="schedule-another-broadcast">&larr; Schedule another broadcast</a>
									</p></div>`);$("#schedule-another-broadcast").on("click",()=>{setState({email_id:false,when:"now",which:"filters",query:{}});setStep(0)})},500)}}).catch(()=>{errorDialog({message:"Something went wrong..."})})};schedule()}).catch(()=>{errorDialog({message:"Something went wrong..."});setStep(3)})});break}$(".gh-next-step").on("click",nextStep)}})};Groundhogg.SendBroadcast=SendBroadcast;$(()=>{$("#gh-schedule-broadcast").on("click",e=>{e.preventDefault();modal({content:`<div id="gh-broadcast-form" style="width: 400px"></div>`});SendBroadcast("#gh-broadcast-form")})})})(jQuery);