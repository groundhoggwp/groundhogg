  ...
  const EmailEditor = ({
    ...
    components: {
      ...
      header () {
        ...
        const titleDisplay = () => {
          return `<span id="email-title">${ specialChars(
            this.email.data.title) }</span><span class="dashicons dashicons-edit"></span>`
        }
        ...
        return `
            ...
            <div class="actions">
                ...
                <div class="publish-actions">
                    ${ this.email.data.status === 'ready'
                            ? `<button id="to-draft" class="gh-button danger text">Back to draft</button>
					<button id="commit" class="gh-button primary" ${ this.hasChanges() ? '' : 'disabled' }>Update</button>`
                            : `<button id="commit" class="gh-button action">Publish</button>` }
                </div>
          ...
          switch (key) {
              ...
              const campaignContent = () => {
                ...
                return `
                    <div class="manage-campaigns" style="width: 400px">
                        ...
                        <p>
                            ${ __('Campaigns are a tool to group marketing assets for reporting. Visit the dashboard to see an analytics breakdown by campaign.',
                                    'groundhogg') }</p>
                    </div>`
              }
                ...
                if (this.email.meta.sharing !== 'enabled') {
                  ...
                  return `
                      <div class="share">
                          ...
                          <p>
                              ${ __('When sharing is enabled this email can be downloaded via a private link.',
                                      'groundhogg') }</p>
                      </div>`
                }
                else {
                  ...
                  return `
                      <div class="share">
                          ...
                          <p>
                              ${ __('Anyone with the above link will be able to download a copy of this email.',
                                      'groundhogg') }</p>
                          <p>${ __('Enable sharing?', 'groundhogg') } ${ toggle({
                              name: 'sharing',
                              id: 'sharing-enabled',
            ...
            case 'archive':
              ...
              dangerConfirmationModal({
                ...
                alert: `
                    <p>
                        <b>${ _x('Archive this funnel?', 'archive is representing a verb in this phrase',
                                'groundhogg') }</b>
                    </p>
                    <p>
                        ${ __('Any active contacts will be removed from the funnel permanently. The funnel will become un-editable until restored.',
                                'groundhogg') }</p>`,
            ...
            case 'send':
              ...
              if (this.email.data.status !== 'ready') {
                confirmationModal({
                  alert: `<p>${ __(
                    'Before this email can be sent it must be published. Would you like to publish it now?',
                    'groundhogg') }<p>`,
            ...
            items: [
              {
                key: 'campaigns',
                //language=HTML
                text: `${ icons.campaign } ${ _x('Campaigns', 'noun meaning collection of marketing materials',
                        'groundhogg') }`,
              },
              {
                key: 'export',
              ...
              },
              {
                ...
                text: `${ icons.folder } <span
                        class="gh-text danger">${ _x('Archive', 'a verb meaning to add an item to an archive',
                        'groundhogg') }</span>`,
              },
              {
                key: 'delete',
