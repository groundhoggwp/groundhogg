(function($){const{copyObject:copyObject,select:select,input:input,tinymceElement:tinymceElement,specialChars:specialChars,improveTinyMCE:improveTinyMCE,inputWithReplacementsAndEmojis:inputWithReplacementsAndEmojis,inputWithReplacements:inputWithReplacements,inputRepeaterWidget:inputRepeaterWidget}=Groundhogg.element;const{post:post,get:get,patch:patch,routes:routes}=Groundhogg.api;const EmailEditor=({selector:selector,email:email,onChange:onChange=email=>{},onCommit:onCommit=email=>{}})=>({selector:selector,email:copyObject(email),origEmail:copyObject(email),$el:$(selector),undoStates:[],redoStates:[],edited:{data:{},meta:{}},components:{editor:function(){return`
        ...
        `},content:function(){return`
			<div class="inline-label">
				...
				${inputWithReplacementsAndEmojis({id:"subject",name:"subject",placeholder:"Subject line...",value:this.email.data.subject})}
			</div>
			<div class="inline-label">
				<label for="preview-text">Preview:</label>
				${inputWithReplacementsAndEmojis({id:"preview-text",name:"pre_header",value:this.email.data.pre_header})}
			</div>
			<div class="email-content-wrap">
				<textarea id="content" name="content">${this.email.data.content||""}</textarea>
			</div>
        `},sidebar:function(){const message_typeOptions={marketing:"Marketing",transactional:"Transactional"};const{reply_to_override:reply_to_override="",alignment:alignment="left",from_user:from_user=0}=this.edited.meta;return`
			<div class="gh-panel">
				<div class="inside">
					...
					<p>
						...
						${input({id:"reply-to",name:"reply_to",value:reply_to_override})}
					</p>
					<div id="email-editor-sidebar-options">
						...
						<div id="email-editor-sidebar-message_type">
							...
							${select({id:"message-type",name:"message_type"},message_typeOptions,this.email.meta.message_type)}
						</div>
					</div>
				</div>
			</div>
