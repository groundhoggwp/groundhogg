(function($){const{copyObject:copyObject,select:select,input:input,tinymceElement:tinymceElement,specialChars:specialChars,breadcrumbs:breadcrumbs,improveTinyMCE:improveTinyMCE,inputWithReplacementsAndEmojis:inputWithReplacementsAndEmojis,inputWithReplacements:inputWithReplacements,inputRepeaterWidget:inputRepeaterWidget,textarea:textarea,isValidEmail:isValidEmail,modal:modal,loadingDots:loadingDots,objectEquals:objectEquals,codeEditor:codeEditor,savingModal:savingModal}=Groundhogg.element;const{post:post,get:get,patch:patch,routes:routes}=Groundhogg.api;const{user_test_email:user_test_email}=Groundhogg;const{emails:EmailsStore}=Groundhogg.stores;const setFrameContent=(frame,content)=>{var blob=new Blob([content],{type:"text/html; charset=utf-8"});frame.src=URL.createObjectURL(blob)};const EmailEditor=({selector:selector,email:email,onChange:onChange=email=>{},onCommit:onCommit=email=>{},breadcrumbs:crumbs=["Emails"],onHeaderMount:onHeaderMount=()=>{},afterPublishActions:afterPublishActions=""})=>({selector:selector,email:copyObject(email),origEmail:copyObject(email),$el:$(selector),undoStates:[],redoStates:[],edited:{data:{},meta:{}},components:{editor:function(){return`
			<div id="email-editor">
