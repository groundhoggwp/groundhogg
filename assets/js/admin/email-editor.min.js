(function($){const{copyObject:copyObject,select:select,input:input,tinymceElement:tinymceElement,specialChars:specialChars,improveTinyMCE:improveTinyMCE,inputWithReplacementsAndEmojis:inputWithReplacementsAndEmojis,inputWithReplacements:inputWithReplacements,inputRepeaterWidget:inputRepeaterWidget,textarea:textarea,isValidEmail:isValidEmail,modal:modal,loadingDots:loadingDots,codeEditor:codeEditor,savingModal:savingModal}=Groundhogg.element;const{post:post,get:get,patch:patch,routes:routes}=Groundhogg.api;const{user_test_email:user_test_email}=Groundhogg;const setFrameContent=(frame,content)=>{var blob=new Blob([content],{type:"text/html; charset=utf-8"});frame.src=URL.createObjectURL(blob)};const EmailEditor=({selector:selector,email:email,onChange:onChange=email=>{},onCommit:onCommit=email=>{},beforeBreadcrumbs:beforeBreadcrumbs='<span class="root">Emails</span>',onHeaderMount:onHeaderMount=()=>{},afterPublishActions:afterPublishActions=""})=>({selector:selector,email:copyObject(email),origEmail:copyObject(email),$el:$(selector),undoStates:[],redoStates:[],edited:{data:{},meta:{}},components:{editor:function(){return`
			<div id="email-editor">
        ...
        `},inspector:function(){}},autoSaveTimeout:null,abortController:null,autoSaveChanges:function(){this.saveUndoState();if(this.autoSaveTimeout){clearTimeout(this.autoSaveTimeout)}this.autoSaveTimeout=setTimeout(()=>{this.autoSaveTimeout=null;this.abortController=new AbortController;const{signal:signal}=this.abortController;post(`${routes.v4.emails}/${this.email.ID}/meta`,{edited:this.edited},{signal:signal}).then(()=>{this.abortController=null})},3e3)},commitChanges:function(){if(this.autoSaveTimeout){clearTimeout(this.autoSaveTimeout)}else if(this.abortController){this.abortController.abort()}const{close:close}=savingModal();patch(`${routes.v4.emails}/${this.email.ID}`,{data:this.edited.data,meta:this.edited.meta}).then(d=>{this.loadEmail(d.item);onCommit(this.email);close()})},updateEmailData:function(newData){this.edited.data={...this.edited.data,...newData};this.autoSaveChanges()},updateEmailMeta:function(newMeta){this.edited.meta={...this.edited.meta,...newMeta};this.autoSaveChanges();onChange(this.edited,this.email)},render:function(){return this.components.editor.call(this)},mount:function(){improveTinyMCE();this.loadEmail(this.email);this.$el.html(this.render());this.onMount()},loadEmail:function(email){console.log(email);this.email=copyObject(email);if(email.meta.edited){this.edited=copyObject(email.meta.edited)}else{this.edited=copyObject(email)}},onMount:function(){let saveTimer;const handleContentOnChange=content=>{clearTimeout(saveTimer);saveTimer=setTimeout(()=>{window.console.log("save");this.updateEmailData({content:content})},300)};const mainContentMount=()=>{$("#commit").on("click",()=>{this.commitChanges()});if(this.edited.meta.type==="html"){const{editor:editor}=codeEditor({selector:"#content",onChange:handleContentOnChange,initialContent:this.edited.data.content});this.codemirror=editor}else{tinymceElement("content",{tinymce:true,quicktags:true},handleContentOnChange)}$("#subject, #preview-text").on("change input",e=>{this.updateEmailData({[e.target.name]:e.target.value})});const getHeadersArray=()=>{const{custom_headers:custom_headers={}}=this.edited.meta;const rows=[];Object.keys(custom_headers).forEach(key=>{rows.push([key,custom_headers[key]])});if(!rows.length){rows.push(["",""])}return rows};const headersEditor=inputRepeaterWidget({selector:"#email-editor-advanced-headers",rows:getHeadersArray(),cellProps:[{placeholder:"Header..."},{placeholder:"Value..."}],cellCallbacks:[input,inputWithReplacements],onChange:rows=>{const headers={};rows.forEach(([key,value])=>{headers[key]=value});this.updateEmailMeta({custom_headers:headers})}});headersEditor.mount()};const mountHeader=()=>{$("#email-editor-header").html(this.components.header.call(this));headerMount()};const headerMount=()=>{if(!this.isEditingTitle){$("#email-title").on("click",e=>{this.isEditingTitle=true;mountHeader()})}else{$("#email-title-edit").focus().on("change blur keydown",e=>{if(e.type==="keydown"&&e.key!=="Enter"){return}this.isEditingTitle=false;mountHeader()})}onHeaderMount()};const mountSidebar=()=>{$("#email-editor-sidebar").html(this.components.sidebar.call(this));sidebarMount()};const sidebarMount=()=>{$(".show-preview").on("click",e=>{const device=e.currentTarget.dataset.device;modal({content:`<iframe id="preview" class="${device}"></div>`});setFrameContent($("#preview")[0],this.edited.data.content)});$("#from-user").select2().on("change",e=>{this.updateEmailData({from_user:e.target.value})});$("#message-type").on("change",e=>{this.updateEmailMeta({message_type:e.target.value})});$("#reply-to").autocomplete({change:e=>{this.updateEmailMeta({reply_to_override:e.target.value})},source:["{owner_email}",...Groundhogg.filters.owners.map(u=>u.data.user_email)]});$(".change-alignment").on("click",e=>{this.updateEmailMeta({alignment:e.currentTarget.dataset.alignment});mountSidebar();$("#"+e.currentTarget.id).focus()});$("#send-test").on("click",e=>{if(!this.testEmailAddress){this.testEmailAddress=user_test_email}const modalContent=(isSending=false)=>{return`<h2>Send a test email to the following address...</h2>
			...
			</div>`};const{$modal:$modal,close:closeModal,setContent:setContent}=modal({content:modalContent()});$("#email-address").autocomplete({source:Groundhogg.filters.owners.map(u=>u.data.user_email),change:e=>{this.testEmailAddress=e.target.value}});$("#initiate-test").on("click",()=>{setContent(modalContent(true));const{stop:stopDots}=loadingDots("#initiate-test");post(`${routes.v4.emails}/${this.email.ID}/test`,{to:this.testEmailAddress,edited:this.edited}).then(r=>{stopDots();setContent(`<p>Test sent to <b>${this.testEmailAddress}</b></p>`);setTimeout(closeModal,2e3)})})})};mainContentMount();sidebarMount();headerMount()},demount:function(){this.onDemount()},onDemount:function(){if(this.edited.meta.type==="html"){this.codemirror.toTextArea()}else{wp.editor.remove("content")}},currentState:function(){const{email:email}=this;return{email:copyObject(email)}},saveUndoState:function(){this.undoStates.push(this.currentState())},undo:function(){var lastState=this.undoStates.pop();if(!lastState){return}this.redoStates.push(this.currentState());Object.assign(this,lastState);this.render()},redo:function(){var lastState=this.redoStates.pop();if(!lastState){return}this.undoStates.push(this.currentState());Object.assign(this,lastState);this.render()}});Groundhogg.EmailEditor=EmailEditor})(jQuery);