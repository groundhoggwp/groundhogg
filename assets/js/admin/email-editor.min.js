(function($){const{copyObject,select,input,tinymceElement,specialChars,breadcrumbs,improveTinyMCE,inputWithReplacementsAndEmojis,inputWithReplacements,inputRepeaterWidget,textarea,isValidEmail,adminPageURL,modal,tooltip,dialog,loadingDots,moreMenu,objectEquals,toggle,icons,codeEditor,confirmationModal,savingModal,dangerConfirmationModal}=Groundhogg.element;const{post,get,patch,delete:apiDelete,routes}=Groundhogg.api;const{user_test_email}=Groundhogg;const{emails:EmailsStore,campaigns:CampaignsStore}=Groundhogg.stores;const{campaignPicker}=Groundhogg.pickers;const{__,_x,_n,_nx,sprintf}=wp.i18n;const setFrameContent=(frame,content)=>{var blob=new Blob([content],{type:"text/html; charset=utf-8"});frame.src=URL.createObjectURL(blob)};const EmailEditor=({selector,email,onChange:onChange=email=>{},onCommit:onCommit=email=>{},breadcrumbs:crumbs=["Emails"],onHeaderMount:onHeaderMount=()=>{},afterPublishActions:afterPublishActions=""})=>({selector:selector,email:copyObject(email),origEmail:copyObject(email),$el:$(selector),undoStates:[],redoStates:[],edited:{data:{},meta:{}},components:{editor(){return`
            <div id="email-editor">
