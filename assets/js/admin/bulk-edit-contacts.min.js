($=>{const{propertiesEditor}=Groundhogg;const{createFilters}=Groundhogg.filters.functions;const{input,progressModal,select,confirmationModal,dialog,bold,loadingModal,inputRepeaterWidget}=Groundhogg.element;const{betterTagPicker}=Groundhogg.components;const{contacts:ContactsStore}=Groundhogg.stores;const{formatNumber}=Groundhogg.formatting;const{sprintf,__,_x,_n}=wp.i18n;const fieldSection=({title:title="",fields:fields=""})=>{return`
        <div class="gh-panel">
        ...
        </div>`};const{query}=BulkEdit;let totalContacts=0;let data={},meta={};let payload={add_tags:[],remove_tags:[]};const updateData=_data=>{data={...data,..._data}};const updateMeta=_meta=>{meta={...meta,..._meta}};const deleteMeta=key=>{delete meta[key]};const setInPayload=_p=>{payload={...payload,..._p}};const fetchContactCount=()=>{return ContactsStore.count({...query}).then(t=>{totalContacts=t})};const setCommitText=()=>{$("#commit").text(sprintf(_n("Edit %s contact","Edit %s contacts",totalContacts,"groundhogg"),formatNumber(totalContacts)))};const mount=()=>{$("#bulk-edit").html(template());createFilters("#filters",query.filters,filters=>{query.filters=filters;fetchContactCount().then(setCommitText)}).init();createFilters("#exclude-filters",query.exclude_filters,filters=>{query.exclude_filters=filters;fetchContactCount().then(setCommitText)}).init();if(query.include){$(".include-filters-wrap, .exclude-filters-wrap").addClass("hidden")}sections.forEach(s=>s.onMount({updateData:updateData,updateMeta:updateMeta,deleteMeta:deleteMeta,setInPayload:setInPayload}));$(".toggle-indicator").on("click",e=>{$(e.target).closest(".gh-panel").toggleClass("closed")});$("#commit").on("click",()=>{confirmationModal({alert:`<p>${sprintf(__("Are you sure you want to edit %s contacts? This action cannot be undone.","groundhogg"),bold(formatNumber(totalContacts)))}</p>`,onConfirm:()=>{progressModal({beforeProgress:()=>{return`<h2>${__("Updating Contacts...","groundhogg")}</h2><p class="gh-text danger">${__("Do not close this window!")}</p>`},onOpen:({setProgress,close})=>{const limit=500;let offset=Math.max(totalContacts-limit,0);const patchContacts=()=>{ContactsStore.patchMany({total_only:true,query:{...query,offset:offset,limit:limit},data:data,meta:meta,...payload}).then(r=>{setProgress((totalContacts-offset)/totalContacts*100);if(offset>0){offset-=limit;if(offset<0){offset=0}patchContacts();return}dialog({message:__("Contacts updated!","groundhogg")});let url=new URL(window.location.href);url.searchParams.delete("action");url.searchParams.delete("number");url.searchParams.delete("offset");url.searchParams.delete("search");window.open(url,"_self")}).catch(err=>{dialog({message:err.message,type:"error"})})};patchContacts()}})}})})};$(()=>{const{close}=loadingModal();fetchContactCount().then(()=>{mount();setCommitText();close()})})})(jQuery);