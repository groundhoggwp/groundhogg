($=>{const{propertiesEditor}=Groundhogg;const{createFilters}=Groundhogg.filters.functions;const{input,progressModal,select,confirmationModal,dialog,bold,adminPageURL,loadingModal,inputRepeaterWidget}=Groundhogg.element;const{betterTagPicker}=Groundhogg.components;const{post,get,patch,routes,ajax}=Groundhogg.api;const{searches:SearchesStore,contacts:ContactsStore,tags:TagsStore,funnels:FunnelsStore}=Groundhogg.stores;const{tagPicker,funnelPicker}=Groundhogg.pickers;const{userHasCap}=Groundhogg.user;const{formatNumber,formatTime,formatDate,formatDateTime}=Groundhogg.formatting;const{sprintf,__,_x,_n}=wp.i18n;const fieldSection=({title:title="",fields:fields=""})=>{return`
        <div class="gh-panel">
