(function($,Templates){const{select:select,regexp:regexp,modal:modal,input:input,primaryButton:primaryButton,loadingDots:loadingDots}=Groundhogg.element;const{templates:templates}=Templates;const{post:post,get:get,routes:routes}=Groundhogg.api;const setFrameContent=(frame,content)=>{var blob=new Blob([content],{type:"text/html; charset=utf-8"});frame.src=URL.createObjectURL(blob)};const rowsOf=(size,items)=>{return items.reduce((rows,item)=>{if(rows[rows.length-1].length===size){rows.push([])}rows[rows.length-1].push(item);return rows},[[]])};const createEmail=template=>{return post(`${routes.v4.emails}`,{...template,ID:false}).then(r=>r.item)};const fillRow=(size,fill,row)=>{while(row.length<size){row.push(fill)}return row};const Add={search:"",selectedTemplate:"",renderTemplate:function(template){const{ID:ID,data:data,meta:meta,steps:steps}=template;return`
		  ...
		  <div>
			...
			</div>`};const{close:close,setContent:setContent}=modal({content:modalContent()});const handleCreate=()=>{setContent(modalContent(true));loadingDots("#create");createEmail({...this.selectedTemplate,data:{...this.selectedTemplate.data,title:this.newTitle}}).then(email=>{window.location.href=email.admin})};$("#create").on("click",handleCreate);$("#title-input").focus().on("change input keydown",e=>{this.newTitle=e.target.value;if(this.newTitle){$("#create").prop("disabled",false)}else{$("#create").prop("disabled",true)}if(e.type==="keydown"&&e.key==="Enter"&&this.newTitle){handleCreate()}})},mountTemplates:function(){$("#view").html(this.renderTemplates());$("button.select-template").on("click",e=>{const templateId=e.target.dataset.template;const template=templates.find(t=>t.ID==templateId);this.selectedTemplate=template;this.newTitle=this.selectedTemplate.data.title;this.titleModal()});$("iframe.template-frame").each(function(){const template=this.dataset.template;setFrameContent(this,templates.find(t=>t.ID===template).context.built)})},renderImport:function(){if(this.importTemplate){return`
			<div id="import">
		  ...
		  </div>`},mountImport:function(){$("#view").html(this.renderImport());if(this.importTemplate){setFrameContent($("#template-preview")[0],this.importTemplate.data.content);$("#create-from-import").on("click",e=>{this.selectedTemplate=this.importTemplate;this.newTitle=this.selectedTemplate.data.title||"";this.titleModal()})}const onReaderLoadHTML=e=>{const content=e.target.result;if(!content){this.importError="The provided file is not a valid funnel.";this.mountImport();this.importError="";return}this.importTemplate={data:{content:content},meta:{type:"html"}};this.mountImport()};const onReaderLoadJSON=e=>{const template=JSON.parse(e.target.result);if(!template){this.importError="The provided file is not a valid funnel.";this.mountImport();this.importError=""}else{this.importError="";this.importTemplate=template;this.mountImport()}};$("a.cancel").on("click",()=>{this.reset();this.mountTemplates()});$("#import-file").on("change",e=>{const files=e.target.files;const reader=new FileReader;const file=files[0];const ext=file.name.split(".").pop().toLowerCase();switch(ext){case"html":reader.onload=onReaderLoadHTML;reader.readAsText(file);return;case"json":reader.onload=onReaderLoadJSON;reader.readAsText(file);return}})},reset:function(){this.selectedTemplate=false;this.importTemplate=false;this.newTitle=""},mount:function(){$("#add-email").html(this.render());this.mountTemplates();$("#search").on("input change",e=>{this.search=e.target.value;this.reset();this.mountTemplates()});$("#import-button").on("click",e=>{this.reset();this.mountImport()});$("#scratch-button").on("click",e=>{this.reset();this.selectedTemplate={data:{},meta:{}};this.titleModal()})}};$(()=>{Add.mount()})})(jQuery,AddEmail);