($=>{const{moreMenu,loadingModal,icons,select,modal,input,uuid,dialog,dangerConfirmationModal,adminPageURL}=Groundhogg.element;const{routes,post,get}=Groundhogg.api;const{options:OptionsStore}=Groundhogg.stores;const{createFilters}=Groundhogg.filters.functions;const{metaPicker}=Groundhogg.pickers;const{__}=wp.i18n;function adjust(color,amount){return"#"+color.replace(/^#/,"").replace(/../g,color=>("0"+Math.min(255,Math.max(0,parseInt(color,16)+amount)).toString(16)).substr(-2))}const{base64_json_encode}=Groundhogg.functions;const ReportTypes={pie_chart:{name:__("Pie Chart","groundhogg"),settings:({field:field=""})=>{return`
        ...
        `},onMount:({id,field,data,filters:filters=[],exclude_filters:exclude_filters=[]})=>{let cuttoff=11;let ctx=$(`.pie-chart[data-id=${id}]`)[0].getContext("2d");let _data=data;if(_data.length>cuttoff){_data=data.slice(0,cuttoff);let _rest=data.slice(cuttoff).reduce((carr,i)=>{return carr+parseInt(i.count)},0);_data.push({count:_rest,value:__("Other")})}const openInContactsView=(e,arr)=>{if(arr.length&&arr[0]._view){window.open(adminPageURL("gh_contacts",{filters:base64_json_encode([[{type:"sub_query",include_filters:filters,exclude_filters:exclude_filters},{type:"meta",meta:field,value:arr[0]._view.label,compare:"equals"}]])}),"_blank")}};let chart=new Chart(ctx,{type:"doughnut",data:{datasets:[{data:_data.map(({count})=>count),backgroundColor:_data.map((d,i)=>adjust("#4fa4ff",-(i*30)))}],labels:_data.map(({value})=>value)},options:{onClick:(e,arr)=>{if(arr.length&&arr[0]._index===cuttoff&&arr[0]._view.label===__("Other")){let _data=data.slice(cuttoff);modal({content:`<canvas class="pie-chart-large" style="height: 600px" data-id="${id}"></canvas>`,onOpen:()=>{let ctx=$(`.pie-chart-large[data-id=${id}]`)[0].getContext("2d");let chart=new Chart(ctx,{type:"doughnut",data:{datasets:[{data:_data.map(({count})=>count),backgroundColor:_data.map((d,i)=>adjust("#4fa4ff",-(i*30)))}],labels:_data.map(({value})=>value)},options:{maintainAspectRatio:false,aspectRatio:1,onClick:openInContactsView,legend:{position:"bottom"}}})}});return}openInContactsView(e,arr)},legend:{position:"right"}}})}},table:{name:__("Table","groundhogg"),settings:({field:field=""})=>{return`
			<div class="row">
				<div class="col">
					<label for="value">${__("Custom Field")}</label>
					${input({id:"field",value:field})}
            ...
            </div>`:""}`},onMount:({id,data,field,filters:filters=[],exclude_filters:exclude_filters=[]})=>{let num=10;const setData=()=>{$(`#${id} tbody`).html(data.slice(0,num).map(row=>dataRow(row)).join(""));$(`#${id} .gh-input-group`).html([10,25,50].filter(i=>i<data.length).map(_num=>`<button class="gh-button ${num===_num?"primary":"secondary"} num-records" data-num="${_num}">${_num}</button>`));$(`#${id} .num-records`).on("click",e=>{num=parseInt(e.target.dataset.num);setData()});$(`.number-total[data-id=${id}]`).on("click",e=>{window.open(adminPageURL("gh_contacts",{filters:base64_json_encode([[{type:"sub_query",include_filters:filters,exclude_filters:exclude_filters},{type:"meta",meta:field,value:e.target.dataset.value,compare:"equals"}]])}),"_blank")})};const dataRow=({value,count})=>{return`
			  <tr>
				  <td>${value}</td>
			...
			<div class="row">
				<div class="col display-flex column">
					...
					${select({id:"value"},{contacts:__("Total number of contacts","groundhogg"),sum:__("Sum of a custom field","groundhogg"),average:__("Average of a custom field","groundhogg"),distinct:__("Number of distinct custom field values","groundhogg"),activity:__("Total number of activities")},value)}
				</div>
			</div>
			...
			</div>`},onMount:({id,filters:filters=[],exclude_filters:exclude_filters=[]})=>{$(`.big-number[data-id=${id}]`).on("click",e=>{window.open(adminPageURL("gh_contacts",{filters:base64_json_encode(filters),exclude_filters:base64_json_encode(exclude_filters)}),"_blank")})}}};const renderReport=report=>{if(report.type==="header"){return`
		...
		</div>`};let commitTimeout;$("#custom-reports").sortable({handle:"h2",tolerance:"pointer",placeholder:"report-placeholder",start:(e,ui)=>{ui.placeholder.addClass(reports.find(r=>r.id===ui.item.data("id")).type);ui.placeholder.height(ui.item.height())},update:()=>{$(".report").each((i,el)=>{let $report=$(el);reports.find(r=>r.id==$report.data("id")).order=$report.index()});if(commitTimeout){clearTimeout(commitTimeout)}commitTimeout=setTimeout(()=>{commitReports()})}});const commitReports=()=>{return OptionsStore.patch({gh_custom_reports:reports.map(({data,...report})=>report)})};const scrollTo=id=>{let el=document.getElementById(id);setTimeout(()=>{el.scrollIntoView(true)},500)};const editReport=report=>{modal({width:600,content:renderReportEdit(report),dialogClasses:"overflow-auto",onOpen:({close,setContent})=>{const updateReport=(props,reload=false)=>{report={...report,...props};if(reload){setContent(renderReportEdit(report));onMount()}};const onMount=()=>{ReportTypes[report.type].settingsOnMount(report,updateReport);$("#name").on("change input",e=>{updateReport({name:e.target.value})});$("#report-type").on("change",e=>{updateReport({type:e.target.value},true);$("#report-type").focus()});createFilters("#filters-here",report.filters,filters=>{updateReport({filters:filters})}).init();createFilters("#exclude-filters-here",report.exclude_filters,exclude_filters=>{updateReport({exclude_filters:exclude_filters})}).init();$("#save").on("click",()=>{if(reports.find(r=>r.id===report.id)){reports=reports.map(r=>r.id===report.id?report:r)}else{reports.push(report)}commitReports().then(()=>{dialog({message:__("Reports saved!","groundhogg")})}).then(()=>{close();loadReport(report.id).then(()=>{scrollTo(report.id)})})})};onMount()}})};const editHeader=report=>{modal({width:400,content:`
		  <h2>${__("Edit Header")}</h2>
		  <div class="gh-input-group">
			  ${input({name:"name",id:"name",value:report.name})}
			  <button class="gh-button primary" id="save">${__("Save")}</button>
		  </div>`,onOpen:({close,setContent})=>{const updateReport=props=>{report={...report,...props}};const onMount=()=>{$("#name").on("change input",e=>{updateReport({name:e.target.value})});$("#save").on("click",()=>{if(reports.find(r=>r.id===report.id)){reports=reports.map(r=>r.id===report.id?report:r)}else{reports.push(report)}commitReports().then(()=>{dialog({message:__("Reports saved!","groundhogg")})}).then(()=>{mount();close();scrollTo(report.id)})})};onMount()}})};const onMount=()=>{reports.forEach(report=>{if(report.type==="header"){return}try{ReportTypes[report.type].onMount(report)}catch(e){}});$(".report-more").on("click",e=>{let reportId=e.currentTarget.dataset.id;let report=reports.find(r=>r.id===reportId);moreMenu(e.currentTarget,{items:[{key:"edit",text:__("Edit")},{key:"delete",text:`<span class="gh-text danger">${__("Delete")}</span>`}],onSelect:k=>{switch(k){case"edit":if(report.type==="header"){editHeader(report);break}editReport(report);break;case"delete":dangerConfirmationModal({alert:`<p>${__("Are you sure you want to delete this report?","groundhogg")}</p>`,confirmText:__("Delete"),onConfirm:()=>{reports=reports.filter(r=>r.id!==report.id);commitReports().then(()=>{dialog({message:__("Report deleted.","groundhogg")});mount()})}});break}}})})};$(".nav-tab-wrapper").append(`
    ...
    `);$("#add-report").on("click",e=>{let newReport={type:"number",name:__("New Report"),id:uuid(),filters:[],exclude_filters:[],order:reports.length};editReport(newReport)});$("#add-header").on("click",e=>{let newReport={type:"header",name:__("New Header"),id:uuid(),order:reports.length};editHeader(newReport)});const{close}=loadingModal();$(()=>{loadReports().then(()=>{$(".wrap").removeClass("blurred");close()})})})(jQuery);