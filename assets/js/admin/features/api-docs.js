( ($) => {
  ...
  const ParamsList = (params, parentParam = '') => Fragment(params.map(({ param, description, type, required, ...props }) => {
    ...
    return Div({
      ...
      Div({ className: 'display-flex gap-10 align-center' }, [
        ...
        required ? `<span class="required">${ __('Required', 'groundhogg') }</span>` : null,
      ]),
      description,
  ...
  ApiRegistry.contacts.endpoints.add('update', {
    ...
    params: [
      ...
      ...( params => {
        params[0] = copyObject(params[0])
        params[0].required = false
        params[0].subParams[0].required = false
        return params
  ...
  ApiRegistry.contacts.endpoints.add('update-single', {
    ...
    params: [
      ( param => {
        param.required = false
        param.subParams[0].required = false
        return param
      } )(copyObject(ApiRegistry.contacts.endpoints['create-single'].params[0])),
