(function($){const classPrefix="groundhogg-toolbar-quick-search";const{tooltip,icons}=Groundhogg.element;const{quickAddForm}=Groundhogg.components;const{userHasCap}=Groundhogg.user;const{isWhiteLabeled}=Groundhogg;const Tabs={search_contacts:{tooltip:`Search for contacts`,cap:"view_contacts",svg:icons.contactSearch,view:()=>{return`
            <div id="quick-search">
            ...
            ${renderTabs()}
            ${userHasCap(Tabs[tab].cap)?Tabs[tab].view():""}`;$quickSearch.html(html);const setTab=t=>{tab=t;mountQuickSearch()};$(`.${classPrefix}-tab-button`).on("click",({currentTarget})=>{setTab(currentTarget.dataset.tab)});Tabs[tab].onMount({setTab:setTab,reMount:mountQuickSearch});Object.keys(Tabs).forEach(t=>tooltip(`#gh-tab-${t}`,{content:Tabs[t].tooltip,position:"bottom"}));$quickSearch.css({top:Math.min(bottom,window.innerHeight-$quickSearch.height()-20),left:right-$quickSearch.outerWidth()});$(`.${classPrefix}-close`).on("click",()=>{close()})};const $quickSearch=$(renderQuickSearch());$("body").append($quickSearch).addClass("groundhogg-toolbar-quick-search-open");$quickSearch.on("keydown",({key})=>{switch(key){case"Esc":case"Escape":close();break}});mountQuickSearch()})})})(jQuery);