(function($){const{tags:TagsStore,emails:EmailsStore,funnels:FunnelsStore}=Groundhogg.stores;const{get:apiGet,post:apiPost,delete:apiDelete,patch:apiPatch,routes}=Groundhogg.api;const{v4:apiRoutes}=routes;const{tinymceElement,addMediaToBasicTinyMCE,mappableFields,dialog,bold,uuid,orList,andList,specialChars,select,input,textarea,icons,toggle,textAreaWithReplacements,setFrameContent,inputWithReplacements,inputWithReplacementsAndEmojis,ordinal_suffix_of,copyObject}=Groundhogg.element;const{sprintf,__,_x,_n}=wp.i18n;const{formBuilder,rawStepTypes}=Groundhogg;const{linkPicker,emailPicker,tagPicker,apiPicker}=Groundhogg.pickers;$.fn.serializeFormJSON=function(){var o={};var a=this.serializeArray();$.each(a,function(){if(o[this.name]){if(!o[this.name].push){o[this.name]=[o[this.name]]}o[this.name].push(this.value||"")}else{o[this.name]=this.value||""}});return o};const StepPacks={packs:{},add(id,name="",svg=""){this.packs[id]={id:id,name:name,svg:svg}},get(id){return this.packs[id]}};StepPacks.add("core",__("Groundhogg","groundhogg"));const tagWithConditionOnMount=(step,updateStepMeta)=>{tagOnMount(step,updateStepMeta);$("#condition").change(function(e){updateStepMeta({condition:$(this).val()})})};const tagOnMount=(step,updateStepMeta)=>{return tagPicker("#tags",true,items=>{TagsStore.itemsFetched(items)},{width:"100%",placeholder:__("Select one or more tags...","groundhogg")}).on("change",function(e){const tags=$(this).val();const newTags=tags.filter(tag=>!TagsStore.hasItem(parseInt(tag)));if(newTags.length>0){TagsStore.validate(tags).then(tags=>{updateStepMeta({tags:tags.map(tag=>tag.ID)})})}else{updateStepMeta({tags:tags.map(tag=>parseInt(tag))})}})};const fieldMappingTable=({fields:fields={},fieldMap:fieldMap={}})=>{const mappable=fields.map(({id,label},i)=>{return`<tr>
        ...
        <table class="mapping-table">
        ...
        </table>`};const fieldMappingTableOnMount=(updateStepMeta,getCurrentState)=>{$(".mappable-field").select2().on("change",function(e){const{meta}=getCurrentState();updateStepMeta({field_map:{...meta.field_map,[$(this).data("key")]:$(this).val()}})})};const formPicker=(selector,type,onReceiveItems=i=>{})=>{return apiPicker(selector,`${apiRoutes.funnels}/form-integration`,false,false,d=>{onReceiveItems(d.forms);return d.forms.map(form=>({id:form.id,text:form.name}))},q=>({...q,type:type}))};const formsCache={_cache:{},set(type,forms){this._cache[type]=forms},getAll(type){return this._cache[type]},hasType(type){return typeof this._cache[type]!=="undefined"},get(type,id){return this.hasType(type)?this._cache[type].find(f=>f.id===id):false},fetch(type){return apiGet(`${apiRoutes.funnels}/form-integration`,{type:type}).then(d=>this.set(type,d.forms))}};const FormIntegration=({type,...rest})=>({type:type,defaults:{form_id:0,field_map:{}},title({meta}){const{form_id}=meta;const form=formsCache.get(type,form_id);if(form){return`Submits <b>${form.name}</b>`}else{return`Submits <b></b>`}},edit({meta}){const{form_id,field_map}=meta;const form=formsCache.get(type,form_id);return`
          <div class="panel">
              <div class="row">
		  ...
		  <svg viewBox="0 0 20 15" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            </div>`}},default:{pack:"core",promiseController:null,title({ID,data,meta}){return data.step_title},edit({ID,data,meta}){return`
            <div class="panel">
          ...
          <svg viewBox="0 0 42 37" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <path
                      ...
                      fill="currentColor"/>
          </svg>`,title({meta}){const{note_text}=meta;if(note_text){return sprintf(_x("Add %s","add - note text","groundhogg"),`<i>${note_text.replace(/(<([^>]+)>)/gi,"").substring(0,30)}...</i>`)}return __("Add a note","groundhogg")},edit({meta}){const{note_text:note_text=""}=meta;return`
            <div class="panel">
                <div class="row">
          ...
          </svg>`,defaults:{to:"{owner_email}",from:"{owner_email}",reply_to:"{email}",subject:'Notification from "{first}"',note_text:""},title({meta}){const{to}=meta;return`Send notification to ${andList(to.split(",").map(address=>`<b>${address.trim()}</b>`))}`},edit({meta}){const{note_text:note_text="",to:to="",from:from="",reply_to:reply_to="",subject:subject=""}=meta;return`
            <div class="panel">
      ...
      `,title({ID,data,meta}){const roles=Groundhogg.filters.roles;if(meta.role&&meta.role.length===1){return sprintf(__("%s is created","groundhogg"),`<b>${roles[meta.role[0]]?.name}</b>`)}else if(meta.role&&meta.role.length>1){return sprintf(__("%s is created","groundhogg"),orList(meta.role.map(role=>`<b>${roles[role]?.name}</b>`)))}else{return __("A <b>new user</b> is created","groundhogg")}},edit({ID,data,meta}){let roles=Groundhogg.filters.roles;return`
          ...
          </svg>`,title({ID,data,meta}){let{tags}=meta;if(tags){tags=tags.map(id=>parseInt(id))}if(!tags||tags.length===0){return __("Apply a tag","groundhogg")}else if(tags.length<4&&TagsStore.hasItems(tags)){return sprintf(_x("Apply %s","apply tags - list of tag names","groundhogg"),andList(tags.map(id=>`<b>${TagsStore.get(id).data.tag_name}</b>`)))}else{return sprintf(_x("Apply %s tags","apply tags - number","groundhogg"),`<b>${tags.length}</b>`)}},edit({ID,data,meta}){let options=TagsStore.getItems().map(tag=>{return{text:tag.data.tag_name,value:tag.ID}});return`
            <div class="panel">
                <div class="row">
          ...
          </svg>`,title({ID,data,meta}){let{tags}=meta;tags=tags.map(id=>parseInt(id));if(tags.length===0){return __("Remove tags","groundhogg")}else if(tags.length<4&&TagsStore.hasItems(tags)){return sprintf(_x("Remove %s","remove - list of tag names","groundhogg"),andList(tags.map(id=>`<b>${TagsStore.get(id).data.tag_name}</b>`)))}else{return sprintf(_x("Remove %s tags","remove tags - number","groundhogg"),`<b>${tags.length}</b>`)}},edit({ID,data,meta}){let options=TagsStore.getItems().map(tag=>{return{text:tag.data.tag_name,value:tag.ID}});return`
            <div class="panel">
                <div class="row">
          ...
          </svg>`,title({ID,data,meta}){let{tags:tags=[],condition:condition="any"}=meta;tags=tags.map(id=>parseInt(id));if(tags.length>=4){return condition==="all"?sprintf(__("%s tags are applied","groundhogg"),`<b>${tags.length}</b>`):sprintf(__("Any of %s tags are applied","groundhogg"),`<b>${tags.length}</b>`)}else if(tags.length>1&&tags.length<4&&TagsStore.hasItems(tags)){const tagNames=tags.map(tag=>`<b>${TagsStore.get(tag).data.tag_name}</b>`);return condition==="all"?sprintf(_x("%s are applied","list of tags - are applied","groundhogg"),andList(tagNames)):sprintf(_x("%s is applied","list of tags - is applied","groundhogg"),orList(tagNames))}else if(tags.length===1){return sprintf(_x("%s is applied","list of tags - is applied","groundhogg"),`<b>${TagsStore.get(tags[0]).data.tag_name}</b>`)}else{return __("A tag is applied","groundhogg")}},edit({ID,data,meta}){let options=TagsStore.getItems().map(tag=>{return{text:tag.data.tag_name,value:tag.ID}});const{tags:tags=[],condition:condition="any"}=meta;return`
            <div class="panel">
                <div class="row">
          ...
          <svg viewBox="0 0 37 35" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <path
                      ...
                      fill="currentColor"/>
          </svg>`,title({ID,data,meta}){let{tags:tags=[],condition:condition="any"}=meta;tags=tags.map(id=>parseInt(id));if(tags.length>=4){return condition==="all"?sprintf(__("%s tags are removed","groundhogg"),`<b>${tags.length}</b>`):sprintf(__("Any of %s tags are removed","groundhogg"),`<b>${tags.length}</b>`)}else if(tags.length>1&&tags.length<4&&TagsStore.hasItems(tags)){const tagNames=tags.map(tag=>`<b>${TagsStore.get(tag).data.tag_name}</b>`);return condition==="all"?sprintf(_x("%s are removed","list of tags - are removed","groundhogg"),andList(tagNames)):sprintf(_x("%s is removed","list of tags - is removed","groundhogg"),orList(tagNames))}else if(tags.length===1){return sprintf(_x("%s is removed","list of tags - is removed","groundhogg"),`<b>${TagsStore.get(tags[0]).data.tag_name}</b>`)}else{return __("A tag is removed","groundhogg")}},edit({ID,data,meta}){const{condition:condition="any",tags:tags=[]}=meta;return`
            <div class="panel">
                <div class="row">
                    <label class="row-label" for="tags">
      ...
      `,title({meta}){return delayTimerName({...delayTimerDefaults,...meta})},edit({ID,data,meta}){const{delay_amount,delay_type,run_on_type,run_when,run_time,send_in_timezone,run_time_to,run_on_dow_type,run_on_dow,run_on_month_type,run_on_months,run_on_dom}={...delayTimerDefaults,...meta};const runOnTypes={any:"Any day",weekday:"Weekday",weekend:"Weekend",day_of_week:"Day of week",day_of_month:"Day of month"};const runWhenTypes={now:__("Any time","groundhogg"),later:__("Specific time","groundhogg")};if(["minutes","hours","none"].includes(delay_type)){runWhenTypes.between=__("Between","groundhogg")}const runOnDaysOfMonth={};for(let i=1;i<32;i++){runOnDaysOfMonth[i]=i}runOnDaysOfMonth.last="last";const runOnMonthTypes={any:"Of any month",specific:"Of specific month(s)"};const runOnMonthOptions=`
            ...
            <div class="panel">
                ...
                <div class="row">
                    <p>${__("Run in the contact's timezone?","groundhogg")}
                        ${toggle({id:"send_in_timezone",name:"send_in_timezone",checked:Boolean(send_in_timezone)})}</p>
                </div>
            </div>`},onMount({meta},updateStepMeta,updateStepData,getCurrentState){const updatePreview=()=>{$(".delay-preview").html(delayTimerName({...delayTimerDefaults,...getCurrentState().meta}))};$("#send_in_timezone").on("change",e=>{updateStepMeta({send_in_timezone:e.target.checked})});$(".select2").select2({width:"auto"}).on("change",function(e){updateStepMeta({[$(this).attr("name")]:$(this).val()});updatePreview()});$(".delay-input").on("change",({target})=>{const reRender=target.classList.contains("re-render");updateStepMeta({[target.name]:$(target).val()},reRender);if(reRender){$(`[name=${target.name}]`).focus()}else{updatePreview()}}).on("input",function(e){updatePreview()})}},send_email:{pack:"core",defaults:{email_id:null},svg:`
          <svg viewBox="0 0 35 35" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <path d="M3.674 8.903l14.166 8.5 14.167-8.5M20.674 24.487h11.333m0 0l-4.25-4.25m4.25 4.25l-4.25 4.25"
                    stroke="currentColor" stroke-width="2"/>
          </svg>`,title({ID,data,meta}){const{email_id}=meta;const email=EmailsStore.get(parseInt(email_id));return email_id&&email?sprintf(_x("Send %s","send - email title","groundhogg"),`<b>${email.data.title}</b>`):__("Send <b>an email</b>","groundhogg")},edit({ID,data,meta}){const{email_id}=meta;const email=EmailsStore.get(parseInt(email_id));const iframePreview=email=>{const{context}=email;return`
              <div class="panel">
                  <div class="row">
          ...
          <svg viewBox="0 0 35 35" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            </div>`},onMount({meta},updateStepMeta){$(".copy-value").on("click",e=>{let input=$(e.currentTarget).siblings("input")[0];input.focus();input.select();document.execCommand("copy");dialog({message:__("Copied!","groundhogg")})});if(!meta.uuid){updateStepMeta({uuid:uuid()},true)}linkPicker("#redirect-to").on("change",function(e){updateStepMeta({redirect_to:$(this).val()})})},onDuplicate:step=>{step.meta.uuid=uuid()}},email_confirmed:{pack:"core",svg:`
          <svg viewBox="0 0 35 35" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          </svg>`,title({meta}){const{form_name}=meta;if(!form_name){return __("Submits <b>a form</b>","groundhogg")}return sprintf(__("Submits %s","groundhogg"),`<b>${form_name}</b>`)},edit({meta}){const redirectToURL=`<label class="row-label">${__("Redirect to this URL...","groundhogg")}</label>
        ...
        ${textAreaWithReplacements({id:"success-message",name:"success_message",className:"regular-text",value:meta.success_message||""})}`;return`
          ...
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 374.8 374.8">
          ...
          </svg>`,defaults:{method:"beginning"},title:({meta,data,ID})=>{const{method:method="beginning",step_id:step_id=false}=meta;const{funnel_id}=data;switch(method){default:case"beginning":return __("Restart <b>from the beginning</b>","groundhogg");case"selected":if(!step_id){return __("Restart from a specific step","groundhogg")}return sprintf(__('Restart from "%s"',"groundhogg"),StepTypes.getTitle(FunnelsStore.getSteps(funnel_id,true).find(s=>s.ID==step_id)))}},edit:({meta})=>{const{method:method="beginning",step_id:step_id=false}=meta;return`
            <div class="panel">
                <div class="row">
            ...
            </div>`},onMount:({data,meta},updateStepMeta)=>{$("#method").on("change",e=>{updateStepMeta({method:e.target.value},true)});stepTypeSelect("#step_id",{funnel_id:parseInt(data.funnel_id),selected:parseInt(meta.step_id),group:"action"}).on("select2:select",({params})=>{updateStepMeta({step_id:parseInt(params.data.id)})})}}};const stepTypeSelect=(el,{funnel_id:funnel_id=null,edited:edited=false,selected:selected=null,group:group="any"})=>{const select2Template=step=>{if(!step.id){return step.text}const{step_type,step_group}=step.data;const StepType=StepTypes.getType(step.data.step_type);return $(`
