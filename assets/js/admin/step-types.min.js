      ...
      `,title:function({ID:ID,data:data,meta:meta}){const roles=this.context.roles;if(meta.role&&meta.role.length===1){return sprintf(__("%s is created","groundhogg"),`<b>${roles[meta.role[0]]}</b>`)}else if(meta.role&&meta.role.length>1){return sprintf(__("%s is created","groundhogg"),orList(meta.role.map(role=>`<b>${roles[role]}</b>`)))}else{return __("A <b>new user</b> is created","groundhogg")}},edit:function({ID:ID,data:data,meta:meta}){let roles=this.context.roles;return`
		  ...
		  <svg viewBox="0 0 37 35" fill="none" xmlns="http://www.w3.org/2000/svg">
			  ...
			  <path
				  ...
				  fill="currentColor"/>
		  </svg>`,title:function({ID:ID,data:data,meta:meta}){let{tags:tags,condition:condition}=meta;tags=tags.map(id=>parseInt(id));if(tags.length>=4){return condition==="all"?sprintf(__("%s tags are removed","groundhogg"),`<b>${tags.length}</b>`):sprintf(__("Any of %s tags are removed","groundhogg"),`<b>${tags.length}</b>`)}else if(tags.length>1&&tags.length<4&&TagsStore.hasItems(tags)){const tagNames=tags.map(tag=>`<b>${TagsStore.get(tag).data.tag_name}</b>`);return condition==="all"?sprintf(_x("%s are removed","list of tags - are removed","groundhogg"),andList(tagNames)):sprintf(_x("%s is removed","list of tags - is removed","groundhogg"),orList(tagNames))}else if(tags.length===1){return sprintf(_x("%s is removed","list of tags - is removed","groundhogg"),`<b>${TagsStore.get(tags[0]).data.tag_name}</b>`)}else{return __("A tag is removed","groundhogg")}},edit:function({ID:ID,data:data,meta:meta}){let options=TagsStore.getItems().map(tag=>{return{text:tag.data.tag_name,value:tag.ID}});const{condition:condition}=meta;return`
			<div class="panel">
				<div class="row">
					<label class="row-label" for="tags">
