(function($){const{tags:TagsStore,emails:EmailsStore,funnels:FunnelsStore}=Groundhogg.stores;const{get:apiGet,post:apiPost,delete:apiDelete,patch:apiPatch,routes:routes}=Groundhogg.api;const{v4:apiRoutes}=routes;const{tinymceElement:tinymceElement,mappableFields:mappableFields,bold:bold,uuid:uuid,orList:orList,andList:andList,specialChars:specialChars,select:select,input:input,textarea:textarea,toggle:toggle,textAreaWithReplacementsAndEmojis:textAreaWithReplacementsAndEmojis,setFrameContent:setFrameContent,inputWithReplacements:inputWithReplacements,inputWithReplacementsAndEmojis:inputWithReplacementsAndEmojis,ordinal_suffix_of:ordinal_suffix_of,copyObject:copyObject}=Groundhogg.element;const{sprintf:sprintf,__:__,_x:_x,_n:_n}=wp.i18n;const{formBuilder:formBuilder,rawStepTypes:rawStepTypes}=Groundhogg;const{linkPicker:linkPicker,emailPicker:emailPicker,tagPicker:tagPicker,apiPicker:apiPicker}=Groundhogg.pickers;$.fn.serializeFormJSON=function(){var o={};var a=this.serializeArray();$.each(a,function(){if(o[this.name]){if(!o[this.name].push){o[this.name]=[o[this.name]]}o[this.name].push(this.value||"")}else{o[this.name]=this.value||""}});return o};const StepPacks={packs:{},add:function(id,name="",svg=""){this.packs[id]={id:id,name:name,svg:svg}},get:function(id){return this.packs[id]}};StepPacks.add("core",__("Groundhogg","groundhogg"));const tagWithConditionOnMount=(step,updateStepMeta)=>{tagOnMount(step,updateStepMeta);$("#condition").change(function(e){updateStepMeta({condition:$(this).val()})})};const tagOnMount=(step,updateStepMeta)=>{return tagPicker("#tags",true,items=>{TagsStore.itemsFetched(items)},{width:"100%",placeholder:__("Select one or more tags...","groundhogg")}).on("change",function(e){const tags=$(this).val();const newTags=tags.filter(tag=>!TagsStore.hasItem(parseInt(tag)));if(newTags.length>0){TagsStore.validate(tags).then(tags=>{updateStepMeta({tags:tags.map(tag=>tag.ID)})})}else{updateStepMeta({tags:tags.map(tag=>parseInt(tag))})}})};const fieldMappingTable=({fields:fields={},fieldMap:fieldMap={}})=>{const mappable=fields.map(({id:id,label:label},i)=>{return`<tr>
		...
		<table class="mapping-table">
		  ...
		  <svg viewBox="0 0 42 37" fill="none" xmlns="http://www.w3.org/2000/svg">
			  ...
			  <path
				  ...
				  fill="currentColor"/>
		  </svg>`,title:function({meta:meta}){const{note_text:note_text}=meta;if(note_text){return sprintf(_x("Add %s","add - note text","groundhogg"),`<i>${note_text.replace(/(<([^>]+)>)/gi,"").substring(0,30)}...</i>`)}return __("Add a note","groundhogg")},edit:function({meta:meta}){const{note_text:note_text=""}=meta;return`
			<div class="panel">
				<div class="row">
			...
			</div>`},onMount:function(step,updateStepMeta){let saveTimer;tinymceElement("note_text",{tinymce:true,quicktags:true},content=>{clearTimeout(saveTimer);saveTimer=setTimeout(function(){updateStepMeta({note_text:content})},300)})},onDemount:function(){wp.editor.remove("note_text")}},admin_notification:{pack:"core",svg:`
		  <svg viewBox="0 0 31 43" fill="none" xmlns="http://www.w3.org/2000/svg">
			...
			</div>`},onMount:function(step,updateStepMeta){$("#subject, #reply-to, #from, #to").on("change",function(e){const $this=$(this);updateStepMeta({[$this.prop("name")]:$this.val()})});let saveTimer;tinymceElement("note_text",{tinymce:true,quicktags:true},content=>{clearTimeout(saveTimer);saveTimer=setTimeout(function(){updateStepMeta({note_text:content})},300)})},onDemount:function(){wp.editor.remove("note_text")}},account_created:{pack:"core",defaults:{role:[]},svg:`
      ...
      `,title:function({ID:ID,data:data,meta:meta}){const roles=this.context.roles;if(meta.role&&meta.role.length===1){return sprintf(__("%s is created","groundhogg"),`<b>${roles[meta.role[0]]}</b>`)}else if(meta.role&&meta.role.length>1){return sprintf(__("%s is created","groundhogg"),orList(meta.role.map(role=>`<b>${roles[role]}</b>`)))}else{return __("A <b>new user</b> is created","groundhogg")}},edit:function({ID:ID,data:data,meta:meta}){let roles=this.context.roles;return`
			...
			</div>`},onMount:function(step,updateStepMeta){$("#roles").select2({width:"100%",placeholder:__("Select one or more user roles...","groundhogg")}).on("change",function(e){let roles=$(this).val();updateStepMeta({role:roles})})}},apply_tag:{pack:"core",defaults:{tags:[]},svg:`
		  <svg viewBox="0 0 35 37" fill="none" xmlns="http://www.w3.org/2000/svg">
		  ...
		  </svg>`,title:function({ID:ID,data:data,meta:meta}){let{tags:tags}=meta;if(tags){tags=tags.map(id=>parseInt(id))}if(!tags||tags.length===0){return __("Apply a tag","groundhogg")}else if(tags.length<4&&TagsStore.hasItems(tags)){return sprintf(_x("Apply %s","apply tags - list of tag names","groundhogg"),andList(tags.map(id=>`<b>${TagsStore.get(id).data.tag_name}</b>`)))}else{return sprintf(_x("Apply %s tags","apply tags - number","groundhogg"),`<b>${tags.length}</b>`)}},edit:function({ID:ID,data:data,meta:meta}){let options=TagsStore.getItems().map(tag=>{return{text:tag.data.tag_name,value:tag.ID}});return`
			<div class="panel">
				<div class="row">
			...
			</div>`},onMount:function(step,updateStepMeta){tagOnMount(step,updateStepMeta)},preload:preloadTags},remove_tag:{pack:"core",defaults:{tags:[]},svg:`
		  <svg viewBox="0 0 35 35" fill="none" xmlns="http://www.w3.org/2000/svg">
		  ...
		  </svg>`,title:function({ID:ID,data:data,meta:meta}){let{tags:tags}=meta;tags=tags.map(id=>parseInt(id));if(tags.length===0){return __("Remove tags","groundhogg")}else if(tags.length<4&&TagsStore.hasItems(tags)){return sprintf(_x("Remove %s","remove - list of tag names","groundhogg"),andList(tags.map(id=>`<b>${TagsStore.get(id).data.tag_name}</b>`)))}else{return sprintf(_x("Remove %s tags","remove tags - number","groundhogg"),`<b>${tags.length}</b>`)}},edit:function({ID:ID,data:data,meta:meta}){let options=TagsStore.getItems().map(tag=>{return{text:tag.data.tag_name,value:tag.ID}});return`
			<div class="panel">
				<div class="row">
			...
			</div>`},onMount:function(step,updateStepMeta){tagOnMount(step,updateStepMeta)},preload:preloadTags},tag_applied:{pack:"core",defaults:{tags:[]},svg:`
		  <svg viewBox="0 0 39 35" fill="none" xmlns="http://www.w3.org/2000/svg">
		  ...
		  </svg>`,title:function({ID:ID,data:data,meta:meta}){let{tags:tags=[],condition:condition="any"}=meta;tags=tags.map(id=>parseInt(id));if(tags.length>=4){return condition==="all"?sprintf(__("%s tags are applied","groundhogg"),`<b>${tags.length}</b>`):sprintf(__("Any of %s tags are applied","groundhogg"),`<b>${tags.length}</b>`)}else if(tags.length>1&&tags.length<4&&TagsStore.hasItems(tags)){const tagNames=tags.map(tag=>`<b>${TagsStore.get(tag).data.tag_name}</b>`);return condition==="all"?sprintf(_x("%s are applied","list of tags - are applied","groundhogg"),andList(tagNames)):sprintf(_x("%s is applied","list of tags - is applied","groundhogg"),orList(tagNames))}else if(tags.length===1){return sprintf(_x("%s is applied","list of tags - is applied","groundhogg"),`<b>${TagsStore.get(tags[0]).data.tag_name}</b>`)}else{return __("A tag is applied","groundhogg")}},edit:function({ID:ID,data:data,meta:meta}){let options=TagsStore.getItems().map(tag=>{return{text:tag.data.tag_name,value:tag.ID}});const{tags:tags=[],condition:condition="any"}=meta;return`
			<div class="panel">
				<div class="row">
			...
			</div>`},onMount:function(step,updateStepMeta){tagWithConditionOnMount(step,updateStepMeta)},preload:preloadTags},tag_removed:{pack:"core",defaults:{tags:[]},svg:`
		  <svg viewBox="0 0 37 35" fill="none" xmlns="http://www.w3.org/2000/svg">
			  ...
			  <path
				  ...
				  fill="currentColor"/>
		  </svg>`,title:function({ID:ID,data:data,meta:meta}){let{tags:tags=[],condition:condition="any"}=meta;tags=tags.map(id=>parseInt(id));if(tags.length>=4){return condition==="all"?sprintf(__("%s tags are removed","groundhogg"),`<b>${tags.length}</b>`):sprintf(__("Any of %s tags are removed","groundhogg"),`<b>${tags.length}</b>`)}else if(tags.length>1&&tags.length<4&&TagsStore.hasItems(tags)){const tagNames=tags.map(tag=>`<b>${TagsStore.get(tag).data.tag_name}</b>`);return condition==="all"?sprintf(_x("%s are removed","list of tags - are removed","groundhogg"),andList(tagNames)):sprintf(_x("%s is removed","list of tags - is removed","groundhogg"),orList(tagNames))}else if(tags.length===1){return sprintf(_x("%s is removed","list of tags - is removed","groundhogg"),`<b>${TagsStore.get(tags[0]).data.tag_name}</b>`)}else{return __("A tag is removed","groundhogg")}},edit:function({ID:ID,data:data,meta:meta}){const{condition:condition="any",tags:tags=[]}=meta;return`
			<div class="panel">
				<div class="row">
					<label class="row-label" for="tags">
			...
			</div>`},onMount:function(step,updateStepMeta){tagWithConditionOnMount(step,updateStepMeta)},preload:preloadTags},delay_timer:{pack:"core",defaults:{...delayTimerDefaults},svg:`
      ...
      `,title:function({meta:meta}){return delayTimerName({...delayTimerDefaults,...meta})},edit:function({ID:ID,data:data,meta:meta}){const{delay_amount:delay_amount,delay_type:delay_type,run_on_type:run_on_type,run_when:run_when,run_time:run_time,send_in_timezone:send_in_timezone,run_time_to:run_time_to,run_on_dow_type:run_on_dow_type,run_on_dow:run_on_dow,run_on_month_type:run_on_month_type,run_on_months:run_on_months,run_on_dom:run_on_dom}={...delayTimerDefaults,...meta};const runOnTypes={any:"Any day",weekday:"Weekday",weekend:"Weekend",day_of_week:"Day of week",day_of_month:"Day of month"};const runWhenTypes={now:__("Any time","groundhogg"),later:__("Specific time","groundhogg")};if(["minutes","hours","none"].includes(delay_type)){runWhenTypes.between=__("Between","groundhogg")}const runOnDaysOfMonth={};for(let i=1;i<32;i++){runOnDaysOfMonth[i]=i}runOnDaysOfMonth.last="last";const runOnMonthTypes={any:"Of any month",specific:"Of specific month(s)"};const runOnMonthOptions=`
		  ...
		  <svg viewBox="0 0 35 35" fill="none" xmlns="http://www.w3.org/2000/svg">
			  ...
			  <path d="M3.674 8.903l14.166 8.5 14.167-8.5M20.674 24.487h11.333m0 0l-4.25-4.25m4.25 4.25l-4.25 4.25"
			        stroke="currentColor" stroke-width="2"/>
		  </svg>`,title:function({ID:ID,data:data,meta:meta}){const{email_id:email_id}=meta;const email=EmailsStore.get(parseInt(email_id));return email_id&&email?sprintf(_x("Send %s","send - email title","groundhogg"),`<b>${email.data.title}</b>`):__("Send <b>an email</b>","groundhogg")},edit:function({ID:ID,data:data,meta:meta}){const{email_id:email_id}=meta;const email=EmailsStore.get(parseInt(email_id));const iframePreview=email=>{const{context:context}=email;return`
			  <div class="panel">
				  <div class="row">
		  ...
		  <svg viewBox="0 0 35 35" fill="none" xmlns="http://www.w3.org/2000/svg">
			...
			</div>`},onMount:function({meta:meta},updateStepMeta){linkPicker("#redirect-to").on("change",function(e){updateStepMeta({redirect_to:$(this).val()})})}},email_confirmed:{pack:"core",svg:`
		  <svg viewBox="0 0 35 35" fill="none" xmlns="http://www.w3.org/2000/svg">
		  ...
		  </svg>`,title:function({meta:meta}){const{form_name:form_name}=meta;if(!form_name){return __("Submits <b>a form</b>","groundhogg")}return sprintf(__("Submits %s","groundhogg"),`<b>${form_name}</b>`)},edit:function({meta:meta}){const redirectToURL=`<label class="row-label">${__("Redirect to this URL...","groundhogg")}</label>
