(function($){const{tags:TagsStore,emails:EmailsStore,funnels:FunnelsStore}=Groundhogg.stores;const{get:apiGet,post:apiPost,delete:apiDelete,patch:apiPatch,routes}=Groundhogg.api;const{v4:apiRoutes}=routes;const{tinymceElement,addMediaToBasicTinyMCE,mappableFields,bold,uuid,orList,andList,specialChars,select,input,textarea,toggle,textAreaWithReplacements,setFrameContent,inputWithReplacements,inputWithReplacementsAndEmojis,ordinal_suffix_of,copyObject}=Groundhogg.element;const{sprintf,__,_x,_n}=wp.i18n;const{formBuilder,rawStepTypes}=Groundhogg;const{linkPicker,emailPicker,tagPicker,apiPicker}=Groundhogg.pickers;$.fn.serializeFormJSON=function(){var o={};var a=this.serializeArray();$.each(a,function(){if(o[this.name]){if(!o[this.name].push){o[this.name]=[o[this.name]]}o[this.name].push(this.value||"")}else{o[this.name]=this.value||""}});return o};const StepPacks={packs:{},add(id,name="",svg=""){this.packs[id]={id:id,name:name,svg:svg}},get(id){return this.packs[id]}};StepPacks.add("core",__("Groundhogg","groundhogg"));const tagWithConditionOnMount=(step,updateStepMeta)=>{tagOnMount(step,updateStepMeta);$("#condition").change(function(e){updateStepMeta({condition:$(this).val()})})};const tagOnMount=(step,updateStepMeta)=>{return tagPicker("#tags",true,items=>{TagsStore.itemsFetched(items)},{width:"100%",placeholder:__("Select one or more tags...","groundhogg")}).on("change",function(e){const tags=$(this).val();const newTags=tags.filter(tag=>!TagsStore.hasItem(parseInt(tag)));if(newTags.length>0){TagsStore.validate(tags).then(tags=>{updateStepMeta({tags:tags.map(tag=>tag.ID)})})}else{updateStepMeta({tags:tags.map(tag=>parseInt(tag))})}})};const fieldMappingTable=({fields:fields={},fieldMap:fieldMap={}})=>{const mappable=fields.map(({id,label},i)=>{return`<tr>
      ...
      `,title({meta}){return delayTimerName({...delayTimerDefaults,...meta})},edit({ID,data,meta}){const{delay_amount,delay_type,run_on_type,run_when,run_time,send_in_timezone,run_time_to,run_on_dow_type,run_on_dow,run_on_month_type,run_on_months,run_on_dom}={...delayTimerDefaults,...meta};const runOnTypes={any:"Any day",weekday:"Weekday",weekend:"Weekend",day_of_week:"Day of week",day_of_month:"Day of month"};const runWhenTypes={now:__("Any time","groundhogg"),later:__("Specific time","groundhogg")};if(["minutes","hours","none"].includes(delay_type)){runWhenTypes.between=__("Between","groundhogg")}const runOnDaysOfMonth={};for(let i=1;i<32;i++){runOnDaysOfMonth[i]=i}runOnDaysOfMonth.last="last";const runOnMonthTypes={any:"Of any month",specific:"Of specific month(s)"};const runOnMonthOptions=`
          ...
          <svg viewBox="0 0 35 35" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            </div>`},onMount({meta},updateStepMeta){linkPicker("#redirect-to").on("change",function(e){updateStepMeta({redirect_to:$(this).val()})})}},email_confirmed:{pack:"core",svg:`
          <svg viewBox="0 0 35 35" fill="none" xmlns="http://www.w3.org/2000/svg">
