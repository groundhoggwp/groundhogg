($=>{const{notes:NotesStore}=Groundhogg.stores;const{uuid,specialChars,icons,modal,copyObject,input,textarea,select,tinymceElement,moreMenu,tooltip,inputRepeaterWidget,confirmationModal,dangerConfirmationModal,toggle}=Groundhogg.element;const{post,get,patch,routes,ajax}=Groundhogg.api;const{formatNumber,formatTime,formatDate,formatDateTime}=Groundhogg.formatting;const{sprintf,__,_x,_n}=wp.i18n;const optionsRepeater=({selector,options,onChange})=>{inputRepeaterWidget({selector:selector,rows:options.map(o=>[o]),cellCallbacks:[input],cellProps:[{placeholder:__("Option")}],sortable:true,onChange:r=>{onChange(r.map(r=>r[0]))}}).mount()};const fieldTypes={text:{name:__("Text","groundhogg"),view:({label,...props})=>{return`<label class="property-label" for="${props.id}">${label}</label>${input({className:"full-width",...props,type:"text"})}`},onMount:({id,name},onChange)=>{$(`#${id}`).on("change",e=>{onChange({[name]:e.target.value})})},edit:()=>{return""},onEditMount:()=>{}},textarea:{name:__("Textarea","groundhogg"),view:({label,...props})=>{return`<label class="property-label" for="${props.id}">${label}</label>${textarea({className:"full-width",...props})}`},onMount:({id,name},onChange)=>{$(`#${id}`).on("change",e=>{onChange({[name]:e.target.value})})},edit:()=>{return""},onEditMount:()=>{}},number:{name:__("Number","groundhogg"),view:({label,...props})=>{return`<label class="property-label" for="${props.id}">${label}</label>${input({className:"full-width",...props,type:"number"})}`},onMount:({id,name},onChange)=>{$(`#${id}`).on("change",e=>{onChange({[name]:e.target.value})})},edit:()=>{return""},onEditMount:()=>{}},date:{name:__("Date","groundhogg"),view:({label,...props})=>{return`<label class="property-label" for="${props.id}">${label}</label>${input({className:"full-width",...props,type:"date"})}`},onMount:({id,name},onChange)=>{$(`#${id}`).on("change",e=>{onChange({[name]:e.target.value})})},edit:()=>{return""},onEditMount:()=>{}},time:{name:__("Time","groundhogg"),view:({label,...props})=>{return`<label class="property-label" for="${props.id}">${label}</label>${input({className:"full-width",...props,type:"time"})}`},onMount:({id,name},onChange)=>{$(`#${id}`).on("change",e=>{onChange({[name]:e.target.value})})},edit:()=>{return""},onEditMount:()=>{}},checkboxes:{name:__("Checkboxes","groundhogg"),view:({label,id,name,options,value,...props})=>{if(!Array.isArray(value)){value=[value]}return`<label class="property-label">${label}</label>
        ...
        `},onEditMount:(field,updateField)=>{optionsRepeater({selector:"#property-dropdown-options",options:field.options||[""],onChange:options=>updateField({options:options}),sortable:true});$("#allow-multiple").on("change",e=>{updateField({multiple:e.target.checked})})}}};const Templates={noProperties:()=>{return`
		  ...
		  </div>`},group:(group,fields=[],editable)=>{fields.sort(({order:a},{order:b})=>a-b);return`
		  ...
		  </div>`},addField:field=>{const{type,label,name,id,order:order=10}=field;return`
