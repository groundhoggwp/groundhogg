(function($,Templates){const{breadcrumbs:breadcrumbs,select:select,regexp:regexp,modal:modal,input:input,primaryButton:primaryButton,loadingDots:loadingDots,setFrameContent:setFrameContent}=Groundhogg.element;const{templates:templates,stepTypes:stepTypes}=Templates;const{post:post,get:get,routes:routes}=Groundhogg.api;const{__:__,_x:_x,_n:_n,_nx:_nx,sprintf:sprintf}=wp.i18n;const{formatNumber:formatNumber,formatTime:formatTime,formatDate:formatDate,formatDateTime:formatDateTime}=Groundhogg.formatting;const hasRequiredSteps=steps=>{return steps.reduce((hasStep,{data:data,type:type})=>{if(data){return stepTypes.hasOwnProperty(data.step_type)&&hasStep}else if(type){return stepTypes.hasOwnProperty(type)&&hasStep}return false},true)};const rowsOf=(size,items)=>{return items.reduce((rows,item)=>{if(rows[rows.length-1].length===size){rows.push([])}rows[rows.length-1].push(item);return rows},[[]])};const createFunnel=template=>{return post(`${routes.v4.funnels}/import`,{...template}).then(r=>r.item)};const fillRow=(size,fill,row)=>{while(row.length<size){row.push(fill)}return row};const FunnelTemplatePicker=({selector:selector,breadcrumbs:crumbs=["Funnels","Add New"],onSelect:onSelect=funnel=>{console.log(funnel)},afterHeaderActions:afterHeaderActions="",onMount:onMount=()=>{}})=>({search:"",selectedTemplate:"",$el:$(selector),renderTemplate:template=>{const{ID:ID,data:data,meta:meta,steps:steps,campaigns:campaigns}=template;const stepCount=steps.length;const canUse=hasRequiredSteps(steps);const numActions=steps.filter(({data:data})=>data.step_group==="action").length;const numBenchmarks=steps.filter(({data:data})=>data.step_group==="benchmark").length;const pills=[`<span class="pill orange">${sprintf(_n("%d benchmark","%d benchmarks",numBenchmarks,"groundhogg"),numBenchmarks)}</span>`,`<span class="pill green">${sprintf(_n("%d action","%d actions",numBenchmarks,"groundhogg"),numActions)}</span>`,`<span class="pill">${sprintf(__("Added %s","groundhogg"),formatDate(data.date_created))}</span>`];return`
		  <div class="gh-panel template ${canUse?__("enabled","groundhogg"):__("disabled","groundhogg")}"
			  ...
			  <div class="inside">
				  ...
				  <p class="actions">
					  ${hasRequiredSteps(steps)?`<button data-template="${ID}" class="gh-button primary small select-template">Import</button>`:`<span class="gh-text danger">${__("You do not have the required extensions installed for this template.","groundhogg")}</span>`}
				  </p>
			  </div>
		  ...
		  <div class="templates-picker">
			  <div id="header" class="gh-header is-sticky">
				  ...
				  <div class="search-templates">
					  ${select({id:"campaign",name:"campaign",multiple:true,style:{width:"300px"}},templates.reduce((carry,curr)=>{if(curr.campaigns){carry.push(...curr.campaigns.map(c=>({text:c.data.name,value:c.data.slug})).filter(c=>!carry.find(_c=>c.value===_c.value)))}return carry},[]))}
			  ...
			  <div id="view"></div>
		  </div>`},renderTemplates:function(){return`<div id="templates">${rowsOf(3,this.getTemplates().map(t=>this.renderTemplate(t))).map(r=>`<div class="row">${fillRow(3,'<div class="fill"></div>',r).join("")}</div>`).join("")}</div>`},getTemplates:function(){return templates.filter(t=>{if(!this.search){return true}return t.data.title.match(regexp(this.search))}).filter(t=>{if(!this.campaign||!this.campaign.length){return true}return t.campaigns.find(c=>this.campaign.includes(c.data.slug))}).sort((a,b)=>hasRequiredSteps(a.steps)===hasRequiredSteps(b.steps)?0:hasRequiredSteps(a.steps)?-1:1)},titleModal:function(){const modalContent=(isCreating=false)=>{return`
