($=>{const{modal,errorDialog,loadingDots,select,uuid,addMediaToBasicTinyMCE,specialChars,tinymceElement,searchOptionsWidget,input,isNumeric,icons,dialog,tooltip,regexp,isValidEmail,loadingModal,textarea,spinner,skeleton,adminPageURL}=Groundhogg.element;const{contacts:ContactsStore,tags:TagsStore,forms:FormsStore,emails:EmailsStore}=Groundhogg.stores;const{post,routes,postFormData}=Groundhogg.api;const{tagPicker}=Groundhogg.pickers;const{userHasCap}=Groundhogg.user;const{sprintf,__,_x,_n}=wp.i18n;const{formatNumber,formatTime,formatDate,formatDateTime}=Groundhogg.formatting;const{currentUser}=Groundhogg;const selectContactModal=({onSelect:onSelect=()=>{},exclude:exclude=[]})=>{let search,timeout,results;const form=()=>{return`
          <div id="search-form">
          ...
          </div>`};const{close,setContent}=modal({content:form(),dialogClasses:"no-padding"});const renderResult=contact=>{return`
    ...
    #${prefix}-marketing-consent`).on("change",({target})=>{setPayload({meta:{..._payload.meta,[target.name]:target.checked}})});betterTagPicker(`   #${prefix}-tags-here`,{selected:[],onChange:({addTags})=>{setPayload({tags:addTags})}});additionalFieldsOnMount({prefix:prefix,setPayload:setPayload,getPayload:getPayload})};const internalForm=({contact:contact=false,onSubmit:onSubmit=()=>{}})=>{let selectedForm;const ui=()=>{return`
      ...
      `,onOpen:({close})=>{let file=null;let filesToUpload=[];let filesUploaded=[];let uploading=false;const pushFiles=()=>{renderUploadingFiles();file=filesToUpload.pop();if(!file){uploading=false;return}uploading=true;let fd=new FormData;fd.append(fileName,file,file.name);fd.append("gh_admin_ajax_nonce",Groundhogg.nonces._adminajax);fd.append("action",action);beforeUpload(fd);setTimeout(()=>{fetch(ajaxurl,{method:"POST",credentials:"same-origin",body:fd}).then(r=>{if(!r.ok){dialog({message:__("Something when wrong..."),type:"error"});return}return r.json()}).then(r=>{if(!r.success){dialog({message:r.data[0].message,type:"error"});pushFiles();return}onUpload(r,file);filesUploaded.unshift(file);renderUploadedFiles();pushFiles()})},2e3)};const renderUploadingFiles=()=>{$("#uploading-files").html(filesToUpload.map(f=>`<div class="file"><span class="hourglass">⌛</span> ${f.name}</div>`))};const renderUploadedFiles=()=>{$("#uploaded-files").html(filesUploaded.map(f=>`<div class="file">✅ ${f.name}</div>`))};const addFiles=files=>{filesToUpload.push(...files);if(!uploading){pushFiles()}};const $input=$("#upload-file-input");$input.on("change",e=>{addFiles(e.target.files)});$("#select-files").on("click",e=>{e.preventDefault();$input.click()});const $droppable=$(".droppable-handler");$droppable.on("dragover",e=>{e.preventDefault();$droppable.addClass("dragover")}).on("dragleave",e=>{$droppable.removeClass("dragover")}).on("drop",e=>{e.preventDefault();$droppable.removeClass("dragover");let{dataTransfer}=e.originalEvent;addFiles(dataTransfer.files)})}})};const{Div,Img,An,Span,ModalFrame,Iframe,makeEl,Button,Modal,Dashicon,Input,Label,Fragment,Pg,Form,Textarea,InputGroup}=MakeEl;const EmailPreviewModal=async(emailId,{height:height=window.innerHeight*.85,width:width=900})=>{const{close}=loadingModal();let email;try{email=await EmailsStore.maybeFetchItem(emailId)}catch(err){close();throw err}const{from_avatar,from_email,from_name,subject,built:content}=email.context;close();return ModalFrame({frameAttributes:{className:"gh-modal-frame gh-email-preview-modal"}},({close})=>Div({style:{width:`${width}px`,height:`${height}px`}},EmailPreview({close:close,from_avatar:from_avatar,from_email:from_email,from_name:from_name,subject:subject,content:content})))};const EmailPreview=({close:close=false,from_avatar,from_email,from_name,subject,content})=>{return Div({className:"email-preview"},[Div({className:"from-preview display-flex gap-20 has-box-shadow"},[makeEl("img",{src:from_avatar,className:"from-avatar",height:40,width:40,style:{borderRadius:"50%"}}),Div({className:"subject-and-from"},[`<h2>${subject}</h2>`,`<span class="from-name">${from_name}</span> <span class="from-email">&lt;${from_email}&gt;</span>`]),close!==false?Button({className:"gh-button secondary icon text",style:{marginLeft:"auto"},onClick:close},Dashicon("no-alt")):null]),Iframe({id:"desktop-preview-iframe"},content)])};$(()=>{$(document).on("click","table.wp-list-table .gh-email-preview",e=>{e.preventDefault();EmailPreviewModal(parseInt($(e.currentTarget).closest("tr").attr("id")),{})})});const ImagePicker=({multiple:multiple=false,title:title=__("Select a image to upload"),selectText:selectText=__("Use this image"),onChange:onChange=attachment=>{}})=>{let file_frame=wp.media({title:title,button:{text:selectText},multiple:multiple});file_frame.on("select",function(){let attachment=file_frame.state().get("selection").first().toJSON();onChange(attachment)});file_frame.open()};const ImageInput=({id,name:name="src",onChange,value:value=""})=>{const handleChange=(value,attachment)=>{onChange(value,attachment);morphdom(document.getElementById(id),ImageInput({id:id,name:name,onChange:onChange,value:value}))};return Div({id:id,className:"image-picker"},[value?Div({id:`${id}-preview`,className:"image-input-preview",style:{backgroundImage:`url(${value})`},onClick:e=>{e.preventDefault();ImagePicker({multiple:false,onChange:attachment=>handleChange(attachment.url,attachment)})}}):null,InputGroup([Input({type:"text",id:`${id}-src`,value:value,className:"control full-width",name:name,onChange:e=>{handleChange(e.target.value)}}),Button({id:`${id}-select`,className:"gh-button secondary icon",onClick:e=>{e.preventDefault();ImagePicker({multiple:false,onChange:attachment=>handleChange(attachment.url,attachment)})}},icons.image)])])};const FeedbackModal=({subject:subject="",message:message="",onSubmit:onSubmit=r=>{}})=>{const State=Groundhogg.createState({subject:subject,message:message,submitting:false});Modal({width:"400px"},({close,morph})=>Form({className:"display-flex column gap-5",onSubmit:e=>{e.preventDefault();State.set({submitting:true});morph();Groundhogg.api.ajax({action:"gh_plugin_feedback",subject:State.subject,message:State.message}).then(r=>{onSubmit(r);dialog({message:"Thanks for your feedback!"});close()});return false}},[Label({for:"feedback-subject"},["What feature are you submitting feedback for?"]),Input({id:"feedback-subject",value:State.subject,required:true,onInput:e=>State.set({subject:e.target.value})}),Div(),Label({for:"feedback-message"},["What is your feedback? Be as descriptive as possible."]),Textarea({id:"feedback-message",value:State.message,required:true,rows:4,onInput:e=>State.set({message:e.target.value})}),Button({className:"gh-button primary",type:"submit",disabled:State.submitting},"Send feedback"),Pg({},"Your email address will be collected to validate your feedback, but will not be used beyond that.")]))};$(document).on("click","a.feedback-modal",e=>{e.preventDefault();const{subject:subject="",message:message=""}=e.currentTarget.dataset;FeedbackModal({subject:subject,message:message})});const ContactListItem=item=>{let allTags=item.tags;let showTags=allTags.splice(0,10);const{ID}=item;const{full_name,gravatar,date_created,email}=item.data;return Div({className:"contact-list-item",id:`contact-list-item-${ID}`,onClick:e=>{window.open(item.admin,"_self")}},[Div({className:"display-flex gap-10"},[Img({className:"avatar",src:gravatar,alt:"avatar"}),Div({className:"display-flex column"},[Div({className:"display-flex"},[makeEl("h4",{style:{margin:0}},full_name),Span({className:"subscribed"},`&nbsp;— ${sprintf(__("Subscribed %s"),`<abbr title="${formatDateTime(date_created)}">${sprintf(__("%s ago "),item.i18n.created)}</abbr>`)}`)]),Div({},[An({href:`mailto: ${email}`},email)," — ",Span({className:`gh-text ${item.is_marketable?"green":"red"}`},Groundhogg.filters.optin_status[item.data.optin_status])])])]),Div({className:"gh-tags"},[...showTags.map(tag=>Span({className:"gh-tag"},tag.data.tag_name))])])};const ContactList=contacts=>Div({className:"contact-list"},contacts.map(contact=>ContactListItem(contact)));const QuickSearch=()=>{const State=Groundhogg.createState({search:"",searched:false,results:[]});return Div({id:"quick-search-wrap"},morph=>{const updateResults=Groundhogg.functions.debounce(async()=>{let results=await ContactsStore.fetchItems({search:State.search,orderby:"date_created",order:"DESC",limit:5});State.set({results:results,searched:true});morph()},300);return Fragment([Form({action:adminPageURL("gh_contacts")},[Input({type:"hidden",name:"page",value:"gh_contacts"}),Input({id:"quick-search-input",placeholder:__("Search by name or email...","groundhogg"),type:"search",name:"s",value:State.search,onInput:e=>{State.set({search:e.target.value});updateResults()}})]),State.results.length?ContactList(State.results):null,State.results.length===0&&State.searched?Pg({},__("No contacts found for the current search","groundhogg")):null])})};Groundhogg.components={QuickSearch:QuickSearch,addContactModal:addContactModal,internalForm:internalForm,betterTagPicker:betterTagPicker,quickAddForm:quickAddForm,selectContactModal:selectContactModal,quickEditContactModal:quickEditContactModal,makeInput:makeInput,emailModal:emailModal,fileUploader:fileUploader,EmailPreview:EmailPreview,EmailPreviewModal:EmailPreviewModal,ImageInput:ImageInput,ImagePicker:ImagePicker,FeedbackModal:FeedbackModal,ContactList:ContactList,ContactListItem:ContactListItem}})(jQuery);