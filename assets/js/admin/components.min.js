($=>{const{modal:modal,errorDialog:errorDialog,loadingDots:loadingDots,select:select,uuid:uuid,addMediaToBasicTinyMCE:addMediaToBasicTinyMCE,specialChars:specialChars,tinymceElement:tinymceElement,searchOptionsWidget:searchOptionsWidget,input:input,isNumeric:isNumeric,icons:icons,dialog:dialog,tooltip:tooltip,regexp:regexp,isValidEmail:isValidEmail,textarea:textarea,spinner:spinner}=Groundhogg.element;const{contacts:ContactsStore,tags:TagsStore,forms:FormsStore}=Groundhogg.stores;const{post:post,routes:routes}=Groundhogg.api;const{tagPicker:tagPicker}=Groundhogg.pickers;const{sprintf:sprintf,__:__,_x:_x,_n:_n}=wp.i18n;const{formatNumber:formatNumber,formatTime:formatTime,formatDate:formatDate,formatDateTime:formatDateTime}=Groundhogg.formatting;const{currentUser:currentUser}=Groundhogg;const selectContactModal=({onSelect:onSelect=()=>{},exclude:exclude=[]})=>{let search,timeout,results;const form=()=>{return`
		  <div id="search-form">
		  ...
		  </div>`};const{close:close,setContent:setContent}=modal({content:form(),dialogClasses:"no-padding"});const renderResult=contact=>{return`
		  ...
		  </tr>`};const noResults=()=>{return`
		  <tr>
			  <td colspan="3"><p>${__("No contacts match that search...","groundhogg")}</p></td>
		  </tr>`};const onMount=()=>{const setSearchResults=results=>{if(!results.length){$("#search-results table tbody").html(noResults());return}$("#search-results table tbody").html(results.map(r=>renderResult(r)).join(""));$("#search-results tr, .select-contact").on("click",e=>{close();onSelect(ContactsStore.get(parseInt(e.currentTarget.dataset.id)))})};const getResults=()=>{if(timeout){clearTimeout(timeout)}timeout=setTimeout(()=>{ContactsStore.fetchItems({search:search,exclude:exclude,limit:10}).then(items=>{results=items;setSearchResults(results)})},1e3)};$("#contact-search").on("input change",e=>{search=e.target.value;getResults()}).focus();let results=ContactsStore.getItems().filter(c=>!exclude.includes(c.ID));if(results.length){setSearchResults(results)}else{getResults()}};onMount()};const betterTagPicker=(el,{selected:selected=[],removeTags:removeTags=[],addTags:addTags=[],onChange:onChange=changes=>{}})=>{const $el=$(el);let timeout;const template=()=>{return`
		  <div class="gh-tags">
		  ...
		  </div>`};const mount=()=>{$el.html(template());onMount()};const informChanges=()=>{onChange({removeTags:removeTags,addTags:addTags})};const onMount=()=>{tooltip(".add-tag",{content:__("Add a tag","groundhogg")});$(".gh-tag .remove-tag").on("click",e=>{let tagId=parseInt(e.currentTarget.dataset.id);if(removeTags.includes(tagId)){removeTags.splice(removeTags.indexOf(tagId),1)}else{removeTags.push(tagId)}informChanges();mount()});$(".gh-tag .remove-adding-tag").on("click",e=>{let tagId=parseInt(e.currentTarget.dataset.id);if(addTags.includes(tagId)){addTags.splice(addTags.indexOf(tagId),1)}informChanges();mount()});$(".add-tag").on("click",e=>{searchOptionsWidget({target:e.currentTarget,position:"fixed",noOptions:__("No tags found...","groundhogg"),options:TagsStore.items.filter(t=>!selected.map(_t=>_t.ID).includes(t.ID)&&!addTags.includes(t.ID)),filterOption:({data:data},search)=>data.tag_name.match(regexp(search)),filterOptions:(opts,search)=>{if(!search){return opts}return[{ID:search,data:{tag_name:`Add "${search}"`}},...opts]},renderOption:({data:data})=>data.tag_name,onClose:()=>{mount()},onInput:(search,widget)=>{if(timeout){clearTimeout(timeout)}timeout=setTimeout(()=>{TagsStore.fetchItems({search:search}).then(()=>{widget.options=TagsStore.items.filter(t=>!selected.map(_t=>_t.ID).includes(t.ID));widget.mountOptions()})},1500)},onSelect:tag=>{let{ID:ID}=tag;if(!isNumeric(ID)){TagsStore.post({data:{tag_name:ID}}).then(t=>{addTags.push(t.ID);informChanges();mount()});return}addTags.push(ID);informChanges()},onOpen:()=>{}}).mount()})};TagsStore.fetchItems();TagsStore.itemsFetched(selected);mount()};const quickEditContactModal=({contact:contact,prefix:prefix="quick-edit",onEdit:onEdit=contact=>{},additionalFields:additionalFields=()=>"",additionalFieldsOnMount:additionalFieldsOnMount=()=>{}})=>{if(contact&&contact.tags){TagsStore.itemsFetched(contact.tags)}const getContact=()=>{return ContactsStore.get(contact.ID)};const quickEdit=contact=>{return`
		  ...
		  </div>`};const quickEditMounted=({close:close,setContent:setContent})=>{let payload;const clearPayload=()=>{payload={data:{},meta:{},add_tags:[],remove_tags:[]}};clearPayload();const updateContact=data=>{payload={...data,data:{...payload.data,...data.data},meta:{...payload.meta,...data.meta}}};const $quickEdit=$(".contact-quick-edit");$quickEdit.focus();$(`#${prefix}-save`).on("click",e=>{const $btn=$(e.target);$btn.prop("disabled",true);$btn.text(__("Saving","groundhogg"));const{stop:stop}=loadingDots(`#${prefix}-save`);ContactsStore.patch(contact.ID,payload).then(c=>{stop();clearPayload();onEdit(c);setContent(quickEdit(getContact()));quickEditMounted({close:close,setContent:setContent})}).catch(e=>{stop();clearPayload();setContent(quickEdit(getContact()));quickEditMounted({close:close,setContent:setContent});console.log(e);dialog({type:"error",message:e.message})})});$(`.${prefix}-cancel`).on("click",e=>{clearPayload();close()});betterTagPicker(`#${prefix}-tags-here`,{selected:getContact().tags,onChange:({addTags:addTags,removeTags:removeTags})=>{updateContact({add_tags:addTags,remove_tags:removeTags})}});$(`#${prefix}-first-name, #${prefix}-last-name, #${prefix}-email, #${prefix}-optin-status, #${prefix}-owner`).on("change",e=>{updateContact({data:{[e.target.name]:e.target.value}})});$(`#${prefix}-primary-phone, #${prefix}-primary-phone-extension, #${prefix}-mobile-phone`).on("change",e=>{updateContact({meta:{[e.target.name]:e.target.value}})});additionalFieldsOnMount({prefix:prefix,contact:contact,setPayload:updateContact,getPayload:()=>payload})};const{close:close,setContent:setContent}=modal({content:quickEdit(getContact()),onOpen:quickEditMounted})};const quickAddForm=(selector,{prefix:prefix="quick-add",onCreate:onCreate=()=>{},additionalFields:additionalFields=({prefix:prefix})=>"",additionalFieldsOnMount:additionalFieldsOnMount=()=>{}})=>{const quickAddForm=()=>{return`
		  ...
		  </div>`};$(selector).html(quickAddForm());let _payload={data:{owner_id:currentUser.ID},meta:{}};const getPayload=()=>{return _payload};const setPayload=data=>{_payload={..._payload,...data}};$(`#${prefix}-create`).on("click",({target:target})=>{if(!_payload.data.email||!isValidEmail(_payload.data.email)){errorDialog({message:__("A valid email is required!","groundhogg")});return}$(target).prop("disabled",true);const{stop:stop}=loadingDots(`#${prefix}-quick-add-button`);ContactsStore.post(_payload).then(c=>{stop();onCreate(c)})});$(`
    #${prefix}-first-name,
    #${prefix}-last-name,
    #${prefix}-owner,
    #${prefix}-optin-status,
    #${prefix}-email`).on("change input",({target:target})=>{setPayload({data:{..._payload.data,[target.name]:target.value}})});$(`
    #${prefix}-primary-phone,
    #${prefix}-primary-phone-ext,
    #${prefix}-mobile-phone`).on("change input",({target:target})=>{setPayload({meta:{..._payload.meta,[target.name]:target.value}})});$(`
    #${prefix}-terms,
    #${prefix}-data-consent,
    #${prefix}-marketing-consent`).on("change",({target:target})=>{setPayload({meta:{..._payload.meta,[target.name]:target.checked}})});betterTagPicker(`   #${prefix}-tags-here`,{selected:[],onChange:({addTags:addTags})=>{setPayload({tags:addTags})}});additionalFieldsOnMount({prefix:prefix,setPayload:setPayload,getPayload:getPayload})};const internalForm=({contact:contact=false,onSubmit:onSubmit=()=>{}})=>{let selectedForm;const ui=()=>{return`
			<div class="gh-rows-and-columns">
				<div class="gh-row">
					<div class="gh-col">
      ...
      `,onOpen:()=>{let file=null;let filesToUpload=[];let filesUploaded=[];let uploading=false;const pushFiles=()=>{renderUploadingFiles();file=filesToUpload.pop();if(!file){uploading=false;return}uploading=true;let fd=new FormData;fd.append(fileName,file,file.name);fd.append("_wpnonce",nonce);fd.append("action",action);beforeUpload(fd);setTimeout(()=>{fetch(ajaxurl,{method:"POST",credentials:"same-origin",body:fd}).then(r=>{if(!r.ok){dialog({message:__("Something when wrong..."),type:"error"});return}return r.json()}).then(r=>{if(!r.success){dialog({message:r.data[0].message,type:"error"});pushFiles();return}onUpload(r,file);filesUploaded.unshift(file);renderUploadedFiles();pushFiles()})},2e3)};const renderUploadingFiles=()=>{$("#uploading-files").html(filesToUpload.map(f=>`<div class="file"><span class="hourglass">⌛</span> ${f.name}</div>`))};const renderUploadedFiles=()=>{$("#uploaded-files").html(filesUploaded.map(f=>`<div class="file">✅ ${f.name}</div>`))};const addFiles=files=>{filesToUpload.push(...files);if(!uploading){pushFiles()}};const $input=$("#upload-file-input");$input.on("change",e=>{addFiles(e.target.files)});$("#select-files").on("click",e=>{e.preventDefault();$input.click()});const $droppable=$(".droppable-handler");$droppable.on("dragover",e=>{e.preventDefault();$droppable.addClass("dragover")}).on("dragleave",e=>{$droppable.removeClass("dragover")}).on("drop",e=>{e.preventDefault();$droppable.removeClass("dragover");let{dataTransfer:dataTransfer}=e.originalEvent;addFiles(dataTransfer.files)})}})};Groundhogg.components={addContactModal:addContactModal,internalForm:internalForm,betterTagPicker:betterTagPicker,quickAddForm:quickAddForm,selectContactModal:selectContactModal,quickEditContactModal:quickEditContactModal,makeInput:makeInput,emailModal:emailModal,fileUploader:fileUploader}})(jQuery);