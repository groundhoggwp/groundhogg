($=>{const{modal:modal,errorDialog:errorDialog,loadingDots:loadingDots,select:select,uuid:uuid,addMediaToBasicTinyMCE:addMediaToBasicTinyMCE,specialChars:specialChars,tinymceElement:tinymceElement,input:input,icons:icons,dialog:dialog,isValidEmail:isValidEmail,textarea:textarea}=Groundhogg.element;const{contacts:ContactsStore,tags:TagsStore,forms:FormsStore}=Groundhogg.stores;const{post:post,routes:routes}=Groundhogg.api;const{tagPicker:tagPicker}=Groundhogg.pickers;const{sprintf:sprintf,__:__,_x:_x,_n:_n}=wp.i18n;const{formatNumber:formatNumber,formatTime:formatTime,formatDate:formatDate,formatDateTime:formatDateTime}=Groundhogg.formatting;const{currentUser:currentUser}=Groundhogg;const selectContactModal=({onSelect:onSelect=()=>{},exclude:exclude=[]})=>{let search,timeout,results;const form=()=>{return`
		  <div id="search-form">
		  ...
		  </div>`};const quickEditMounted=({close:close,setContent:setContent})=>{let payload;const clearPayload=()=>{payload={data:{},meta:{},add_tags:[],remove_tags:[]}};clearPayload();const updateContact=data=>{payload={...data,data:{...payload.data,...data.data},meta:{...payload.meta,...data.meta}};console.log(payload)};const $quickEdit=$(".contact-quick-edit");$quickEdit.focus();$(`#${prefix}-save`).on("click",e=>{const $btn=$(e.target);$btn.prop("disabled",true);$btn.text(__("Saving","groundhogg"));const{stop:stop}=loadingDots(`#${prefix}-save`);ContactsStore.patch(contact.ID,payload).then(c=>{stop();clearPayload();onEdit(c);setContent(quickEdit(getContact()));quickEditMounted({close:close,setContent:setContent})})});$(`#${prefix}-cancel`).on("click",e=>{clearPayload();close()});tagPicker(`#${prefix}-tags`,true,items=>{TagsStore.itemsFetched(items)},{data:getContact().tags.map(t=>({id:t.ID,text:t.data.tag_name,selected:true}))}).on("select2:unselect",e=>{updateContact({add_tags:payload.add_tags.filter(tId=>tId!=e.params.data.id),remove_tags:[...payload.remove_tags,parseInt(e.params.data.id)]})}).on("select2:select",e=>{updateContact({add_tags:[...payload.add_tags,parseInt(e.params.data.id)].filter(tId=>!getContact().tags.find(t=>t.ID==tId)),remove_tags:payload.remove_tags.filter(tId=>tId!=e.params.data.id)})});$(`#${prefix}-first-name, #${prefix}-last-name, #${prefix}-email, #${prefix}-optin-status, #${prefix}-owner`).on("change",e=>{updateContact({data:{[e.target.name]:e.target.value}})});$(`#${prefix}-primary-phone, #${prefix}-primary-phone-extension, #${prefix}-mobile-phone`).on("change",e=>{updateContact({meta:{[e.target.name]:e.target.value}})})};const{close:close,setContent:setContent}=modal({content:quickEdit(getContact()),onOpen:quickEditMounted})};const addContactModal=({prefix:prefix="quick-add",onCreate:onCreate=()=>{}})=>{let method="quick-add";let selectedForm;const form=()=>{const quickAddForm=()=>{return`
      ...
      `};const onMount=({close:close,setContent:setContent})=>{let payload={data:{owner_id:currentUser.ID},meta:{}};const setPayload=data=>{payload={...payload,...data}};const reMount=()=>{setContent(form());onMount({close:close,setContent:setContent})};$(".use-form").on("click",e=>{method="form";reMount()});$(".use-quick-add").on("click",e=>{method="quick-add";reMount()});$(`.${prefix}-cancel`).on("click",close);if(method=="quick-add"){$(`#${prefix}-create`).on("click",({target:target})=>{if(!payload.data.email||!isValidEmail(payload.data.email)){errorDialog({message:__("A valid email is required!","groundhogg")});return}$(target).prop("disabled",true);const{stop:stop}=loadingDots(`#${prefix}-quick-add-button`);ContactsStore.post(payload).then(c=>{stop();close();onCreate(c)})});$(`
        #${prefix}-first-name,
        #${prefix}-last-name,
