($=>{const{modal:modal,errorDialog:errorDialog,select:select,input:input,isValidEmail:isValidEmail}=Groundhogg.element;const{contacts:ContactsStore}=Groundhogg.stores;const{tagPicker:tagPicker}=Groundhogg.pickers;const{sprintf:sprintf,__:__,_x:_x,_n:_n}=wp.i18n;const{formatNumber:formatNumber,formatTime:formatTime,formatDate:formatDate,formatDateTime:formatDateTime}=Groundhogg.formatting;const selectContactModal=({onSelect:onSelect=()=>{},exclude:exclude=[]})=>{let search,timeout,results;const form=()=>{return`
		  <div id="search-form">
      ...
      #${prefix}-mobile-phone`).on("change input",({target:target})=>{setPayload({meta:{...payload.meta,[target.name]:target.value}})});tagPicker(`#${prefix}-tags`).on("change",({target:target})=>{setPayload({tags:$(target).val()})})};onMount()};Groundhogg.components={addContactModal:addContactModal,selectContactModal:selectContactModal}})(jQuery);