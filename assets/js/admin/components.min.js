($=>{const{modal,errorDialog,loadingDots,select,uuid,specialChars,searchOptionsWidget,input,isNumeric,icons,dialog,tooltip,regexp,isValidEmail,loadingModal,adminPageURL}=Groundhogg.element;const{Div,H2,H4,Toggle,Img,An,Span,ModalFrame,Iframe,makeEl,Button,Modal,ModalWithHeader,Dashicon,ToolTip,Input,TinyMCE,Label,Fragment,ItemPicker,Skeleton,Pg,Form,Textarea,InputGroup}=MakeEl;const{contacts:ContactsStore,tags:TagsStore,forms:FormsStore,emails:EmailsStore}=Groundhogg.stores;const{post,routes,postFormData}=Groundhogg.api;const{tagPicker}=Groundhogg.pickers;const{userHasCap,getOwner}=Groundhogg.user;const{sprintf,__,_x,_n}=wp.i18n;const{formatDateTime}=Groundhogg.formatting;const{currentUser}=Groundhogg;const{maybeCall,debounce,jsonCopy}=Groundhogg.functions;const selectContactModal=({onSelect=()=>{},onClose=()=>{},exclude=[]})=>{Modal({dialogClasses:"no-padding",width:"400px",onOpen:e=>{document.getElementById("quick-search-input").focus()}},({close})=>QuickSearch({itemProps:contact=>({onClick:e=>{onSelect(contact);close()}}),queryOverrides:{limit:15,exclude:exclude}}))};const betterTagPicker=(el,{selected=[],removeTags=[],addTags=[],dates={},onChange=changes=>{}})=>{const $el=$(el);let timeout;const template=()=>{return`
          <div class="gh-tags">
              ${selected.map(tag=>`<span class="gh-tag${removeTags.includes(tag.ID)?" remove":""}">${tag.data.tag_name} ${dates.hasOwnProperty(tag.ID)?`<span class="gh-tooltip top">${dates[tag.ID]}</span>`:""}<span data-id="${tag.ID}" class="remove-tag dashicons dashicons-no-alt"></span></span>`).join("")}
              ...
              <div class="contact-quick-edit-fields gh-rows-and-columns">
                  ...
                  <div class="gh-row">
                      <div class="gh-col">
                          ...
                          ${select({id:`${prefix}-owner`,name:"owner_id"},Groundhogg.filters.owners.map(u=>({text:u.ID===currentUser.ID?__("Me","groundhogg"):u.data.display_name!==u.data.user_email?`${u.data.display_name} (${u.data.user_email})`:u.data.display_name,value:u.ID})),contact.data.owner_id)}
                      </div>
    ...
    #${prefix}-mobile-phone`).on("change input",({target})=>{setPayload({meta:{..._payload.meta,[target.name]:target.value}})});$(`
    #${prefix}-terms,
    #${prefix}-data-consent,
