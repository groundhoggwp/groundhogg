($=>{const{modal:modal,errorDialog:errorDialog,loadingDots:loadingDots,select:select,uuid:uuid,addMediaToBasicTinyMCE:addMediaToBasicTinyMCE,specialChars:specialChars,tinymceElement:tinymceElement,input:input,dialog:dialog,isValidEmail:isValidEmail,textarea:textarea}=Groundhogg.element;const{contacts:ContactsStore,tags:TagsStore}=Groundhogg.stores;const{post:post,routes:routes}=Groundhogg.api;const{tagPicker:tagPicker}=Groundhogg.pickers;const{sprintf:sprintf,__:__,_x:_x,_n:_n}=wp.i18n;const{formatNumber:formatNumber,formatTime:formatTime,formatDate:formatDate,formatDateTime:formatDateTime}=Groundhogg.formatting;const{currentUser:currentUser}=Groundhogg;const selectContactModal=({onSelect:onSelect=()=>{},exclude:exclude=[]})=>{let search,timeout,results;const form=()=>{return`
		  <div id="search-form">
		  ...
		  </div>`};const onMount=({close:close,setContent:setContent})=>{let payload={data:{owner_id:currentUser.ID},meta:{}};const setPayload=data=>{payload={...payload,...data}};$(`#${prefix}-cancel`).on("click",close);$(`#${prefix}-create`).on("click",({target:target})=>{if(!payload.data.email||!isValidEmail(payload.data.email)){errorDialog({message:__("A valid email is required!","groundhogg")});return}$(target).prop("disabled",true);const{stop:stop}=loadingDots(`#${prefix}-quick-add-button`);ContactsStore.post(payload).then(c=>{stop();close();onCreate(c)})});$(`
      #${prefix}-first-name,
      #${prefix}-last-name,
