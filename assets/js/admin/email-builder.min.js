($=>{const{el,objectToStyle,icons,inputWithReplacements,uuid,tinymceElement,specialChars,improveTinyMCE,textarea,modal,input,clickedIn,select,copyObject}=Groundhogg.element;const{formatNumber,formatTime,formatDate,formatDateTime}=Groundhogg.formatting;const{__,_x,_n,_nx,sprintf}=wp.i18n;const{linkPicker}=Groundhogg.pickers;const{api}=Groundhogg;improveTinyMCE({height:400});let fontWeights=["400","500","600","700","normal","bold","bolder"];let fontFamilies={"Arial, sans-serif":"Arial","Arial Black, Arial, sans-serif":"Arial Black","Century Gothic, Times, serif":"Century Gothic","Courier, monospace":"Courier","Courier New, monospace":"Courier New","Geneva, Tahoma, Verdana, sans-serif":"Geneva","Georgia, Times, Times New Roman, serif":"Georgia","Helvetica, Arial, sans-serif":"Helvetica","Lucida, Geneva, Verdana, sans-serif":"Lucida","Tahoma, Verdana, sans-serif":"Tahoma","Times, Times New Roman, Baskerville, Georgia, serif":"Times","Times New Roman, Times, Georgia, serif":"Times New Roman","Verdana, Geneva, sans-serif":"Verdana"};let MAX_EMAIL_WITH=600;const createBlock=(type,props={})=>{return{id:uuid(),type:type,...copyObject(BlockRegistry.blocks[type].defaults),...props}};const BlockEditor=(el,{email,onChange,emailControls,emailControlsOnMount,scrollDepth,onScroll,onMount})=>({$el:$(el),email:email,init(){$("head").append(`<style id="builder-style" type="text/css"></style>`);this.blocks=email.meta.blocks?email.meta.blocks:[createBlock("text",{content:email.data.content})];this.scrollDepth=scrollDepth;this.render()},editingBlock:false,templates:{css:()=>{},blockControls:block=>{return`
      ...
      `},controlsOnMount:({updateBlock})=>{$(".layout-choice").on("click",e=>{updateBlock({layout:e.currentTarget.dataset.layout},false,true)})},edit:({id,columns,style,layout:layout="two_columns"},editing)=>{return`
		  <table class="email-columns" border="0" cellpadding="0" cellspacing="0" width="100%">
			  <tr>
				  ${columnLayouts[layout](columns,editing)}
			  </tr>
		  </table>
      `},css:({columns})=>{return`${columns.map(col=>col.length?renderBlocksCSS(col):"").join("")}`},editOnMount:()=>{},html:({id,columns,style,layout:layout="two_columns"},editing)=>{return`
		  <table class="email-columns" border="0" cellpadding="0" cellspacing="0" width="100%">
			  <tr>
				  ${columnLayouts[layout](columns,editing)}
			  </tr>
      ...
      `},defaults:{layout:"two_columns",columns:[[],[],[]]}});registerBlock("text",__("Text"),{svg:`
		<svg xmlns="http://www.w3.org/2000/svg" style="enable-background:new 0 0 977.7 977.7" xml:space="preserve"
			...
			</div>`};return`${textControlGroup(__("Paragraphs"),"p",p)}
      ${textControlGroup(__("Heading 1"),"h1",h1)}
      ${textControlGroup(__("Heading 2"),"h2",h2)}
      ${textControlGroup(__("Heading 3"),"h3",h3)}`},controlsOnMount:({updateBlock,curBlock})=>{$(".font-control").on("change",({target})=>{let tag=target.dataset.tag;updateBlock({[tag]:{...curBlock()[tag],[target.name]:target.value}})})},edit:({id,content})=>{return`
      ...
      `},html:({content})=>{return`
		  <div style="text-align: left">
			  ${content}
		  </div>`},defaults:{content:`<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Proin egestas dolor non nulla varius, id fermentum ante euismod. Ut a sodales nisl, at maximus felis. Suspendisse potenti. Etiam fermentum magna nec diam lacinia, ut volutpat mauris accumsan. Nunc id convallis magna. Ut eleifend sem aliquet, volutpat sapien quis, condimentum leo.</p>`,p:{fontSize:14,fontWeight:"normal",fontFamily:"Arial, sans-serif"},h1:{fontSize:32,fontWeight:"500",fontFamily:"Arial, sans-serif"},h2:{fontSize:24,fontWeight:"500",fontFamily:"Arial, sans-serif"},h3:{fontSize:18,fontWeight:"500",fontFamily:"Arial, sans-serif"}}});registerBlock("image",__("Image"),{svg:icons.image,controls:({src,width,alt,title,align:align="center"})=>{return`
		  <div class="control-group gh-panel">
			  <div class="control-group-header space-between">
      ...
      `},css:()=>"",controlsOnMount:({updateBlock})=>{$("#align-left").on("click",()=>{updateBlock({align:"left"},false,true)});$("#align-center").on("click",()=>{updateBlock({align:"center"},false,true)});$("#align-right").on("click",()=>{updateBlock({align:"right"},false,true)});$("#image-src,#image-alt,#image-title").on("change",e=>{updateBlock({[e.target.name]:e.target.value})});var file_frame;$("#select-image").on("click",event=>{var picker=$(this);event.preventDefault();if(file_frame){file_frame.open();return}file_frame=wp.media.frames.file_frame=wp.media({title:__("Select a image to upload"),button:{text:__("Use this image")},multiple:false});file_frame.on("select",function(){var attachment=file_frame.state().get("selection").first().toJSON();console.log(attachment);let{height,width}=attachment;if(width>MAX_EMAIL_WITH){let ratio=height/width;width=MAX_EMAIL_WITH;height=width*ratio}updateBlock({src:attachment.url,alt:attachment.alt,title:attachment.title,width:width},false,true)});file_frame.open()})},edit:({src,width,height,alt:alt="",title:title="",align:align="center"})=>{return`
		  ...
		  </div>`},editOnMount:({id,updateBlock})=>{let $img=$(`[data-id=${id}] img`);setTimeout(()=>{$img.resizable({aspectRatio:true,maxWidth:600,stop:(e,ui)=>{updateBlock({width:ui.size.width})}})},100)},html:({src,width,height,alt:alt="",title:title="",align:align="center"})=>{return`
		  <div class="img-container" style="text-align: ${align}">
			  <img alt="${specialChars(alt)}" title="${specialChars(title)}" width="${width}" height="auto"
			       src="${src}" style="vertical-align: bottom"/>
		  </div>`},defaults:{src:"http://via.placeholder.com/600x338",alt:"placeholder image",title:"placeholder image",width:MAX_EMAIL_WITH,height:338,align:"center"}});registerBlock("button",__("Button"),{svg:`
      ...
      `},controlsOnMount:({updateBlock,curBlock})=>{$(".query-control").on("change",e=>{updateBlock({query:{...curBlock().query,[e.target.name]:e.target.value}})})},defaults:{query:{numberposts:5}}});Groundhogg.EmailBlockEditor=BlockEditor})(jQuery);