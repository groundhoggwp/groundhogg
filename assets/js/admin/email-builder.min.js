($=>{const{el,objectToStyle,icons,inputWithReplacements,uuid,tinymceElement,specialChars,improveTinyMCE,textarea,modal,input,clickedIn,select,copyObject}=Groundhogg.element;const{formatNumber,formatTime,formatDate,formatDateTime}=Groundhogg.formatting;const{__,_x,_n,_nx,sprintf}=wp.i18n;const{linkPicker}=Groundhogg.pickers;const{api}=Groundhogg;improveTinyMCE({height:400});let fontWeights=["400","500","600","700","normal","bold","bolder"];let fontFamilies={"Arial, sans-serif":"Arial","Arial Black, Arial, sans-serif":"Arial Black","Century Gothic, Times, serif":"Century Gothic","Courier, monospace":"Courier","Courier New, monospace":"Courier New","Geneva, Tahoma, Verdana, sans-serif":"Geneva","Georgia, Times, Times New Roman, serif":"Georgia","Helvetica, Arial, sans-serif":"Helvetica","Lucida, Geneva, Verdana, sans-serif":"Lucida","Tahoma, Verdana, sans-serif":"Tahoma","Times, Times New Roman, Baskerville, Georgia, serif":"Times","Times New Roman, Times, Georgia, serif":"Times New Roman","Verdana, Geneva, sans-serif":"Verdana"};const createBlock=(type,props={})=>{return{id:uuid(),type:type,...copyObject(BlockRegistry.blocks[type].defaults),...props}};const BlockEditor=(el,{email,onChange,emailControls,emailControlsOnMount,scrollDepth,onScroll,template,onMount})=>({$el:$(el),email:email,init(){$("head").append(`<style id="builder-style" type="text/css"></style>`);this.blocks=email.meta.blocks?email.meta.blocks:[createBlock("text",{content:email.data.content})];this.scrollDepth=scrollDepth;this.render()},editingBlock:false,templates:{css:()=>{},blockControls:block=>{const{design:design={}}=block;const{padding:padding={},margin:margin={},borderStyle:borderStyle="none",borderWidth:borderWidth={},borderRadius:borderRadius={}}=design;const boxInputGroup=({values:values={},prefix})=>{return`
              <div class="gh-input-group">
                  ${input({type:"number",name:"top",value:values.top,attr:prefix,className:`${prefix} design-attr full-width`})}
                  ${input({type:"number",name:"right",value:values.right,attr:prefix,className:`${prefix} design-attr full-width`})}
                  ${input({type:"number",name:"bottom",value:values.bottom,attr:prefix,className:`${prefix} design-attr full-width`})}
                  ${input({type:"number",name:"left",value:values.left,attr:prefix,className:`${prefix} design-attr full-width`})}
              </div>`};const designControls=()=>{return`
                  <div class="control">
                  ...
                  <div class="control space-between">
                      ...
                      ${select({id:"border-style"},{none:__("None"),solid:__("Solid"),dashed:__("Dashed"),dotted:__("Dotted")})}
                  </div>
                  <div class="control">
          ...
          `};return`
            <div class="block-controls">
                ...
                ${controlGroup({name:"Design",controls:designControls()})}
        ...
        <div class="builder-block is-editing" data-id="${block.id}" data-type="${block.type}">
            ...
            ${blockToolbar()}
        </div>`};const renderBlocksDesignCSS=blocks=>{return blocks.filter(b=>b.type).map((id,design)=>{return`
        [data-id=${id}] {
  ...
  <path color="currentColor"
        ...
        </svg>`,controls:({text,link,style,align,size})=>{return`
      ...
      `},controlsOnMount:({updateBlock,curBlock})=>{$(".query-control").on("change",e=>{updateBlock({query:{...curBlock().query,[e.target.name]:e.target.value}})})},defaults:{query:{numberposts:5}}});const controlGroup=({name:name="",controls:controls="",open:open=false})=>{return`
