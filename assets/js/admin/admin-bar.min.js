(function($){const classPrefix="groundhogg-toolbar-quick-search";const{moreMenu:moreMenu,input:input,select:select,tooltip:tooltip,loadingDots:loadingDots,isValidEmail:isValidEmail,dialog:dialog,errorDialog:errorDialog,clickedIn:clickedIn,inputWithReplacements:inputWithReplacements}=Groundhogg.element;const{tagPicker:tagPicker}=Groundhogg.pickers;const{contacts:ContactsStore}=Groundhogg.stores;const Tabs={search_contacts:{tooltip:`Search for contacts`,svg:`
		  ...
		  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
			...
			${renderTabs()}${Tabs[tab].view()}`;$quickSearch.html(html);$(`.${classPrefix}-tab-button`).on("click",({currentTarget:currentTarget})=>{tab=currentTarget.dataset.tab;mountQuickSearch()});Tabs[tab].onMount();Object.keys(Tabs).forEach(t=>tooltip(`#gh-tab-${t}`,{content:Tabs[t].tooltip,position:"bottom"}));$quickSearch.css({top:Math.min(bottom,window.innerHeight-$quickSearch.height()-20),left:right-$quickSearch.outerWidth()});$(`.${classPrefix}-close`).on("click",()=>{close()})};const $quickSearch=$(renderQuickSearch());$("body").append($quickSearch).addClass("groundhogg-toolbar-quick-search-open");$quickSearch.on("keydown",({key:key})=>{switch(key){case"Esc":case"Escape":close();break}});mountQuickSearch()})})})(jQuery);