(function($){const classPrefix="groundhogg-toolbar-quick-search";const{moreMenu:moreMenu,input:input,select:select,tooltip:tooltip,loadingDots:loadingDots,isValidEmail:isValidEmail,dialog:dialog,errorDialog:errorDialog,clickedIn:clickedIn,inputWithReplacements:inputWithReplacements}=Groundhogg.element;const{tagPicker:tagPicker}=Groundhogg.pickers;const{contacts:ContactsStore}=Groundhogg.stores;const Tabs={search_contacts:{tooltip:__(`Search for contacts`, 'groundhogg'),svg:`
		  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
			...
			<div id="quick-search-wrap">
				${input({type:"search",id:"quick-search-input",placeholder:__('Search by name, email, or phone...', 'groundhogg')
			})}
				<div class="${classPrefix}-results"></div>
			</div>`},onMount:()=>{const mountSearchResults=items=>{$(`.${classPrefix}-results`).replaceWith(renderSearchResults(items));$(`.${classPrefix}-result`).on("click",e=>{if(clickedIn(e,".email-contact")){e.stopPropagation();console.log("send-email");return}if(clickedIn(e,".call-primary")){e.preventDefault();console.log("call-primary");return}const ID=parseInt(e.currentTarget.dataset.contact);const contact=ContactsStore.get(ID);console.log(contact);window.location.href=contact.admin})};const renderSearchResult=item=>{return`
			  ...
			  </div>`};const renderSearchResults=(items=[])=>{if(!items||items.length===0){return`
				<div class="${classPrefix}-results"></div>`}return`
			  <div class="${classPrefix}-results">
				  ${items.map(item=>renderSearchResult(item)).join("")}
			  </div>`};let timeout;$("#quick-search-input").on("change input",({target:target})=>{if(timeout){clearTimeout(timeout)}timeout=setTimeout(()=>{ContactsStore.fetchItems({search:target.value,limit:10}).then(items=>{mountSearchResults(items)})},1e3)}).focus()}},create_contact:{tooltip:`Create a contact`,svg:`
		  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
			  <defs/>
			  <path fill="currentColor"
		  ...
		  </svg>`,view:()=>{const subClassPrefix=`${classPrefix}-quick-add`;return`
			<div class="gh-rows-and-columns">
				<div class="gh-row">
					...
					<div class="gh-col">
						<label for="${subClassPrefix}-last-name">${__('Email Address', 'groundhogg')}</label>
						${input({id:`${subClassPrefix}-last-name`,name:"last_name",placeholder:"Doe"})}
					</div>
				</div>
				<div class="gh-row">
					<div class="gh-col">
						<label for="${subClassPrefix}-email">${__('First Name', 'groundhogg')}</label>
				...
				<div class="gh-row">
					<div class="gh-col">
						...
						${select({id:`${subClassPrefix}-tags`,multiple:true,dataPlaceholder:__('Type to select tags...', 'groundhogg')})}
					</div>
				</div>
				<div class="gh-row">
					<div class="gh-col">
						<button id="${classPrefix}-quick-add-button" class="gh-button primary">${__('Create Contact', 'groundhogg')}						</button>
					</div>
				</div>
			</div>`},onMount:()=>{const subClassPrefix=`${classPrefix}-quick-add`;let payload={data:{}};const setPayload=data=>{payload={...payload,...data};console.log(payload)};$(`#${classPrefix}-quick-add-button`).on("click",({target:target})=>{if(!payload.data.email||!isValidEmail(payload.data.email)){errorDialog({message:__('A valid email is required!', 'groundhogg')});return}$(target).prop("disabled",true);const{stop:stop}=loadingDots(`#${classPrefix}-quick-add-button`);ContactsStore.post(payload).then(c=>{stop();window.location.href=c.admin})});$(`#${subClassPrefix}-first-name, #${subClassPrefix}-last-name, #${subClassPrefix}-email`).on("change input",({target:target})=>{setPayload({data:{...payload.data,[target.name]:target.value}})});tagPicker(`#${subClassPrefix}-tags`).on("change",({target:target})=>{setPayload({tags:$(target).val()})})}},send_email:{tooltip:`Send an email`,svg:`
		  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
			  <path fill="currentColor"
			        d="M467 61H45a45 45 0 00-45 45v300a45 45 0 0045 45h422a45 45 0 0045-45V106a45 45 0 00-45-45zm-6.2 30L257 294.8 51.4 91h409.4zM30 399.8V112l144.5 143.2L30 399.8zM51.2 421l144.6-144.6 50.6 50.3a15 15 0 0021.2 0l49.4-49.5L460.8 421H51.2zM482 399.8L338.2 256 482 112.2v287.6z"/>
		  </svg>`,view:()=>{return`
				...
				<div class="gh-row">
					<div class="gh-col">
						<div class="gh-input-inline-label">
							...
							${inputWithReplacements({id:"subject-line"})}
						</div>
					</div>
				</div>
				<div class="gh-row"></div>
			</div>`},onMount:()=>{}},broadcast:{tooltip:`Send a broadcast`,svg:`
		  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
			  <path fill="currentColor"
			...
			<div id="send-broadcast"></div>`},onMount:({setTab:setTab})=>{Groundhogg.SendBroadcast("#send-broadcast",{},{onScheduled:()=>{setTab("broadcast")}})}}};$(()=>{const $menuItem=$("#wp-admin-bar-groundhogg");let openFlag=false;let tab="search_contacts";const close=()=>{openFlag=false;$("#groundhogg-toolbar-quick-search").remove();$("body").removeClass("groundhogg-toolbar-quick-search-open")};$menuItem.on("click",e=>{if(openFlag){close();return}openFlag=true;const{right:right,bottom:bottom}=e.currentTarget.getBoundingClientRect();const renderTabs=()=>{const renderTab=(t,props)=>{return`
