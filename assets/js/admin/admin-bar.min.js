(function($){const classPrefix="groundhogg-toolbar-quick-search";const{moreMenu:moreMenu,input:input,select:select,tooltip:tooltip,loadingDots:loadingDots,isValidEmail:isValidEmail,dialog:dialog,errorDialog:errorDialog,clickedIn:clickedIn,icons:icons,adminPageURL:adminPageURL}=Groundhogg.element;const{userHasCap:userHasCap}=Groundhogg.user;const{sprintf:sprintf,__:__,_x:_x,_n:_n}=wp.i18n;const{formatNumber:formatNumber,formatTime:formatTime,formatDate:formatDate,formatDateTime:formatDateTime}=Groundhogg.formatting;const{tagPicker:tagPicker}=Groundhogg.pickers;const{contacts:ContactsStore}=Groundhogg.stores;const Tabs={search_contacts:{tooltip:`Search for contacts`,cap:"view_contacts",svg:icons.contactSearch,view:()=>{return`
			...
			</div>`},onMount:()=>{const mountSearchResults=(items,search)=>{$(`.${classPrefix}-results`).replaceWith(renderSearchResults(items,search));tooltip(`.${classPrefix}-result .edit-profile`,{content:__("Edit profile","groundhogg"),position:"top"});tooltip(`.${classPrefix}-result .send-email`,{content:__("Send email","groundhogg"),position:"top"});tooltip(`.${classPrefix}-result .call-primary`,{content:__("Call","groundhogg"),position:"top"});tooltip(`.${classPrefix}-result .call-mobile`,{content:__("Call mobile","groundhogg"),position:"top"});$(`.${classPrefix}-result`).on("click",e=>{if(clickedIn(e,".email-contact")){return}if(clickedIn(e,".call-primary")){return}if(clickedIn(e,".call-mobile")){return}const ID=parseInt(e.currentTarget.dataset.contact);const contact=ContactsStore.get(ID);window.location.href=contact.admin})};const renderSearchResult=item=>{return`
			...
			<div class="gh-rows-and-columns">
				...
				<div class="gh-row phone">
					<div class="cghol">
						...
						${input({type:"tel",id:`${subClassPrefix}-primary-phone`,name:"primary_phone"})}
					</div>
					<div class="primary-phone-ext">
						<label
							for="quick-edit-primary-phone-extension">${_x("Ext.","phone number extension","groundhogg")}</label>
						${input({type:"number",id:`${subClassPrefix}-primary-phone-ext`,name:"primary_phone_extension"})}
					</div>
				</div>
				<div class="gh-row">
					<div class="gh-col">
						<label for="quick-edit-mobile-phone">${__("Mobile Phone","groundhogg")}</label>
						${input({type:"tel",id:`${subClassPrefix}-mobile-phone`,name:"mobile_phone"})}
					</div>
				</div>
				<div class="gh-row">
					<div class="gh-col">
						<label for="${subClassPrefix}-tags">${__("Tags","groundhogg")}</label>
						${select({id:`${subClassPrefix}-tags`,multiple:true,dataPlaceholder:__("Type to select tags...","groundhogg"),style:{width:"100%"}})}
					</div>
				</div>
				<div class="gh-row">
					<div class="gh-col">
			...
			</div>`},onMount:()=>{const subClassPrefix=`${classPrefix}-quick-add`;let payload={data:{},meta:{}};const setPayload=data=>{payload={...payload,...data}};$(`#${classPrefix}-quick-add-button`).on("click",({target:target})=>{if(!payload.data.email||!isValidEmail(payload.data.email)){errorDialog({message:__("A valid email is required!","groundhogg")});return}$(target).prop("disabled",true);const{stop:stop}=loadingDots(`#${classPrefix}-quick-add-button`);ContactsStore.post(payload).then(c=>{stop();window.location.href=c.admin})});$(`
        #${subClassPrefix}-first-name, 
        #${subClassPrefix}-last-name, 
        #${subClassPrefix}-email`).on("change input",({target:target})=>{setPayload({data:{...payload.data,[target.name]:target.value}})});$(`
        #${subClassPrefix}-primary-phone,
        #${subClassPrefix}-primary-phone-ext,
        #${subClassPrefix}-mobile-phone`).on("change input",({target:target})=>{setPayload({meta:{...payload.meta,[target.name]:target.value}})});tagPicker(`#${subClassPrefix}-tags`).on("change",({target:target})=>{setPayload({tags:$(target).val()})})}},broadcast:{cap:"schedule_broadcasts",tooltip:`Send a broadcast`,svg:icons.megaphone,view:()=>{return`
