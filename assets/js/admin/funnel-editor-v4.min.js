(function(Funnel,$){const{tags:TagsStore,emails:EmailsStore,funnels:FunnelsStore,campaigns:CampaignsStore,contacts:ContactsStore,searches:SearchesStore}=Groundhogg.stores;const{stepTypeSelect}=Groundhogg;const{get:apiGet,post:apiPost,delete:apiDelete,patch:apiPatch,routes}=Groundhogg.api;const{formatNumber,formatTime,formatDate,formatDateTime}=Groundhogg.formatting;const{v4:apiRoutes}=routes;const{createFilters}=Groundhogg.filters.functions;const{icons,improveTinyMCE,buttonToggle,dialog,confirmationModal,dangerConfirmationModal,loadingModal,modal,button,select,input,textarea,objectEquals,copyObject,uniqid,specialChars,regexp,breadcrumbs,adminPageURL,tooltip,toggle,loadingDots,savingModal,bold,moreMenu,clickInsideElement,progressBar}=Groundhogg.element;const{campaignPicker,emailPicker,searchesPicker}=Groundhogg.pickers;const{StepTypes,StepPacks}=Groundhogg;const{__,_x,_n,_nx,sprintf}=wp.i18n;const toEditorButton=()=>{return`
  ...
  <path fill="currentColor" d="M448 24H224a40 40 0 00-40 40v32a24 24 0 0048 0V72h208v368H232v-24a24 24 0 00-48 0v32a40 40 0 0040 40h224a40 40 0 0040-40V64a40 40 0 00-40-40z"/>
        ...
        `,confirmText:__("Delete"),onConfirm:()=>{removeStep()}})}else{removeStep()}},getStep(stepId){return this.funnel.steps.find(step=>step.ID===stepId)},getSteps(){return this.funnel.steps},getCurrentStep(){return this.getStep(this.activeStep)},updateStep(stepId,newData){const step=this.getStep(stepId);const newStep={...step,...newData};newStep.data.step_title=StepTypes.getType(newStep.data.step_type).title(newStep);const toReplace=this.funnel.steps.findIndex(step=>step.ID===stepId);this.autoSaveEditedFunnel();this.saveUndoState();if(toReplace!==-1){this.funnel.steps[toReplace]=newStep}this.renderStepFlow();return newStep},updateCurrentStep(newData,reRenderStepEdit=false){const step=this.updateStep(this.activeStep,newData);if(reRenderStepEdit){this.renderStepEdit()}return step},updateCurrentStepData(newData,reRenderStepEdit=false){const{data,meta}=this.getCurrentStep();const step=this.updateStep(this.activeStep,{data:{...data,...newData}});if(reRenderStepEdit){this.renderStepEdit()}return step},updateCurrentStepMeta(newMeta,reRenderStepEdit=false){const{data,meta}=this.getCurrentStep();const step=this.updateStep(this.activeStep,{meta:{...meta,...newMeta}});if(reRenderStepEdit){this.renderStepEdit()}return step},autoSaveTimeout:null,abortController:null,autoSaveEditedFunnel(){const self=this;if(this.autoSaveTimeout){clearTimeout(this.autoSaveTimeout)}this.autoSaveTimeout=setTimeout(()=>{self.autoSaveTimeout=null;self.abortController=new AbortController;const{signal}=self.abortController;FunnelsStore.patchMeta(this.funnel.ID,{edited:{steps:self.funnel.steps,title:self.funnel.data.title}},{signal:signal}).then(data=>{self.setLastSaved();self.abortController=null})},3e3)},...Funnel};$(function(){Editor.init()});fill("beforeStepNotes.send_email",{render({ID,meta}){const{email_id}=meta;const email=EmailsStore.get(parseInt(email_id));if(!email_id||!email){return""}return`
		  ...
		  <div class="panel">
			  <div class="row">
				  <div class="column">
					  ...
					  ${select({id:"email-picker-right",name:"email_id"},EmailsStore.getItems().map(item=>{return{text:item.data.title,value:item.ID}}),email&&email.ID)}
				  </div>
			  </div>
			  <div class="row">
				  <div class="column">
      ...
      `},onMount({ID,meta},updateStepMeta){const{email_id}=meta;const email=EmailsStore.get(parseInt(email_id));if(!email_id||!email){return}$("#edit-email-right").on("click",()=>{Editor.renderEmailEditor(email)});$("#add-new-email-right").on("click",()=>{Editor.renderEmailTemplatePicker(updateStepMeta)});emailPicker("#email-picker-right",false,items=>EmailsStore.itemsFetched(items),{},{width:"100%"}).on("change",e=>{updateStepMeta({email_id:e.target.value},true)})}});fill("beforeStepNotes.form_fill",{render({ID,meta}){if(!stepIsReal(ID)){return""}const copyValue=toCopy=>{return`
			<div class="gh-input-group">${input({className:"code",value:toCopy,onfocus:"this.select()",readonly:true})}
