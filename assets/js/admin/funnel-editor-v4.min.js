(function(Funnel,$){const TagsStore=Groundhogg.stores.tags;const EmailsStore=Groundhogg.stores.emails;const apiGet=Groundhogg.api.get;const apiPost=Groundhogg.api.post;const apiRoutes=Groundhogg.api.routes.v4;const{tinymceElement,confirmationModal,loadingModal,modal,select,input,textarea,toggle,textAreaWithReplacements,textAreaWithReplacementsAndEmojis}=Groundhogg.element;const{formBuilder}=Groundhogg;const{linkPicker,emailPicker,tagPicker}=Groundhogg.pickers;$.fn.serializeFormJSON=function(){var o={};var a=this.serializeArray();$.each(a,function(){if(o[this.name]){if(!o[this.name].push){o[this.name]=[o[this.name]]}o[this.name].push(this.value||"")}else{o[this.name]=this.value||""}});return o};const slot=(name,...args)=>{return SlotFillProvider.slot(name,...args)};const fill=(name,component)=>{return SlotFillProvider.fill(name,component)};const slotsMounted=()=>{return SlotFillProvider.slotsMounted()};const slotsDemounted=()=>{return SlotFillProvider.slotsDemounted()};const stepIsReal=stepId=>{return Editor.origFunnel.steps.find(step=>step.ID===stepId)};const SlotFillProvider={fills:[],_slotsMounted:[],_slotsDemounted:[],slot(slotName,...args){this._slotsMounted.push({name:slotName,args:args});return this.fills.filter(fill=>fill.slot===slotName).map(fill=>fill.render(...args)).join("")},slotsMounted(){let slot;while(this._slotsMounted.length>0){slot=this._slotsMounted.pop();this.fills.filter(fill=>fill.slot===slot.name).forEach(fill=>{fill.onMount(...slot.args)});this._slotsDemounted.push(slot)}},slotsDemounted(){let slot;while(this._slotsDemounted.length>0){slot=this._slotsDemounted.pop();this.fills.filter(fill=>fill.slot===slot.name).forEach(fill=>{fill.onDemount(...slot.args)})}},fill(slot,component){this.fills.push({slot:slot,...{render(){},onMount(){},onDemount(){},...component}})}};const getStepType=type=>{return Editor.stepTypes.hasOwnProperty(type)?Editor.stepTypes[type]:StepTypes.getType("error")};const Editor={activeAddType:"actions",view:"addingStep",activeStep:{},htmlModules:{},isEditingTitle:false,stepFlowContextMenu:null,stepOpenInContextMenu:null,funnel:{ID:0,data:{},steps:[]},origFunnel:{ID:0,data:{},steps:[]},stepTypes:{},undoStates:[],redoStates:[],stepErrors:{},funnelErrors:[],htmlTemplates:{publishActions(status){if(status==="inactive"){return`
			  ...
			  </button>`}},funnelTitleEdit(title,isEditing){if(isEditing){return`<input type="text" class="funnel-title-edit regular-text" id="funnel-title-edit" name="funnel_title"
		                 value="${specialChars(title)}">`}else{return`<span class="title-inner">${specialChars(title)}</span><span class="pencil"><span class="dashicons dashicons-edit"></span></span>`}},stepEditPanel(step){const{ID,data,meta}=step;const{step_type,step_title,step_group}=data;const StepType=getStepType(step_type);let hasErrors=false;let errors=[];if(Editor.stepErrors.hasOwnProperty(ID)&&Editor.stepErrors[ID].length>0){hasErrors=true;errors=Editor.stepErrors[ID]}const updateStepMeta=(meta,reRenderStepEdit=false)=>{return Editor.updateCurrentStepMeta(meta,reRenderStepEdit)};const updateStep=(data,reRenderStepEdit=false)=>{return Editor.updateCurrentStep(data,reRenderStepEdit)};const slotArgs=[step,updateStepMeta,updateStep];return`
			...
			</div>`},stepAddPanel(activeType){return`
			<div class="step-add">
			...
			</div>`},stepTypeSelect(type){return`
			<div class="type-select">
        ...
        `}},init(){this.loadingClose=loadingModal().close;var self=this;var $doc=$(document);this.loadFunnel(this.funnel);$doc.on("click",".step-add .select-type",function(){self.saveUndoState();self.activeAddType=$(this).data("type");self.renderStepAdd()});$doc.on("mouseleave",".step-flow .steps .step",function(e){const $step=$(this);$(".step-menu ul",$step).hide()});$doc.on("click",".step-flow .steps .step",function(e){const $step=$(this);const step=self.funnel.steps.find(step=>step.ID===parseInt($step.data("id")));switch(true){case $(e.target).is(".step-menu-move-up"):self.moveStepUp(step);break;case $(e.target).is(".step-menu-move-down"):self.moveStepDown(step);break;case $(e.target).is(".step-menu-new-step-before"):self.insertPlaceholderStep(step,"before");break;case $(e.target).is(".step-menu-new-step-after"):self.insertPlaceholderStep(step,"after");break;case $(e.target).is(".step-menu-duplicate"):const newStep=copyObject(step);newStep.ID=uniqid();self.addStep(newStep);break;case $(e.target).is(".step-menu-delete"):self.deleteStep(parseInt($step.data("id")));break;case $(e.target).is(".step-menu")||$(e.target).parent(".step-menu").length>0:const $menu=$(".step-menu ul",$step).toggle();const p=$menu.offset();const h=$menu.outerHeight();if(p.top+h+100>$(window).height()){$menu.css({bottom:"90%",top:"auto"})}else{$menu.css({top:"90%",bottom:"auto"})}break;case $(e.target).is(".step-menu-edit"):default:const clickedStep=parseInt($step.data("id"));if(clickedStep===self.activeStep){return}self.saveUndoState();self.previousActiveStep=self.activeStep;self.activeStep=clickedStep;self.view="editingStep";self.renderStepFlow();self.renderStepEdit()}});$doc.on("click",".step-flow .add-new-step",function(){self.activeStep=null;self.view="addingStep";self.renderStepFlow();self.renderStepAdd()});$doc.on("click",".undo-and-redo .undo",function(){self.undo()});$doc.on("click",".undo-and-redo .redo",function(){self.redo()});$doc.on("click",".header-stuff .title",function(){if(!self.isEditingTitle){self.isEditingTitle=true;self.renderTitle()}});$doc.on("click",".publish-actions .deactivate",function(){confirmationModal({alert:`<p><b>Are you sure you want to deactivate the funnel?</b></p>
		  <p>Any pending events will be paused and contacts will not be able to move forward in the funnel.</p>
		  <p>Reactivating the funnel will restart any paused events.</p>`,onConfirm:()=>{self.deactivate()}})});$doc.on("click",".publish-actions .update-and-launch, .publish-actions .update",function(){confirmationModal({alert:`<p><b>Are you sure you want to commit these changes?</b></p><p>The changes made will take immediate
			  effect to anyone currently in the funnel.</p>`,onConfirm:()=>{self.commitChanges()}})});$(document).on("tinymce-editor-setup",function(event,editor){editor.settings.toolbar1="formatselect,bold,italic,bullist,numlist,blockquote,alignleft,aligncenter,alignright,link,spellchecker,wp_adv,dfw,groundhoggreplacementbtn,groundhoggemojibtn";editor.settings.toolbar2="strikethrough,hr,forecolor,pastetext,removeformat,charmap,outdent,indent,undo,redo,wp_help";editor.settings.height=200;editor.on("click",function(ed,e){$(document).trigger("to_mce")})});$doc.on("blur change keydown",".funnel-title-edit",function(e){if(e.type==="keydown"&&e.keyCode!==13){self.resizeTitleEdit();return}self.saveUndoState();self.funnel.data.title=e.target.value;self.isEditingTitle=false;self.update({data:{title:e.target.value}},false);self.renderTitle()});self.setupSortable();self.setupStepTypes();self.maybePreloadTagsAndEmails()},render(){this.renderTitle();this.renderPublishActions();this.renderStepFlow();this.renderStepAdd();this.renderStepEdit();this.loadingClose()},async maybePreloadTagsAndEmails(){const preloadTags=[];const preloadEmails=[];this.funnel.steps.forEach(({meta})=>{const{tags,email_id}=meta;if(tags){preloadTags.push(...tags)}else if(email_id){preloadEmails.push(email_id)}});if(preloadEmails.length===0&&preloadTags.length===0){this.render();return}if(preloadTags.length>0){await TagsStore.fetchItems({include:preloadTags})}if(preloadEmails.length>0){await EmailsStore.fetchItems({include:preloadEmails})}this.render()},loadFunnel(funnel){this.funnel=funnel;this.origFunnel=copyObject(funnel);if(this.funnel.meta.edited){this.funnel.steps=this.funnel.meta.edited.steps}},setupSortable(){var self=this;$(".step-flow .steps").sortable({placeholder:"step-placeholder",cancel:".text-helper",start:function(e,ui){ui.placeholder.height(ui.item.height());ui.placeholder.width(ui.item.width())},receive:function(e,ui){self.saveUndoState();var type=$(ui.helper).data("type");var group=$(ui.helper).data("group");var id=Date.now();$(ui.helper).addClass("step");$(ui.helper).data("id",id);self.addStep({ID:id,data:{ID:id,funnel_id:Editor.funnel.ID,step_title:Editor.stepTypes[type].name,step_type:type,step_group:group,step_order:$(ui.helper).prevAll(".step").length},meta:StepTypes.getType(type).defaults})},update:function(e,ui){self.saveUndoState();self.syncOrderWithFlow();self.autoSaveEditedFunnel();self.renderStepFlow();self.renderStepEdit()}}).disableSelection()},setupStepTypes(){for(var prop in this.stepTypes){if(Object.prototype.hasOwnProperty.call(this.stepTypes,prop)&&Object.prototype.hasOwnProperty.call(StepTypes,prop)){Object.assign(this.stepTypes[prop],{...StepTypes.default,...StepTypes[prop]});Object.assign(StepTypes[prop],{...this.stepTypes[prop]})}else{Object.assign(this.stepTypes[prop],StepTypes.default)}}},getStepType(type){},initStepFlowContextMenu(){var self=this;this.stepFlowContextMenu=createContextMenu({menuClassName:"step-context-menu",targetSelector:".step-flow .steps .step",items:[{key:"duplicate",text:"Duplicate"},{key:"delete",text:"Delete"}],onOpen(e,el){self.stepOpenInContextMenu=parseInt(el.dataset.id)},onSelect(key){switch(key){case"delete":self.deleteStep(self.stepOpenInContextMenu);break;case"duplicate":const stepToCopy=self.funnel.steps.find(step=>step.ID===self.stepOpenInContextMenu);const newStep=copyObject(stepToCopy);newStep.ID=uniqid();self.addStep(newStep);break}}});this.stepFlowContextMenu.init()},renderStepFlow(){var self=this;this.checkForStepErrors();var steps=this.funnel.steps.sort((a,b)=>a.data.step_order-b.data.step_order).map(step=>self.htmlTemplates.stepFlowCard(step,self.activeStep)).join("");$(".step-flow .steps").html(steps);this.renderPublishActions()},addStepError(id,error){if(!this.stepErrors.hasOwnProperty(id)){this.stepErrors[id]=[]}this.stepErrors[id].push(error)},checkForStepErrors(){const self=this;this.stepErrors=[];this.funnel.steps.forEach(step=>{const errors=[];const{step_group,step_order,step_type}=step.data;const typeHandler=getStepType(step_type);if(step_group==="action"&&step_order===1){errors.push("Actions cannot be at the start of a funnel.")}else if(typeHandler.type==="error"){errors.push("Settings not found.")}if(typeHandler){typeHandler.validate(step,errors)}errors.forEach(error=>this.addStepError(step.ID,error))})},mountStep(step){step=step||this.funnel.steps.find(s=>s.ID===this.activeStep);if(!step){return}const updateStepMeta=(meta,reRenderStepEdit=false)=>{return this.updateCurrentStepMeta(meta,reRenderStepEdit)};const updateStep=(data,reRenderStepEdit=false)=>{return this.updateCurrentStep(data,reRenderStepEdit)};const StepType=getStepType(step.data.step_type);StepType.onMount(step,updateStepMeta,updateStep);this.lastStepEditMounted=this.activeStep},demountStep(step){step=step||this.funnel.steps.find(s=>s.ID===this.lastStepEditMounted);if(!step){return}const updateStepMeta=meta=>{return this.updateCurrentStepMeta(meta)};const updateStep=data=>{return this.updateCurrentStep(data)};const StepType=getStepType(step.data.step_type);StepType.onDemount(step,updateStepMeta,updateStep);this.lastStepEditMounted=null},renderStepEdit(){if(this.view!=="editingStep"){return}var self=this;const step=this.funnel.steps.find(step=>step.ID===this.activeStep);const previousStep=this.funnel.steps.find(step=>step.ID===this.previousActiveStep);if(this.activeStep===this.lastStepEditMounted){this.demountStep(step)}else if(previousStep){this.demountStep(previousStep)}slotsDemounted();$("#control-panel").html(this.htmlTemplates.stepEditPanel(step));this.mountStep(step);$("#step-notes").on("change",function(e){self.updateCurrentStepMeta({step_notes:$(this).val()})});slotsMounted()},renderStepAdd(){if(this.view!=="addingStep"){return}const self=this;self.demountStep();$("#control-panel").html(self.htmlTemplates.stepAddPanel(self.activeAddType));self.renderStepFlow();if(self.addingStepOrder){$(".add-step").on("click",function(){const $button=$(this);var type=$button.data("type");var group=$button.data("group");var id=Date.now();self.addStep({ID:id,data:{ID:id,funnel_id:Editor.funnel.ID,step_title:Editor.stepTypes[type].name,step_type:type,step_group:group,step_order:self.addingStepOrder},meta:StepTypes.getType(type).defaults})})}else{$(".add-step").draggable({connectToSortable:".step-flow .steps",helper:"clone",revert:"invalid",revertDuration:0})}},renderTitle(){$(".header-stuff .title").html(this.htmlTemplates.funnelTitleEdit(this.funnel.data.title,this.isEditingTitle));if(this.isEditingTitle){$(".funnel-title-edit").focus();this.resizeTitleEdit()}},resizeTitleEdit(){$(".funnel-title-edit").width(this.funnel.data.title.length+1+"ch")},renderPublishActions(){$(".publish-actions").html(this.htmlTemplates.publishActions(this.funnel.data.status))},syncOrderWithFlow(){var self=this;$(".step-flow .steps .step").each(function(i){self.funnel.steps.find(step=>step.ID===$(this).data("id")).data.step_order=i+1});this.fixStepOrders()},saveUndoState(){const{view,funnel,activeStep,activeAddType,isEditingTitle}=this;this.undoStates.push({view:view,activeStep:activeStep,isEditingTitle:isEditingTitle,activeAddType:activeAddType,funnel:copyObject(funnel)})},undo(){var lastState=this.undoStates.pop();if(!lastState){return}Object.assign(this,lastState);this.redoStates.push(lastState);this.render()},redo(){var lastState=this.redoStates.pop();if(!lastState){return}Object.assign(this,lastState);this.undoStates.push(lastState);this.render()},update(data,reload=true){var self=this;return apiPost(`${apiRoutes.funnels}/${self.funnel.ID}`,data).then(data=>{self.setLastSaved();if(data.item&&reload){self.loadFunnel(data.item);self.render()}})},activate(){const{close}=modal({content:"<p>Launching...</p>",canClose:false});this.update({data:{status:"active"}}).then(()=>close())},deactivate(){const{close}=modal({content:"<h1>Deactivating...</h1>",canClose:false});this.update({data:{status:"inactive"}}).then(()=>close())},commitChanges(){var self=this;if(objectEquals(this.funnel.steps,this.origFunnel.steps)){return this.activate()}if(this.autoSaveTimeout){clearTimeout(this.autoSaveTimeout)}else if(this.abortController){this.abortController.abort()}const{close}=modal({content:"<h1>Saving...</h1>",canClose:false});apiPost(`${apiRoutes.funnels}/${self.funnel.ID}/commit`,{edited:{steps:self.funnel.steps}}).then(data=>{self.setLastSaved();if(data.item){self.loadFunnel(data.item);self.render()}else if(data.code==="error"){const{errors}=data.data.data;const errorHTML=errors.map(({errors,error_data})=>{return`<p>${Object.keys(errors).map(code=>errors[code][0])}</p>`});errors.forEach(error=>{const{errors,error_data}=error;for(const code in errors){if(errors.hasOwnProperty(code)){self.addStepError(error_data[code].step.ID,errors[code][0])}}});self.render();confirmationModal({alert:`<p>Your funnel could not be launched due to <b>${errors.length}</b> errors. Please rectify the
				following errors and try again.</p>
			<div class="commit-errors">${errorHTML}</div>`})}}).then(()=>close())},setLastSaved(){clearInterval(self.lastSavedTimer);self.lastSavedTimer=setInterval(this.updateLastSaved,30*1e3,new Date);this.updateLastSaved(new Date)},updateLastSaved(lastSaved){var delta=Math.round((+new Date-lastSaved)/1e3);var minute=60,hour=minute*60,day=hour*24,week=day*7;var fuzzy="Saved ";if(delta<30){fuzzy+="just now"}else if(delta<minute){fuzzy+=delta+" seconds ago"}else if(delta<2*minute){fuzzy+="a minute ago"}else if(delta<hour){fuzzy+=Math.floor(delta/minute)+" minutes ago."}else if(Math.floor(delta/hour)==1){fuzzy+="1 hour ago"}else if(delta<day){fuzzy=Math.floor(delta/hour)+" hours ago."}else if(delta<day*2){fuzzy+="yesterday"}$(".header-actions").attr("data-lastSaved",fuzzy)},addStep(step){if(!step){return}this.saveUndoState();this.funnel.steps.push(step);this.fixStepOrders();delete this.addingStepOrder;this.renderStepFlow();this.autoSaveEditedFunnel()},moveStep(step,direction){if(!step){return}const move="up"===direction?-1.1:1.1;this.saveUndoState();step.data.step_order=step.data.step_order+move;window.console.log("steps",this.funnel.steps);this.fixStepOrders();this.renderStepFlow();this.autoSaveEditedFunnel()},moveStepUp(step){this.moveStep(step,"up")},moveStepDown(step){this.moveStep(step,"down")},insertPlaceholderStep(step,beforeAfter){const self=this;self.previousActiveStep=step.ID;self.view="addingStep";self.addingStepOrder="before"===beforeAfter?parseInt(step.data.step_order)-.1:parseInt(step.data.step_order)+.1;self.renderStepFlow();self.renderStepAdd();const $html=$(`<div class="step-placeholder">Choose a step to add here &rarr;<button type="button" class="button button-secondary">Cancel</button></div>`);$("button",$html).on("click",function(){delete self.addingStepOrder;$html.remove();self.activeStep=self.previousActiveStep;self.view="editingStep";self.renderStepFlow();self.renderStepEdit()});if("before"===beforeAfter){$html.insertBefore(`.steps [data-id="${step.ID}"]`)}else{$html.insertAfter(`.steps [data-id="${step.ID}"]`)}},fixStepOrders(){let newOrder=1;this.funnel.steps.sort((a,b)=>a.data.step_order-b.data.step_order).forEach(step=>{step.data.step_order=newOrder;newOrder++})},deleteStep(stepId){if(!stepId){return}const removeStep=()=>{this.saveUndoState();this.funnel.steps=this.funnel.steps.filter(step=>step.ID!==stepId);this.fixStepOrders();this.renderStepFlow();if(this.activeStep===stepId){this.view="addingStep";this.renderStepAdd();this.activeStep=null}this.autoSaveEditedFunnel()};const origStep=Editor.origFunnel.steps.find(s=>s.ID===stepId);if(origStep){confirmationModal({alert:`
        ...
        `,confirmText:"Delete",onConfirm:()=>{removeStep()}})}else{removeStep()}},getStep(stepId){return this.funnel.steps.find(step=>step.ID===stepId)},getSteps(){return this.funnel.steps},getCurrentStep(){return this.getStep(this.activeStep)},updateStep(stepId,newData){const step=this.getStep(stepId);const newStep={...step,...newData};newStep.data.step_title=StepTypes.getType(newStep.data.step_type).title(newStep);var toReplace=this.funnel.steps.findIndex(step=>step.ID===stepId);this.autoSaveEditedFunnel();this.saveUndoState();if(toReplace!==-1){this.funnel.steps[toReplace]=newStep}this.renderStepFlow()},updateCurrentStep(newData,reRenderStepEdit=false){this.updateStep(this.activeStep,newData);if(reRenderStepEdit){this.renderStepEdit()}},updateCurrentStepMeta(newMeta,reRenderStepEdit=false){const{data,meta}=this.getCurrentStep();this.updateStep(this.activeStep,{meta:{...meta,...newMeta}});if(reRenderStepEdit){this.renderStepEdit()}},autoSaveTimeout:null,abortController:null,autoSaveEditedFunnel(){var self=this;if(this.autoSaveTimeout){clearTimeout(this.autoSaveTimeout)}this.autoSaveTimeout=setTimeout(()=>{self.autoSaveTimeout=null;self.abortController=new AbortController;const{signal}=self.abortController;apiPost(`${apiRoutes.funnels}/${self.funnel.ID}/meta`,{edited:{steps:self.funnel.steps,title:self.funnel.data.title}},{signal:signal}).then(data=>{self.setLastSaved();self.abortController=null})},3e3)},...Funnel};$(function(){Editor.init()});function copyObject(object,initial){initial=initial||{};return $.extend(true,initial,object)}function objectEquals(a,b){return JSON.stringify(a)===JSON.stringify(b)}function uniqid(){return Date.now()}function andList(array,text="and"){if(array.length===1){return array[0]}return`${array.slice(0,-1).join(", ")} ${text} ${array[array.length-1]}`}function orList(array){return andList(array,"or")}function isString(string){return typeof string==="string"}const specialChars=string=>{if(!isString(string)){return string}return string.replace(/&/g,"&amp;").replace(/>/g,"&gt;").replace(/</g,"&lt;").replace(/"/g,"&quot;")};const kebabize=str=>{return str.split("").map((letter,idx)=>{return letter.toUpperCase()===letter?`${idx!==0?"-":""}${letter.toLowerCase()}`:letter}).join("")};const objectToStyle=object=>{const props=[];for(const prop in object){if(object.hasOwnProperty(prop)){props.push(`${kebabize(prop)}:${specialChars(object[prop])}`)}}return props.join(";")};const objectToProps=object=>{const props=[];for(const prop in object){if(object.hasOwnProperty(prop)){switch(prop){case"className":props.push(`class="${specialChars(object[prop])}"`);break;case"style":props.push(`style="${specialChars(objectToStyle(object[prop]))}"`);break;default:props.push(`${kebabize(prop)}="${specialChars(object[prop])}"`);break}}}return props.join(" ")};const Elements={input(props){props={type:"text",className:"input",...props};return`<input ${objectToProps(props)}/>`},select(props,options,selected){return`<select ${objectToProps(props)}>${createOptions(options,selected)}</select>`},option:function(value,text,selected){return`
		  <option value="${specialChars(value)}" ${selected?"selected":""}>${text}</option>`},mappableFields(props,selected){return Elements.select(props,{0:"-- Do not map --",...Groundhogg.fields.mappable},selected)},inputWithReplacementsAndEmojis({type:type="text",name,id,value,className,placeholder:placeholder=""},replacements=true,emojis=true){const classList=[replacements&&"input-with-replacements",emojis&&"input-with-emojis"];return`
		  ...
		  <svg viewBox="0 0 20 15" fill="none" xmlns="http://www.w3.org/2000/svg">
			  ...
			  <path
				  ...
				  fill="#fff"/>
		  </svg>`,name:"Error",type:"error",title({data}){return`<b>${data.step_type}</b> settings not found`},edit({ID,data,meta}){return`
			<div class="panel">
			...
			</div>`},onMount(){}},default:{promiseController:null,title({ID,data,meta}){return data.step_title},edit({ID,data,meta}){return`
			<div class="panel">
			...
			</div>`},onMount(step){var self=this;self.promiseController=new AbortController;const{signal}=self.promiseController;apiPost(`${apiRoutes.steps}/html`,step,{signal:signal}).then(r=>{$("#dynamic-step-settings").html(r.html);$(document).trigger("gh-init-pickers");const $form=$("#settings-form");$form.on("submit",function(e){e.preventDefault();return false}).on("change",function(e){e.preventDefault();const meta=$(this).serializeFormJSON();Editor.updateCurrentStepMeta(meta)});self.promiseController=null}).catch(()=>{})},onDemount(){if(this.promiseController){this.promiseController.abort()}},validate:function(step,errors){},defaults:{}},apply_note:{defaults:{note_text:""},svg:`
		  <svg viewBox="0 0 42 37" fill="none" xmlns="http://www.w3.org/2000/svg">
			  ...
			  <path
				  ...
				  fill="currentColor"/>
		  </svg>`,title({meta}){return"Apply Note"},edit({meta}){const{note_text}=meta;return`
			<div class="panel">
				<div class="row">
			...
			</div>`},onMount(step,updateStepMeta){let saveTimer;tinymceElement("note_text",{tinymce:true,quicktags:true},content=>{clearTimeout(saveTimer);saveTimer=setTimeout(function(){updateStepMeta({note_text:content})},300)})},onDemount(){wp.editor.remove("note_text")}},admin_notification:{svg:`
		  <svg viewBox="0 0 31 43" fill="none" xmlns="http://www.w3.org/2000/svg">
		  ...
		  </svg>`,defaults:{to:"{owner_email}",from:"{owner_email}",reply_to:"{email}",subject:'Notification from "{first}"',note_text:""},title({meta}){const{to}=meta;return`Send notification to ${andList(to.split(",").map(address=>`<b>${address.trim()}</b>`))}`},edit({meta}){return`
			<div class="panel">
				<div class="row">
			...
			</div>`},onMount(step,updateStepMeta){$("#subject, #reply-to, #from, #to").on("change",function(e){const $this=$(this);updateStepMeta({[$this.prop("name")]:$this.val()})});let saveTimer;tinymceElement("note_text",{tinymce:true,quicktags:true},content=>{clearTimeout(saveTimer);saveTimer=setTimeout(function(){updateStepMeta({note_text:content})},300)})},onDemount(){wp.editor.remove("note_text")}},account_created:{defaults:{role:[]},svg:`
      ...
      `,title({ID,data,meta}){const roles=Editor.stepTypes.account_created.context.roles;if(meta.role&&meta.role.length===1){return`<b>${roles[meta.role[0]]}</b> is created`}else if(meta.role&&meta.role.length>1){return`${orList(meta.role.map(role=>`<b>${roles[role]}</b>`))} is created`}else{return"User Created"}},edit({ID,data,meta}){let roles=Editor.stepTypes.account_created.context.roles;return`
			<div class="panel">
				<div class="row">
			...
			</div>`},onMount(step,updateStepMeta){$("#roles").select2().on("change",function(e){let roles=$(this).val();updateStepMeta({role:roles})})}},create_user:{title({ID,data,meta}){const roles=Editor.stepTypes.account_created.context.roles;if(meta.role){return`Create <b>${roles[meta.role]}</b>`}else{return"Create user"}},edit({ID,data,meta}){let options=[];let roles=Editor.stepTypes.account_created.context.roles;for(var role in roles){if(Object.prototype.hasOwnProperty.call(roles,role)){options.push(Elements.option(role,roles[role],meta.role===role))}}return`
			<div class="panel">
				<div class="row">
		  ...
		  <svg viewBox="0 0 35 37" fill="none" xmlns="http://www.w3.org/2000/svg">
		  ...
		  </svg>`,title({ID,data,meta}){let{tags}=meta;if(tags){tags=tags.map(id=>parseInt(id))}if(!tags||tags.length===0){return"Apply tag"}else if(tags.length<4&&TagsStore.hasItems(tags)){return`Apply ${andList(tags.map(id=>`<b>${TagsStore.get(id).data.tag_name}</b>`))}`}else{return`Apply <b>${tags.length}</b> tags`}},edit({ID,data,meta}){let options=TagsStore.getItems().map(tag=>{return{text:tag.data.tag_name,value:tag.ID}});return`
			<div class="panel">
				<div class="row">
			...
			</div>`},onMount(step,updateStepMeta){tagOnMount(step,updateStepMeta)}},remove_tag:{defaults:{tags:[]},svg:`
		  <svg viewBox="0 0 35 35" fill="none" xmlns="http://www.w3.org/2000/svg">
		  ...
		  </svg>`,title({ID,data,meta}){let{tags}=meta;tags=tags.map(id=>parseInt(id));if(tags.length===0){return"Remove tags"}else if(tags.length<4&&TagsStore.hasItems(tags)){return`Remove ${andList(tags.map(id=>`<b>${TagsStore.get(id).data.tag_name}</b>`))}`}else{return`Remove <b>${tags.length}</b> tags`}},edit({ID,data,meta}){let options=TagsStore.getItems().map(tag=>{return{text:tag.data.tag_name,value:tag.ID}});return`
			<div class="panel">
				<div class="row">
			...
			</div>`},onMount(step,updateStepMeta){tagOnMount(step,updateStepMeta)}},tag_applied:{defaults:{tags:[]},svg:`
		  <svg viewBox="0 0 39 35" fill="none" xmlns="http://www.w3.org/2000/svg">
		  ...
		  </svg>`,title({ID,data,meta}){let{tags,condition}=meta;tags=tags.map(id=>parseInt(id));if(!tags){return"Tag is applied"}else if(tags.length>=4){return condition==="all"?`<b>${tags.length}</b> tags are applied`:`Any of <b>${tags.length}</b> tags are applied`}else if(tags.length>1&&tags.length<4&&TagsStore.hasItems(tags)){const tagNames=tags.map(tag=>`<b>${TagsStore.get(tag).data.tag_name}</b>`);return condition==="all"?`${andList(tagNames)} are applied`:`${orList(tagNames)} is applied`}else if(tags.length===1){return`<b>${TagsStore.get(tags[0]).data.tag_name}</b> is applied`}else{return"Tag is applied"}},edit({ID,data,meta}){let options=TagsStore.getItems().map(tag=>{return{text:tag.data.tag_name,value:tag.ID}});const{condition}=meta;return`
			<div class="panel">
				<div class="row">
			...
			</div>`},onMount(step,updateStepMeta){tagWithConditionOnMount(step,updateStepMeta)}},tag_removed:{defaults:{tags:[]},svg:`
		  <svg viewBox="0 0 37 35" fill="none" xmlns="http://www.w3.org/2000/svg">
			  ...
			  <path
				  ...
				  fill="currentColor"/>
		  </svg>`,title({ID,data,meta}){let{tags,condition}=meta;tags=tags.map(id=>parseInt(id));if(!tags){return"Tag is removed"}else if(tags.length>=4){return condition==="all"?`<b>${tags.length}</b> tags are removed`:`Any of <b>${tags.length}</b> tags are removed`}else if(tags.length>1&&tags.length<4&&TagsStore.hasItems(tags)){const tagNames=tags.map(tag=>`<b>${TagsStore.get(tag).data.tag_name}</b>`);return condition==="all"?`${andList(tagNames)} are applied`:`${orList(tagNames)} is removed`}else if(tags.length===1){return`<b>${TagsStore.get(tags[0]).data.tag_name}</b> is removed`}else{return"Tag is removed"}},edit({ID,data,meta}){let options=TagsStore.getItems().map(tag=>{return{text:tag.data.tag_name,value:tag.ID}});const{condition}=meta;return`
			<div class="panel">
				<div class="row">
					<label class="row-label" for="tags">When <select id="condition">
						<option value="any" ${condition==="any"?"selected":""}>Any</option>
			...
			</div>`},onMount(step,updateStepMeta){tagWithConditionOnMount(step,updateStepMeta)}},delay_timer:{defaults:{...delayTimerDefaults},svg:`
      ...
      `,title({meta}){return delayTimerName({...delayTimerDefaults,...meta})},edit({ID,data,meta}){const{delay_amount,delay_type,run_on_type,run_when,run_time,send_in_timezone,run_time_to,run_on_dow_type,run_on_dow,run_on_month_type,run_on_months,run_on_dom}={...delayTimerDefaults,...meta};const delayTypes={minutes:"Minutes",hours:"Hours",days:"Days",weeks:"Weeks",months:"Months",years:"Years",none:"No delay"};const runOnTypes={any:"Any day",weekday:"Weekday",weekend:"Weekend",day_of_week:"Day of week",day_of_month:"Day of month"};const runWhenTypes={now:"Any time",later:"Specific time"};if(delay_type==="minutes"||delay_type==="hours"){runWhenTypes.between="Between"}const runOnDOWTypes={any:"Any",first:"First",second:"Second",third:"Third",fourth:"Fourth",last:"Last"};const runOnDaysOfMonth={};for(let i=1;i<32;i++){runOnDaysOfMonth[i]=i}runOnDaysOfMonth.last="last";const runOnMonthTypes={any:"Of any month",specific:"Of specific month(s)"};const runOnDaysOfWeek={monday:"Monday",tuesday:"Tuesday",wednesday:"Wednesday",thursday:"Thursday",friday:"Friday",saturday:"Saturday",sunday:"Sunday"};const runOnMonths={january:"January",february:"February",march:"March",april:"April",may:"May",june:"June",july:"July",august:"August",september:"September",october:"October",november:"November",december:"December"};const runOnMonthOptions=`
		  ...
		  <svg viewBox="0 0 35 35" fill="none" xmlns="http://www.w3.org/2000/svg">
			  ...
			  <path d="M3.674 8.903l14.166 8.5 14.167-8.5M20.674 24.487h11.333m0 0l-4.25-4.25m4.25 4.25l-4.25 4.25"
			        stroke="currentColor" stroke-width="2"/>
		  </svg>`,title({ID,data,meta}){const title=meta.email_id?EmailsStore.get(parseInt(meta.email_id)).data.title:"an email";return`Send <b>${title}</b>`},edit({ID,data,meta}){const{email_id}=meta;return`
			<div class="panel">
				<div class="row">
					<label class="row-label">Select an email to send...</label>
					${select({id:"email-picker",name:"email_id"},EmailsStore.getItems().map(item=>{return{text:item.data.title,value:item.ID}},email_id))}
				</div>
			</div>`},onMount(step,updateStepMeta){emailPicker("#email-picker",false,items=>{EmailsStore.itemsFetched(items)}).on("change",function(e){updateStepMeta({email_id:parseInt($(this).val())});Editor.renderStepEdit()})}},link_click:{defaults:{redirect_to:""},svg:`
		  <svg viewBox="0 0 35 35" fill="none" xmlns="http://www.w3.org/2000/svg">
			  <path d="M8.594 4.671v22.305l5.329-5.219 3.525 8.607 3.278-1.23-3.688-8.718h7.14L8.593 4.67z"
			        stroke="currentColor" stroke-width="2"/>
		  </svg>`,title({meta}){const{redirect_to}=meta;if(redirect_to){const targetUrl=new URL(redirect_to);const homeUrl=new URL(Groundhogg.managed_page.root);if(targetUrl.hostname===homeUrl.hostname){return`Clicked to <b>${targetUrl.pathname}</b>`}else{return`Clicked to <b>${targetUrl.hostname}</b>`}}else{return"Clicked a tracking link"}},edit({meta}){return`
			<div class="panel">
				<div class="row">
			...
			</div>`},onMount({meta},updateStepMeta){linkPicker("#redirect-to").on("change",function(e){updateStepMeta({redirect_to:$(this).val()})})}},email_confirmed:{svg:`
		  <svg viewBox="0 0 35 35" fill="none" xmlns="http://www.w3.org/2000/svg">
		  ...
		  <svg viewBox="0 0 35 31" fill="none" xmlns="http://www.w3.org/2000/svg">
			  ...
			  <path
				  ...
				  stroke="currentColor" stroke-width="1.5"/>
		  </svg>`,title({meta}){return`Submits <b>${meta.form_name||"a form"}</b>`},edit({meta}){const redirectToURL=`<label class="row-label">Redirect to this URL...</label>
		...
		${textAreaWithReplacementsAndEmojis({id:"success-message",name:"success_message",className:"regular-text",value:meta.success_message||""})}`;return`
			...
			</div>`},onMount({meta},updateStepMeta){linkPicker("#success-page").on("change",e=>{updateStepMeta({success_page:e.target.value})});$("#success-message").on("change",e=>{updateStepMeta({success_message:e.target.value})});$("#enable-ajax").on("change",e=>{updateStepMeta({enable_ajax:e.target.checked},true)});$("#form-name").on("change",e=>{updateStepMeta({form_name:e.target.value})});const editor=formBuilder("#edit-form",meta.form,form=>{updateStepMeta({form:form})});editor.init()}}};for(const func in Editor){if(Editor.hasOwnProperty(func)&&typeof func==="function"){Editor[func]=Editor[func].bind(Editor)}}Groundhogg.helpers={objectToProps:objectToProps,specialChars:specialChars,isString:isString};Groundhogg.funnelEditor=Editor;Groundhogg.funnelEditor.functions={slot:slot,fill:fill,slotsDemounted:slotsDemounted,slotsMounted:slotsMounted,getSteps(){return Editor.getSteps()},stepTitle(step){return StepTypes.getType(step.data.step_type).title(step)},registerStepType(type,opts){return StepTypes.register(type,opts)},updateCurrentStepMeta(newMeta){return Editor.updateCurrentStepMeta(newMeta)},renderStepEdit(){return Editor.renderStepEdit()},getCurrentStep(){return Editor.getCurrentStep()},getCurrentStepMeta(){return Editor.getCurrentStep().meta},getProceedingSteps(stepId){const step=stepId?Editor.getStep(stepId):Editor.getCurrentStep();return Editor.getSteps().filter(_step=>_step.data.step_order>step.data.step_order).sort((a,b)=>a.data.step_order-b.data.step_order)},getPrecedingSteps(stepId){const step=stepId?Editor.getStep(stepId):Editor.getCurrentStep();return Editor.getSteps().filter(_step=>_step.data.step_order<step.data.step_order).sort((a,b)=>a.data.step_order-b.data.step_order)}};Groundhogg.funnelEditor.elements=Elements;fill("beforeStepNotes.form_fill",{render({ID,meta}){if(!stepIsReal(ID)){return""}const copyValue=toCopy=>{return input({className:"code",value:toCopy,onfocus:"this.select()",readonly:true})};return`
