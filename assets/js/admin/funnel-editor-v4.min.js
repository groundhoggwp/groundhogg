(function(Funnel,$){const Tags=Groundhogg.stores.tags;const apiGet=Groundhogg.api.get;const apiPost=Groundhogg.api.post;const apiRoutes=Groundhogg.api.routes.v4;$.fn.serializeFormJSON=function(){var o={};var a=this.serializeArray();$.each(a,function(){if(o[this.name]){if(!o[this.name].push){o[this.name]=[o[this.name]]}o[this.name].push(this.value||"")}else{o[this.name]=this.value||""}});return o};const slot=(name,...args)=>{return SlotFillProvider.slot(name,...args)};const fill=(name,component)=>{return SlotFillProvider.fill(name,component)};const slotsMounted=()=>{return SlotFillProvider.slotsMounted()};const slotsDemounted=()=>{return SlotFillProvider.slotsDemounted()};const SlotFillProvider={fills:[],_slotsMounted:[],_slotsDemounted:[],slot:function(slotName,...args){this._slotsMounted.push({name:slotName,args:args});return this.fills.filter(fill=>fill.slot===slotName).map(fill=>fill.render(...args)).join("")},slotsMounted:function(){let slot;while(this._slotsMounted.length>0){slot=this._slotsMounted.pop();this.fills.filter(fill=>fill.slot===slot.name).forEach(fill=>{fill.onMount(...slot.args)});this._slotsDemounted.push(slot)}},slotsDemounted:function(){let slot;while(this._slotsDemounted.length>0){slot=this._slotsDemounted.pop();this.fills.filter(fill=>fill.slot===slot.name).forEach(fill=>{fill.onDemount(...slot.args)})}},fill:function(slot,component){this.fills.push({slot:slot,...{render:function(){},onMount:function(){},onDemount:function(){},...component}})}};const Editor={activeAddType:"actions",view:"addingStep",activeStep:{},htmlModules:{},isEditingTitle:false,stepFlowContextMenu:null,stepOpenInContextMenu:null,funnel:{ID:0,data:{},steps:[]},origFunnel:{ID:0,data:{},steps:[]},stepTypes:{},undoStates:[],redoStates:[],stepErrors:{},funnelErrors:[],htmlTemplates:{funnelTitleEdit:function(title,isEditing){if(isEditing){return`<input type="text" class="funnel-title-edit regular-text" id="funnel-title-edit" name="funnel_title"
		                 value="${title}">`}else{return`<span class="title-inner">${title}</span><span class="pencil"><span class="dashicons dashicons-edit"></span></span>`}},stepEditPanel:function(step){const{ID:ID,data:data,meta:meta}=step;const{step_type:step_type,step_title:step_title,step_group:step_group}=data;let hasErrors=false;let errors=[];if(Editor.stepErrors.hasOwnProperty(ID)&&Editor.stepErrors[ID].length>0){hasErrors=true;errors=Editor.stepErrors[ID]}return`
			...
			<div class="add-step ${step.type} ${step.group}" data-type="${step.type}" data-group="${step.group}"
				...
				<p>${step.name}</p></div>`},stepFlowCard:function(step,activeStep){const{ID:ID,data:data,meta:meta}=step;const{step_type:step_type,step_title:step_title,step_group:step_group,step_order:step_order}=data;const origStep=Editor.origFunnel.steps.find(s=>s.ID===ID);let status;let hasErrors=false;if(Editor.stepErrors.hasOwnProperty(ID)&&Editor.stepErrors[ID].length>0){status="config-error";hasErrors=true}else if(origStep&&!objectEquals(step,origStep)){status="edited"}else if(!origStep){status="new"}const nextStep=Editor.funnel.steps.find(step=>step.data.step_order===step_order+1);const prevStep=Editor.funnel.steps.find(step=>step.data.step_order===step_order-1);return`
        ...
        `}},init:function(){var self=this;var $doc=$(document);self.origFunnel=copyObject(self.funnel);$doc.on("click",".step-add .select-type",function(){self.saveUndoState();self.activeAddType=$(this).data("type");self.renderStepAdd()});$doc.on("mouseleave",".step-flow .steps .step",function(e){const $step=$(this);$(".step-menu ul",$step).hide()});$doc.on("click",".step-flow .steps .step",function(e){const $step=$(this);switch(true){case $(e.target).is(".step-menu-duplicate"):window.console.log("duplicate");const stepToCopy=self.funnel.steps.find(step=>step.ID===self.stepOpenInContextMenu);const newStep=copyObject(stepToCopy);newStep.ID=uniqid();self.addStep(newStep);break;case $(e.target).is(".step-menu-delete"):window.console.log("delete");self.deleteStep(self.stepOpenInContextMenu);break;case $(e.target).is(".step-menu")||$(e.target).parent(".step-menu").length>0:window.console.log("toggle menu");$(".step-menu ul",$step).toggle();break;case $(e.target).is(".step-menu-edit"):default:window.console.log("edit");const clickedStep=parseInt($(this).data("id"));if(clickedStep===self.activeStep){return}self.saveUndoState();self.previousActiveStep=self.activeStep;self.activeStep=clickedStep;self.view="editingStep";self.renderStepFlow();self.renderStepEdit()}});$doc.on("click",".step-flow .add-new-step",function(){self.activeStep=null;self.view="addingStep";self.renderStepFlow();self.renderStepAdd()});$doc.on("click",".undo-and-redo .undo",function(){self.undo()});$doc.on("click",".undo-and-redo .redo",function(){self.redo()});$doc.on("click",".header-stuff .title",function(){if(!self.isEditingTitle){self.isEditingTitle=true;self.renderTitle()}});$(document).on("tinymce-editor-setup",function(event,editor){editor.settings.toolbar1="formatselect,bold,italic,bullist,numlist,blockquote,alignleft,aligncenter,alignright,link,spellchecker,wp_adv,dfw,groundhoggreplacementbtn,groundhoggemojibtn";editor.settings.toolbar2="strikethrough,hr,forecolor,pastetext,removeformat,charmap,outdent,indent,undo,redo,wp_help";editor.settings.height=200;editor.on("click",function(ed,e){$(document).trigger("to_mce")})});$doc.on("blur change keydown",".funnel-title-edit",function(e){if(e.type==="keydown"&&e.keyCode!==13){self.resizeTitleEdit();return}self.saveUndoState();self.funnel.data.title=e.target.value;self.isEditingTitle=false;self.autoSaveEditedFunnel();self.renderTitle()});Tags.preloadTags();if(this.funnel.meta.edited){this.funnel.steps=this.funnel.meta.edited.steps;this.funnel.data.title=this.funnel.meta.edited.title}self.setupSortable();self.setupStepTypes();self.initStepFlowContextMenu();self.render()},setupSortable:function(){var self=this;$(".step-flow .steps").sortable({placeholder:"step-placeholder",cancel:".text-helper",start:function(e,ui){ui.placeholder.height(ui.item.height());ui.placeholder.width(ui.item.width())},receive:function(e,ui){self.saveUndoState();var type=$(ui.helper).data("type");var group=$(ui.helper).data("group");var id=Date.now();$(ui.helper).addClass("step");$(ui.helper).data("id",id);self.addStep({ID:id,data:{ID:id,funnel_id:Editor.funnel.ID,step_type:type,step_group:group,step_order:$(ui.helper).prevAll(".step").length},meta:StepTypes.getType(type).defaults})},update:function(e,ui){self.saveUndoState();self.syncOrderWithFlow();self.autoSaveEditedFunnel();self.renderStepFlow()}}).disableSelection()},setupStepTypes:function(){for(var prop in this.stepTypes){if(Object.prototype.hasOwnProperty.call(this.stepTypes,prop)&&Object.prototype.hasOwnProperty.call(StepTypes,prop)){Object.assign(this.stepTypes[prop],{...StepTypes.default,...StepTypes[prop]})}else{Object.assign(this.stepTypes[prop],StepTypes.default)}}},initStepFlowContextMenu:function(){var self=this;this.stepFlowContextMenu=createContextMenu({menuClassName:"step-context-menu",targetSelector:".step-flow .steps .step",items:[{key:"duplicate",text:"Duplicate"},{key:"delete",text:"Delete"}],onOpen:function(e,el){self.stepOpenInContextMenu=parseInt(el.dataset.id)},onSelect:function(key){switch(key){case"delete":self.deleteStep(self.stepOpenInContextMenu);break;case"duplicate":const stepToCopy=self.funnel.steps.find(step=>step.ID===self.stepOpenInContextMenu);const newStep=copyObject(stepToCopy);newStep.ID=uniqid();self.addStep(newStep);break}}});this.stepFlowContextMenu.init()},renderStepFlow:function(){var self=this;this.checkForStepErrors();var steps=this.funnel.steps.sort((a,b)=>a.data.step_order-b.data.step_order).map(step=>self.htmlTemplates.stepFlowCard(step,self.activeStep)).join("");$(".step-flow .steps").html(steps)},checkForStepErrors:function(){const self=this;this.funnel.steps.forEach(step=>{const errors=[];self.stepErrors[step.ID]=errors;const{step_group:step_group,step_order:step_order,step_type:step_type}=step.data;if(step_group==="action"&&step_order===1){errors.push("Actions cannot be at the start of a funnel.")}const typeHandler=StepTypes.getType(step_type);if(typeHandler){typeHandler.validate(step,errors)}})},renderStepEdit:function(){if(this.view!=="editingStep"){return}var self=this;const step=this.funnel.steps.find(step=>step.ID===this.activeStep);const previousStep=this.funnel.steps.find(step=>step.ID===this.previousActiveStep);if(previousStep){this.stepTypes[previousStep.data.step_type].onDemount(previousStep)}slotsDemounted();$("#control-panel").html(this.htmlTemplates.stepEditPanel(step));this.stepTypes[step.data.step_type].onMount(step);$("#step-notes").on("change",function(e){self.updateCurrentStepMeta({step_notes:$(this).val()})});slotsMounted()},renderStepAdd:function(){if(this.view!=="addingStep"){return}$("#control-panel").html(this.htmlTemplates.stepAddPanel(this.activeAddType));var self=this;$(".add-step").draggable({connectToSortable:".step-flow .steps",helper:"clone",revert:"invalid",revertDuration:0})},renderTitle:function(){$(".header-stuff .title").html(this.htmlTemplates.funnelTitleEdit(this.funnel.data.title,this.isEditingTitle));if(this.isEditingTitle){$(".funnel-title-edit").focus();this.resizeTitleEdit()}},resizeTitleEdit:function(){$(".funnel-title-edit").width(this.funnel.data.title.length+1+"ch")},render:function(){this.renderTitle();this.renderStepFlow();this.renderStepAdd();this.renderStepEdit()},syncOrderWithFlow:function(){var self=this;$(".step-flow .steps .step").each(function(i){self.funnel.steps.find(step=>step.ID===$(this).data("id")).data.step_order=i+1});this.fixStepOrders()},saveUndoState:function(){const{view:view,funnel:funnel,activeStep:activeStep,activeAddType:activeAddType,isEditingTitle:isEditingTitle}=this;this.undoStates.push({view:view,activeStep:activeStep,isEditingTitle:isEditingTitle,activeAddType:activeAddType,funnel:copyObject(funnel)})},undo:function(){var lastState=this.undoStates.pop();if(!lastState){return}Object.assign(this,lastState);this.redoStates.push(lastState);this.render()},redo:function(){var lastState=this.redoStates.pop();if(!lastState){return}Object.assign(this,lastState);this.undoStates.push(lastState);this.render()},addStep:function(step){if(!step){return}this.saveUndoState();this.funnel.steps.push(step);this.fixStepOrders();this.renderStepFlow();this.autoSaveEditedFunnel()},fixStepOrders:function(){let newOrder=1;this.funnel.steps.sort((a,b)=>a.data.step_order-b.data.step_order).forEach(step=>{step.data.step_order=newOrder;newOrder++})},deleteStep:function(stepId){if(!stepId){return}this.saveUndoState();this.funnel.steps=this.funnel.steps.filter(step=>step.ID!==stepId);this.fixStepOrders();this.renderStepFlow();if(this.activeStep===stepId){this.view="addingStep";this.renderStepAdd();this.activeStep=null}this.autoSaveEditedFunnel()},getStep:function(stepId){return this.funnel.steps.find(step=>step.ID===stepId)},getSteps:function(){return this.funnel.steps},getCurrentStep:function(){return this.getStep(this.activeStep)},updateStep:function(stepId,newData){const step=this.getStep(stepId);const newStep={...step,...newData};var toReplace=this.funnel.steps.findIndex(step=>step.ID===stepId);this.saveUndoState();if(toReplace!==-1){this.funnel.steps[toReplace]=newStep}this.renderStepFlow()},updateCurrentStep:function(newData){this.updateStep(this.activeStep,newData);this.autoSaveEditedFunnel()},updateCurrentStepMeta:function(newMeta){console.log(this);const{meta:meta}=this.getCurrentStep();this.updateStep(this.activeStep,{meta:{...meta,...newMeta}});this.autoSaveEditedFunnel()},autoSaveTimeout:null,autoSaveEditedFunnel:function(){var self=this;if(this.autoSaveTimeout){clearTimeout(this.autoSaveTimeout)}this.autoSaveTimeout=setTimeout(function(){apiPost(`${apiRoutes.funnels}/${self.funnel.ID}/meta`,{edited:{steps:self.funnel.steps,title:self.funnel.data.title}})},3e3)},...Funnel};$(function(){Editor.init()});function copyObject(object,initial){initial=initial||{};return $.extend(true,initial,object)}function objectEquals(a,b){return JSON.stringify(a)===JSON.stringify(b)}function uniqid(){return Date.now()}function andList(array,text="and"){if(array.length===1){return array[0]}return`${array.slice(0,-1).join(", ")} ${text} ${array[array.length-1]}`}function orList(array){return andList(array,"or")}function isString(string){return typeof string==="string"}const specialChars=string=>{if(!isString(string)){return string}return string.replace(/&/g,"&amp;").replace(/>/g,"&gt;").replace(/</g,"&lt;").replace(/"/g,"&quot;")};const kebabize=str=>{return str.split("").map((letter,idx)=>{return letter.toUpperCase()===letter?`${idx!==0?"-":""}${letter.toLowerCase()}`:letter}).join("")};const objectToStyle=object=>{const props=[];for(const prop in object){if(object.hasOwnProperty(prop)){props.push(`${kebabize(prop)}:${specialChars(object[prop])}`)}}return props.join(";")};const objectToProps=object=>{const props=[];for(const prop in object){if(object.hasOwnProperty(prop)){switch(prop){case"className":props.push(`class="${specialChars(object[prop])}"`);break;case"style":props.push(`style="${specialChars(objectToStyle(object[prop]))}"`);break;default:props.push(`${kebabize(prop)}="${specialChars(object[prop])}"`);break}}}return props.join(" ")};const Elements={input:function(props){props={type:"text",className:"input",...props};return`<input ${objectToProps(props)}/>`},select:function(props,options,selected){return`<select ${objectToProps(props)}>${createOptions(options,selected)}</select>`},option:function(value,text,selected){return`
		  ...
		  <svg viewBox="0 0 42 37" fill="none" xmlns="http://www.w3.org/2000/svg">
			...
			</div>`},onMount:function(){wp.editor.initialize("note_text",{tinymce:true,quicktags:true});for(id in tinymce.editors){if(id.trim()){elementReady(id)}}tinymce.on("AddEditor",function(e){elementReady(e.editor.id)});function elementReady(editor_id){var _editor=tinymce.editors[editor_id];let saveTimer=null;_editor.on("keyup",function(e){clearTimeout(saveTimer);saveTimer=setTimeout(function(){Editor.updateCurrentStepMeta({note_text:tinyMCE.activeEditor.getContent({format:"raw"})})},1e3)})}},onDemount:function(){wp.editor.remove("note_text")}},admin_notification:{svg:`
		  <svg viewBox="0 0 31 43" fill="none" xmlns="http://www.w3.org/2000/svg">
		  ...
		  </svg>`,defaults:{to:"{owner_email}",from:"{owner_email}",reply_to:"{email}",note_text:""},title:function({meta:meta}){const{to:to}=meta;return`Send notification to <b>${to}</b>`},edit:function({meta:meta}){return`
			<div class="panel">
				<div class="row">
					<label class="row-label" for="to">Send this notification to...</label>
					${Elements.inputWithReplacements({type:"text",id:"to",name:"to",className:"regular-text",value:meta.to||"{owner_email}"})}
				...
				<div class="row">
					...
					${Elements.inputWithReplacements({type:"text",id:"from",name:"from",className:"regular-text",value:meta.from||"{owner_email}"})}
				...
				<div class="row">
					...
					${Elements.inputWithReplacements({type:"text",id:"reply-to",name:"reply_to",className:"regular-text",value:meta.reply_to||"{email}"})}
				...
				<div class="row">
					...
					${Elements.inputWithReplacementsAndEmojis({type:"text",id:"subject",name:"subject",className:"regular-text",value:meta.subject||'Notification from "{full_name}"'})}
			...
			</div>`},onMount:function(){wp.editor.initialize("note_text",{tinymce:true,quicktags:true})},onDemount:function(){wp.editor.remove("note_text")}},account_created:{svg:`
      ...
      `,title:function({ID:ID,data:data,meta:meta}){const roles=Editor.stepTypes.account_created.context.roles;if(meta.role&&meta.role.length===1){return`<b>${roles[meta.role[0]]}</b> is created`}else if(meta.role&&meta.role.length>1){return`${orList(meta.role.map(role=>`<b>${roles[role]}</b>`))} is created`}else{return"User Created"}},edit:function({ID:ID,data:data,meta:meta}){let options=[];let roles=Editor.stepTypes.account_created.context.roles;for(var role in roles){if(Object.prototype.hasOwnProperty.call(roles,role)){options.push(Elements.option(role,roles[role],meta.role.indexOf(role)>=0))}}return`
		  ...
		  <svg viewBox="0 0 35 37" fill="none" xmlns="http://www.w3.org/2000/svg">
		  ...
		  </svg>`,title:function({ID:ID,data:data,meta:meta}){if(!meta.tags){return"Apply tags"}return`Apply <b>${meta.tags.length}</b> ${meta.tags.length>1?"tags":"tag"}`},edit:function({ID:ID,data:data,meta:meta}){let options=Tags.items.map(tag=>Elements.option(tag.ID,tag.data.tag_name,meta.tags&&meta.tags.indexOf(tag.ID)!==-1));return`
			<div class="panel">
				<div class="row">
			...
			</div>`},onMount:function(step){tagOnMount(step)}},remove_tag:{svg:`
		  <svg viewBox="0 0 35 35" fill="none" xmlns="http://www.w3.org/2000/svg">
		  ...
		  </svg>`,title:function({ID:ID,data:data,meta:meta}){if(!meta.tags){return"Remove tags"}return`Remove <b>${meta.tags.length}</b> ${meta.tags.length>1?"tags":"tag"}`},edit:function({ID:ID,data:data,meta:meta}){let options=Tags.items.map(tag=>Elements.option(tag.ID,tag.data.tag_name,meta.tags&&meta.tags.indexOf(tag.ID)!==-1));return`
			<div class="panel">
				<div class="row">
			...
			</div>`},onMount:function(step){tagOnMount(step)}},tag_applied:{svg:`
		  <svg viewBox="0 0 39 35" fill="none" xmlns="http://www.w3.org/2000/svg">
			...
			</div>`},onMount:function(step){tagWithConditionOnMount(step)}},tag_removed:{svg:`
		  <svg viewBox="0 0 37 35" fill="none" xmlns="http://www.w3.org/2000/svg">
			...
			</div>`},onMount:function(step){tagWithConditionOnMount(step)}},delay_timer:{svg:`
      ...
      `,title:function({meta:meta}){return delayTimerName({...delayTimerDefaults,...meta})},edit:function({ID:ID,data:data,meta:meta}){const{delay_amount:delay_amount,delay_type:delay_type,run_on_type:run_on_type,run_when:run_when,run_time:run_time,send_in_timezone:send_in_timezone,run_time_to:run_time_to,run_on_dow_type:run_on_dow_type,run_on_dow:run_on_dow,run_on_month_type:run_on_month_type,run_on_months:run_on_months,run_on_dom:run_on_dom}={...delayTimerDefaults,...meta};const delayTypes={minutes:"Minutes",hours:"Hours",days:"Days",weeks:"Weeks",months:"Months",years:"Years",none:"No delay"};const runOnTypes={any:"Any day",weekday:"Weekday",weekend:"Weekend",day_of_week:"Day of week",day_of_month:"Day of month"};const runWhenTypes={now:"Any time",later:"Specific time"};if(delay_type==="minutes"||delay_type==="hours"){runWhenTypes.between="Between"}const runOnDOWTypes={any:"Any",first:"First",second:"Second",third:"Third",fourth:"Fourth",last:"Last"};const runOnDaysOfMonth={};for(let i=1;i<32;i++){runOnDaysOfMonth[i]=i}runOnDaysOfMonth.last="last";const runOnMonthTypes={any:"Of any month",specific:"Of specific month(s)"};const runOnDaysOfWeek={monday:"Monday",tuesday:"Tuesday",wednesday:"Wednesday",thursday:"Thursday",friday:"Friday",saturday:"Saturday",sunday:"Sunday"};const runOnMonths={january:"January",february:"February",march:"March",april:"April",may:"May",june:"June",july:"July",august:"August",september:"September",october:"October",november:"November",december:"December"};const runOnMonthOptions=`
			...
			<div class="panel">
				<div class="row">
					<h3 class="delay-preview" style="font-weight: normal">${delayTimerName({...meta,...delayTimerDefaults})}</h3>
				</div>
				<div class="row">
		  ...
		  <svg viewBox="0 0 35 35" fill="none" xmlns="http://www.w3.org/2000/svg">
			  ...
			  <path d="M3.674 8.903l14.166 8.5 14.167-8.5M20.674 24.487h11.333m0 0l-4.25-4.25m4.25 4.25l-4.25 4.25"
			        stroke="currentColor" stroke-width="2"/>
		  </svg>`,title:function({ID:ID,data:data,meta:meta}){return`Send Email`},edit:function({ID:ID,data:data,meta:meta}){return`
			<div class="panel">
