(function(Funnel,$){const{tags:TagsStore,emails:EmailsStore,funnels:FunnelsStore,campaigns:CampaignsStore,contacts:ContactsStore}=Groundhogg.stores;const{get:apiGet,post:apiPost,delete:apiDelete,patch:apiPatch,routes:routes}=Groundhogg.api;const{formatNumber:formatNumber,formatTime:formatTime,formatDate:formatDate,formatDateTime:formatDateTime}=Groundhogg.formatting;const{v4:apiRoutes}=routes;const{createFilters:createFilters}=Groundhogg.filters.functions;const{icons:icons,improveTinyMCE:improveTinyMCE,buttonToggle:buttonToggle,dialog:dialog,confirmationModal:confirmationModal,dangerConfirmationModal:dangerConfirmationModal,loadingModal:loadingModal,modal:modal,select:select,input:input,textarea:textarea,objectEquals:objectEquals,copyObject:copyObject,uniqid:uniqid,specialChars:specialChars,regexp:regexp,tooltip:tooltip,toggle:toggle,savingModal:savingModal,bold:bold,moreMenu:moreMenu,clickInsideElement:clickInsideElement}=Groundhogg.element;const{campaignPicker:campaignPicker,emailPicker:emailPicker}=Groundhogg.pickers;const{StepTypes:StepTypes,StepPacks:StepPacks}=Groundhogg;const{__:__,_x:_x,_n:_n,_nx:_nx,sprintf:sprintf}=wp.i18n;const toEditorButton=()=>{return`
  ...
  <path fill="currentColor" d="M448 24H224a40 40 0 00-40 40v32a24 24 0 0048 0V72h208v368H232v-24a24 24 0 00-48 0v32a40 40 0 0040 40h224a40 40 0 0040-40V64a40 40 0 00-40-40z"/>
        ...
        `}},init:function(){this.loadingClose=loadingModal().close;const self=this;const $doc=$(document);$doc.on("click",".step-add .select-type",function(){self.saveUndoState();self.activeAddType=$(this).data("type");self.renderStepAdd()});$doc.on("click",".step-flow .steps .step",function(e){const $step=$(this);const step=self.funnel.steps.find(step=>step.ID===parseInt($step.data("id")));const setStepEdit=()=>{if(step.ID===self.activeStep){return}self.saveUndoState();self.previousActiveStep=self.activeStep;self.activeStep=step.ID;self.view="editingStep";self.renderStepFlow();self.renderStepEdit()};if(clickInsideElement(e,".step-menu-button")){moreMenu(this,{items:[{key:"edit",text:__("Edit")},{key:"move-up",text:__("Move up","to move a step before another step","groundhogg")},{key:"move-down",text:_x("Move down","to move a step after another step","groundhogg")},{key:"duplicate",text:_x("Duplicate","to duplicate a step","groundhogg")},{key:"delete",text:'<span class="gh-text danger">'+__("Delete")+"</span>"}],onSelect:key=>{switch(key){case"move-up":self.moveStepUp(step);break;case"move-down":self.moveStepDown(step);break;case"duplicate":const newStep=copyObject(step);newStep.ID=uniqid();self.addStep(newStep);break;case"delete":self.deleteStep(step.ID);break;case"edit":setStepEdit();break}}})}else{setStepEdit()}});$doc.on("click",".step-flow .add-new-step",function(){self.activeStep=null;self.view="addingStep";self.renderStepFlow();self.renderStepAdd()});$doc.on("click",".undo-and-redo .undo",function(){self.undo()});$doc.on("click",".undo-and-redo .redo",function(){self.redo()});$doc.on("click",".header-stuff #title",function(){if(!self.isEditingTitle){self.isEditingTitle=true;self.renderTitle()}});$doc.on("blur change keydown","#funnel-title-edit",function(e){if(e.type==="keydown"&&e.key!=="Enter"){self.resizeTitleEdit();return}self.saveUndoState();self.funnel.data.title=e.target.value;self.isEditingTitle=false;self.update({data:{title:e.target.value}},false);self.renderTitle()});$doc.on("click","#more-menu",()=>{moreMenu("#more-menu",{onSelect:key=>{switch(key){case"campaigns":const campaignContent=()=>{return`
				  ...
				  <p>${__("The funnel will become un-editable until restored.","groundhogg")}</p>`,confirmText:_x("Archive","a verb meaning to add an item to an archive","groundhogg"),onConfirm:()=>{console.log("yikes")}});break;case"contacts":let state={method:"filters",filters:[],search:"",total_contacts:0};const setState=(s,reRender=false)=>{state={...state,...s};if(reRender){setContent(addContactsToFunnel(state));addContactsOnMount(state)}};const addContactsOnMount=({method:method,filters:filters=[]})=>{const totalContacts=query=>{ContactsStore.count(query).then(total=>{setState({total_contacts:total})})};buttonToggle("#add-contacts",{active:method,onSelect:key=>{setState({method:key},true)},options:[{key:"filters",label:icons.filter+__("Filters","groundhogg")},{key:"searches",label:icons.contactSearch+__("Saved Search","groundhogg")}]});if(method==="filters"){createFilters("#add-via-filters",filters,filters=>{setState({filters:filters})}).mount()}};const addContactsToFunnel=({method:method,total_contacts:total_contacts})=>{return`
					  <div>
