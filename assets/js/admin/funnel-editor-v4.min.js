(function(Funnel,$){const{tags:TagsStore,emails:EmailsStore,funnels:FunnelsStore,campaigns:CampaignsStore}=Groundhogg.stores;const{get:apiGet,post:apiPost,delete:apiDelete,patch:apiPatch,routes:routes}=Groundhogg.api;const{v4:apiRoutes}=routes;const{icons:icons,improveTinyMCE:improveTinyMCE,dialog:dialog,confirmationModal:confirmationModal,dangerConfirmationModal:dangerConfirmationModal,loadingModal:loadingModal,modal:modal,select:select,input:input,textarea:textarea,objectEquals:objectEquals,copyObject:copyObject,uniqid:uniqid,specialChars:specialChars,regexp:regexp,tooltip:tooltip,toggle:toggle,savingModal:savingModal,moreMenu:moreMenu,clickInsideElement:clickInsideElement}=Groundhogg.element;const{campaignPicker:campaignPicker,emailPicker:emailPicker}=Groundhogg.pickers;const{StepTypes:StepTypes,StepPacks:StepPacks}=Groundhogg;const{__:__,_x:_x,_n:_n,_nx:_nx,sprintf:sprintf}=wp.i18n;const toEditorButton=()=>{return`
		<button id="close-email-editor" class="gh-button secondary text icon">
			...
			</div>`},publishActions:function(status){const moreMenu=`
  ...
  <path fill="currentColor" d="M448 24H224a40 40 0 00-40 40v32a24 24 0 0048 0V72h208v368H232v-24a24 24 0 00-48 0v32a40 40 0 0040 40h224a40 40 0 0040-40V64a40 40 0 00-40-40z"/>
        ...
        `,confirmText:__("Delete"),onConfirm:()=>{removeStep()}})}else{removeStep()}},getStep:function(stepId){return this.funnel.steps.find(step=>step.ID===stepId)},getSteps:function(){return this.funnel.steps},getCurrentStep:function(){return this.getStep(this.activeStep)},updateStep:function(stepId,newData){const step=this.getStep(stepId);const newStep={...step,...newData};newStep.data.step_title=StepTypes.getType(newStep.data.step_type).title(newStep);const toReplace=this.funnel.steps.findIndex(step=>step.ID===stepId);this.autoSaveEditedFunnel();this.saveUndoState();if(toReplace!==-1){this.funnel.steps[toReplace]=newStep}this.renderStepFlow();return newStep},updateCurrentStep:function(newData,reRenderStepEdit=false){const step=this.updateStep(this.activeStep,newData);if(reRenderStepEdit){this.renderStepEdit()}return step},updateCurrentStepData:function(newData,reRenderStepEdit=false){const{data:data,meta:meta}=this.getCurrentStep();const step=this.updateStep(this.activeStep,{data:{...data,...newData}});if(reRenderStepEdit){this.renderStepEdit()}return step},updateCurrentStepMeta:function(newMeta,reRenderStepEdit=false){const{data:data,meta:meta}=this.getCurrentStep();const step=this.updateStep(this.activeStep,{meta:{...meta,...newMeta}});if(reRenderStepEdit){this.renderStepEdit()}return step},autoSaveTimeout:null,abortController:null,autoSaveEditedFunnel:function(){const self=this;if(this.autoSaveTimeout){clearTimeout(this.autoSaveTimeout)}this.autoSaveTimeout=setTimeout(()=>{self.autoSaveTimeout=null;self.abortController=new AbortController;const{signal:signal}=self.abortController;FunnelsStore.patchMeta(this.funnel.ID,{edited:{steps:self.funnel.steps,title:self.funnel.data.title}},{signal:signal}).then(data=>{self.setLastSaved();self.abortController=null})},3e3)},...Funnel};$(function(){Editor.init()});fill("beforeStepNotes.send_email",{render:function({ID:ID,meta:meta}){const{email_id:email_id}=meta;const email=EmailsStore.get(parseInt(email_id));if(!email_id||!email){return""}return`
      ...
      `},onMount:function({ID:ID,meta:meta},updateStepMeta){const{email_id:email_id}=meta;const email=EmailsStore.get(parseInt(email_id));if(!email_id||!email){return}$("#edit-email-right").on("click",()=>{Editor.renderEmailEditor(email)});$("#add-new-email-right").on("click",()=>{Editor.renderEmailTemplatePicker(updateStepMeta)});emailPicker("#email-picker-right",false,items=>EmailsStore.itemsFetched(items),{},{width:"100%"}).on("change",e=>{updateStepMeta({email_id:e.target.value},true)})}});fill("beforeStepNotes.form_fill",{render:function({ID:ID,meta:meta}){if(!stepIsReal(ID)){return""}const copyValue=toCopy=>{return input({className:"code",value:toCopy,onfocus:"this.select()",readonly:true})};return`
