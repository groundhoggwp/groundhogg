      ...
      `,title:function({ID:ID,data:data,meta:meta}){const roles=Editor.stepTypes.account_created.context.roles;if(meta.role&&meta.role.length===1){return`<b>${roles[meta.role[0]]}</b> is created`}else if(meta.role&&meta.role.length>1){return`${orList(meta.role.map(role=>`<b>${roles[role]}</b>`))} is created`}else{return"User Created"}},edit:function({ID:ID,data:data,meta:meta}){let options=[];let roles=Editor.stepTypes.account_created.context.roles;for(var role in roles){if(Object.prototype.hasOwnProperty.call(roles,role)){options.push(Elements.option(role,roles[role],meta.role.indexOf(role)>=0))}}return`
		  ...
		  <svg viewBox="0 0 37 35" fill="none" xmlns="http://www.w3.org/2000/svg">
			  ...
			  <path
				  ...
				  fill="currentColor"/>
		  </svg>`,title:function({ID:ID,data:data,meta:meta}){const{tags:tags,condition:condition}=meta;tags=tags.map(id=>parseInt(id));if(!tags){return"Tag is removed"}else if(tags.length>=4){return condition==="all"?`<b>${tags.length}</b> tags are removed`:`Any of <b>${tags.length}</b> tags are removed`}else if(tags.length>1&&tags.length<4&&TagsStore.hasItems(tags)){const tagNames=tags.map(tag=>`<b>${TagsStore.get(tag).data.tag_name}</b>`);return condition==="all"?`${andList(tagNames)} are applied`:`${orList(tagNames)} is removed`}else if(tags.length===1){return`<b>${TagsStore.get(tags[0]).data.tag_name}</b> is removed`}else{return"Tag is removed"}},edit:function({ID:ID,data:data,meta:meta}){let options=TagsStore.getItems().map(tag=>{return{text:tag.data.tag_name,value:tag.ID}});const{condition:condition}=meta;return`
			<div class="panel">
				<div class="row">
					<label class="row-label" for="tags">When <select id="condition">
						<option value="any" ${condition==="any"?"selected":""}>Any</option>
