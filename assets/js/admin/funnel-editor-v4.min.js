(function(Funnel,$){const TagsStore=Groundhogg.stores.tags;const EmailsStore=Groundhogg.stores.emails;const apiGet=Groundhogg.api.get;const apiPost=Groundhogg.api.post;const apiRoutes=Groundhogg.api.routes.v4;const{tinymceElement:tinymceElement,confirmationModal:confirmationModal,loadingModal:loadingModal,modal:modal,select:select,input:input,textarea:textarea,regexp:regexp,toggle:toggle,textAreaWithReplacements:textAreaWithReplacements,textAreaWithReplacementsAndEmojis:textAreaWithReplacementsAndEmojis,flattenObject:flattenObject}=Groundhogg.element;const{formBuilder:formBuilder}=Groundhogg;const{linkPicker:linkPicker,emailPicker:emailPicker,tagPicker:tagPicker,apiPicker:apiPicker}=Groundhogg.pickers;$.fn.serializeFormJSON=function(){var o={};var a=this.serializeArray();$.each(a,function(){if(o[this.name]){if(!o[this.name].push){o[this.name]=[o[this.name]]}o[this.name].push(this.value||"")}else{o[this.name]=this.value||""}});return o};const slot=(name,...args)=>{return SlotFillProvider.slot(name,...args)};const fill=(name,component)=>{return SlotFillProvider.fill(name,component)};const slotsMounted=()=>{return SlotFillProvider.slotsMounted()};const slotsDemounted=()=>{return SlotFillProvider.slotsDemounted()};const stepIsReal=stepId=>{return Editor.origFunnel.steps.find(step=>step.ID===stepId)};const SlotFillProvider={fills:[],_slotsMounted:[],_slotsDemounted:[],slot:function(slotName,...args){this._slotsMounted.push({name:slotName,args:args});return this.fills.filter(fill=>fill.slot===slotName).map(fill=>fill.render(...args)).join("")},slotsMounted:function(){let slot;while(this._slotsMounted.length>0){slot=this._slotsMounted.pop();this.fills.filter(fill=>fill.slot===slot.name).forEach(fill=>{fill.onMount(...slot.args)});this._slotsDemounted.push(slot)}},slotsDemounted:function(){let slot;while(this._slotsDemounted.length>0){slot=this._slotsDemounted.pop();this.fills.filter(fill=>fill.slot===slot.name).forEach(fill=>{fill.onDemount(...slot.args)})}},fill:function(slot,component){this.fills.push({slot:slot,...{render:function(){},onMount:function(){},onDemount:function(){},...component}})}};const getStepType=type=>{return Editor.stepTypes.hasOwnProperty(type)?Editor.stepTypes[type]:StepTypes.getType("error")};const Editor={activeAddType:"actions",view:"addingStep",activeStep:{},htmlModules:{},isEditingTitle:false,stepFlowContextMenu:null,stepOpenInContextMenu:null,funnel:{ID:0,data:{},steps:[]},origFunnel:{ID:0,data:{},steps:[]},stepTypes:{},undoStates:[],redoStates:[],stepErrors:{},funnelErrors:[],htmlTemplates:{publishActions:function(status){if(status==="inactive"){return`
        ...
        `},emailEditor:function(step){return`
      ...
      `,title:function({ID:ID,data:data,meta:meta}){const roles=Editor.stepTypes.account_created.context.roles;if(meta.role&&meta.role.length===1){return`<b>${roles[meta.role[0]]}</b> is created`}else if(meta.role&&meta.role.length>1){return`${orList(meta.role.map(role=>`<b>${roles[role]}</b>`))} is created`}else{return"User Created"}},edit:function({ID:ID,data:data,meta:meta}){let roles=Editor.stepTypes.account_created.context.roles;return`
		  ...
		  </svg>`,title:function({ID:ID,data:data,meta:meta}){let{tags:tags,condition:condition}=meta;tags=tags.map(id=>parseInt(id));if(!tags){return"Tag is applied"}else if(tags.length>=4){return condition==="all"?`<b>${tags.length}</b> tags are applied`:`Any of <b>${tags.length}</b> tags are applied`}else if(tags.length>1&&tags.length<4&&TagsStore.hasItems(tags)){const tagNames=tags.map(tag=>`<b>${TagsStore.get(tag).data.tag_name}</b>`);return condition==="all"?`${andList(tagNames)} are applied`:`${orList(tagNames)} is applied`}else if(tags.length===1){return`<b>${TagsStore.get(tags[0]).data.tag_name}</b> is applied`}else{return"Tag is applied"}},edit:function({ID:ID,data:data,meta:meta}){let options=TagsStore.getItems().map(tag=>{return{text:tag.data.tag_name,value:tag.ID}});const{condition:condition}=meta;return`
			<div class="panel">
				<div class="row">
      ...
      `,title:function({meta:meta}){return delayTimerName({...delayTimerDefaults,...meta})},edit:function({ID:ID,data:data,meta:meta}){const{delay_amount:delay_amount,delay_type:delay_type,run_on_type:run_on_type,run_when:run_when,run_time:run_time,send_in_timezone:send_in_timezone,run_time_to:run_time_to,run_on_dow_type:run_on_dow_type,run_on_dow:run_on_dow,run_on_month_type:run_on_month_type,run_on_months:run_on_months,run_on_dom:run_on_dom}={...delayTimerDefaults,...meta};const delayTypes={minutes:"Minutes",hours:"Hours",days:"Days",weeks:"Weeks",months:"Months",years:"Years",none:"No delay"};const runOnTypes={any:"Any day",weekday:"Weekday",weekend:"Weekend",day_of_week:"Day of week",day_of_month:"Day of month"};const runWhenTypes={now:"Any time",later:"Specific time"};if(delay_type==="minutes"||delay_type==="hours"){runWhenTypes.between="Between"}const runOnDOWTypes={any:"Any",first:"First",second:"Second",third:"Third",fourth:"Fourth",last:"Last"};const runOnDaysOfMonth={};for(let i=1;i<32;i++){runOnDaysOfMonth[i]=i}runOnDaysOfMonth.last="last";const runOnMonthTypes={any:"Of any month",specific:"Of specific month(s)"};const runOnDaysOfWeek={monday:"Monday",tuesday:"Tuesday",wednesday:"Wednesday",thursday:"Thursday",friday:"Friday",saturday:"Saturday",sunday:"Sunday"};const runOnMonths={january:"January",february:"February",march:"March",april:"April",may:"May",june:"June",july:"July",august:"August",september:"September",october:"October",november:"November",december:"December"};const runOnMonthOptions=`
		  ...
		  <svg viewBox="0 0 35 35" fill="none" xmlns="http://www.w3.org/2000/svg">
			...
			</div>`},onMount:function(step,updateStepMeta){emailPicker("#email-picker",false,items=>{EmailsStore.itemsFetched(items)}).on("change",function(e){updateStepMeta({email_id:parseInt($(this).val())},true)});$("#email-preview").on("load",function(e){this.height=this.contentWindow.document.body.offsetHeight;this.style.height=this.contentWindow.document.body.offsetHeight+"px";$(window).trigger("resize")});$("#render-email-edit").on("click",function(){Editor.view="editingEmail";Editor.renderEmailEditor()})},validate:function({meta:meta},errors){const{email_id:email_id}=meta;const email=EmailsStore.get(email_id);if(email_id&&email&&email.data.status!=="ready"){errors.push("Email is in draft mode. Please update status to ready!")}}},link_click:{defaults:{redirect_to:""},svg:`
		  <svg viewBox="0 0 35 35" fill="none" xmlns="http://www.w3.org/2000/svg">
		...
		<table class="mapping-table">
		...
		</table>`};const fieldMappingTableOnMount=updateStepMeta=>{$(".mappable-field").select2().on("change",function(e){const{meta:meta}=Editor.getCurrentStep();updateStepMeta({field_map:{...meta.field_map,[$(this).data("key")]:$(this).val()}})})};const formPicker=(selector,type,onReceiveItems=i=>{})=>{return apiPicker(selector,`${apiRoutes.funnels}/form-integration`,false,false,getResults=d=>{onReceiveItems(d.forms);return d.forms.map(form=>({id:form.id,text:form.name}))},getParams=q=>({...q,type:type}))};const formsCache={_cache:{},set:function(type,forms){this._cache[type]=forms},getAll:function(type){return this._cache[type]},hasType:function(type){return typeof this._cache[type]!=="undefined"},get:function(type,id){return this.hasType(type)?this._cache[type].find(f=>f.id===id):false}};const registerFormIntegration=(type,opts)=>{StepTypes.register(type,FormIntegration({type:type,...opts}))};const FormIntegration=({type:type,...rest})=>({...rest,defaults:{form_id:0,field_map:{}},title:function({meta:meta}){const{form_id:form_id}=meta;const form=formsCache.get(type,form_id);if(form){return`Submits <b>${form.name}</b>`}else{return`Submits <b></b>`}},edit:function({meta:meta}){const{form_id:form_id,field_map:field_map}=meta;const form=formsCache.get(type,form_id);return`
		  <div class="panel">
			  <div class="row">
		  ...
		  </div>`},onMount:function(step,updateStepMeta){formPicker("#form-id",type,items=>{formsCache.set(type,items)}).on("change",e=>{updateStepMeta({form_id:parseInt(e.target.value)},true)});fieldMappingTableOnMount(updateStepMeta)}});fill("beforeStepNotes.form_fill",{render:function({ID:ID,meta:meta}){if(!stepIsReal(ID)){return""}const copyValue=toCopy=>{return input({className:"code",value:toCopy,onfocus:"this.select()",readonly:true})};return`
